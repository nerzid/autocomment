...
public class ProcessExceptionHandlerTest extends AbstractBaseTest {
    ...
    public void testFaultWithoutHandler() {
        ...
        // add rule Reader{source} to ProcessExceptionHandlerTest{builder}
        builder.addRuleFlow(source);
        InternalKnowledgePackage pkg = builder.getPackage();
        PackageBuilderErrors errors = builder.getErrors();
        if ((errors != null) && (!(errors.isEmpty()))) {
            for (DroolsError error : errors.getErrors()) {
                ProcessExceptionHandlerTest.logger.error(error.toString());
            }
            Assert.fail("Package could not be compiled");
        }
        ...
        // assert equals void{STATE_ABORTED} to void{Assert}
        Assert.assertEquals(STATE_ABORTED, processInstance.getState());
    }
    ...
    public void testProcessExceptionHandlerAction() {
        ...
        // set global String{"list"} to StatefulKnowledgeSession{session}
        session.setGlobal("list", list);
        ProcessInstance processInstance = ((ProcessInstance) (session.startProcess("org.drools.exception")));
        // assert equals void{STATE_ACTIVE} to void{Assert}
        Assert.assertEquals(STATE_ACTIVE, processInstance.getState());
        // assert equals int{1} to void{Assert}
        Assert.assertEquals(1, list.size());
        // assert equals String{"SomeValue"} to void{Assert}
        Assert.assertEquals("SomeValue", list.get(0));
    }
    ...
    public void testProcessExceptionHandlerTriggerNode() {
        ...
        StatefulKnowledgeSession session = createKieSession(builder.getPackage());
        ProcessInstance processInstance = ((ProcessInstance) (session.startProcess("org.drools.exception")));
        // assert equals void{STATE_COMPLETED} to void{Assert}
        Assert.assertEquals(STATE_COMPLETED, processInstance.getState());
    }
