...
public class JbpmSerializationHelper extends SerializationHelper {
    ...
    public static StatefulKnowledgeSession getSerialisedStatefulKnowledgeSession(StatefulKnowledgeSession ksession, ObjectMarshallingStrategy[] strategies, boolean dispose) throws Exception {
        ...
        final byte[] b1 = JbpmSerializationHelper.serializeKnowledgeSession(marshaller, ksession);
        StatefulKnowledgeSession ksession2 = JbpmSerializationHelper.deserializeKnowledgeSession(marshaller, b1);
        final byte[] b2 = JbpmSerializationHelper.serializeKnowledgeSession(marshaller, ksession2);
        // bytes should be the same.
        if (!(JbpmSerializationHelper.areByteArraysEqual(b1, b2))) {
    ...
    public static boolean areByteArraysEqual(byte[] b1, byte[] b2) {
        if ((b1.length) != (b2.length)) {
            JbpmSerializationHelper.logger.info("Different length: b1={} b2={}", b1.length, b2.length);
            return false;
        } 
        for (int i = 0, length = b1.length; i < length; i++) {
            if ((b1[i]) != (b2[i])) {
                JbpmSerializationHelper.logger.info("Difference at {} : [{}] != [{}]", i, b1[i], b2[i]);
                return false;
            } 
        }
        return true;
