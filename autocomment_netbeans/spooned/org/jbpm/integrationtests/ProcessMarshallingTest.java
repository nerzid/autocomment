...
public class ProcessMarshallingTest extends AbstractBaseTest {
    ...
    public void testMarshallingProcessInstancesAndGlobals() throws Exception {
        ...
        Assert.assertEquals(1, ((List<Object>) (ksession.getGlobal("list"))).size());
        Assert.assertEquals(p, ((List<Object>) (ksession.getGlobal("list"))).get(0));
    ...
    public void testMarshallingProcessInstanceWithWorkItem() throws Exception {
        ...
        Assert.assertTrue(((handler.getWorkItem()) != null));
        ...
        Assert.assertEquals("ThisIsMyValue", variableScopeInstance.getVariable("myVariable"));
        session.getWorkItemManager().completeWorkItem(handler.getWorkItem().getId(), null);
        Assert.assertEquals(0, session.getProcessInstances().size());
    }
    ...
    public void testMarshallingWithHumanTaskAndRule() throws Exception {
        ...
        Assert.assertTrue(((handler1.getWorkItem()) != null));
        long workItemId = handler2.getWorkItem().getId();
        Assert.assertTrue((workItemId != (-1)));
        session = JbpmSerializationHelper.getSerialisedStatefulKnowledgeSession(session);
        session.getWorkItemManager().registerWorkItemHandler("Human Task", handler2);
        Assert.assertEquals(2, session.getProcessInstances().size());
        handler2.reset();
        session.getWorkItemManager().completeWorkItem(workItemId, null);
        Assert.assertTrue(((handler2.getWorkItem()) != null));
        Assert.assertEquals("John Doe", handler2.getWorkItem().getParameter("SwimlaneActorId"));
        Assert.assertEquals("x-value", handler2.getWorkItem().getParameter("Priority"));
    ...
    public void testMarshallingWithMultipleHumanTasks() throws Exception {
        ...
        builder.addProcessFromXml(new StringReader(process));
        StatefulKnowledgeSession session = createKieSession(builder.getPackage());
        ProcessMarshallingTest.TestListWorkItemHandler handler = new ProcessMarshallingTest.TestListWorkItemHandler();
        ...
        Assert.assertEquals(1, session.getProcessInstances().size());
        Assert.assertEquals(3, handler.getWorkItems().size());
    ...
    public void testMarshallingProcessInstanceWithTimer() throws Exception {
        ...
        Assert.assertEquals(1, session.getProcessInstances().size());
        session.halt();
        final StatefulKnowledgeSession session2 = JbpmSerializationHelper.getSerialisedStatefulKnowledgeSession(session);
        int sleeps = 3;
        ...
        Assert.assertEquals(0, session2.getProcessInstances().size());
        session2.halt();
    }
    ...
    private static class TestListWorkItemHandler implements WorkItemHandler {
        ...
        public void executeWorkItem(WorkItem workItem, WorkItemManager manager) {
            ProcessMarshallingTest.logger.debug("Executing workItem {}", workItem.getParameter("TaskName"));
    ...
    public void testVariablePersistenceMarshallingStrategies() throws Exception {
        ...
        VariableScopeInstance variableScopeInstance = ((VariableScopeInstance) (((ProcessInstance) (session.getProcessInstances().iterator().next())).getContextInstance(VariableScope.VARIABLE_SCOPE)));
        Assert.assertEquals("ThisIsMyValue", variableScopeInstance.getVariable("myVariable"));
        Assert.assertEquals(myPerson, variableScopeInstance.getVariable("myPerson"));
        session.getWorkItemManager().completeWorkItem(handler.getWorkItem().getId(), null);
        Assert.assertEquals(0, session.getProcessInstances().size());
    }
}
