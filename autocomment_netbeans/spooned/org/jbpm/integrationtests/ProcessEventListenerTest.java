...
public class ProcessEventListenerTest extends AbstractBaseTest {
    ...
    public void testInternalNodeSignalEvent() {
        Reader source = new StringReader(ProcessEventListenerTest.process);
        ...
        Assert.assertEquals(28, processEventList.size());
        for (ProcessEvent e : processEventList) {
            ProcessEventListenerTest.logger.debug(e.toString());
        }
        Assert.assertEquals("org.drools.core.event", ((ProcessStartedEvent) (processEventList.get(2))).getProcessInstance().getProcessId());
    }
