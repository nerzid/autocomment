...
public class JPAWorkItemHandler extends AbstractLogOrThrowWorkItemHandler implements Cacheable {
    ...
    public JPAWorkItemHandler(String persistenceUnit, ClassLoader cl) {
        ...
        try {
            ...
            JPAWorkItemHandler.this.emf = Persistence.createEntityManagerFactory(persistenceUnit);
        } finally {
            Thread.currentThread().setContextClassLoader(tccl);
        }
        JPAWorkItemHandler.this.classloader = cl;
    }

    public void executeWorkItem(WorkItem wi, WorkItemManager wim) {
        Object actionParam = wi.getParameter(JPAWorkItemHandler.P_ACTION);
        Object entity = wi.getParameter(JPAWorkItemHandler.P_ENTITY);
        Object id = wi.getParameter(JPAWorkItemHandler.P_ID);
        Object type = wi.getParameter(JPAWorkItemHandler.P_TYPE);
        Object queryName = wi.getParameter(JPAWorkItemHandler.P_QUERY);
        Object queryParams = wi.getParameter(JPAWorkItemHandler.P_QUERY_PARAMS);
        ...
        if ((entity == null) && (JPAWorkItemHandler.P_ACTION.equals(JPAWorkItemHandler.QUERY_ACTION))) {
        ...
        JPAWorkItemHandler.logger.debug("Action {} on {}", action, entity);
        EntityManager em = emf.createEntityManager();
        try {
            // join the process transaction
            em.joinTransaction();
            switch (action) {
                case JPAWorkItemHandler.DELETE_ACTION :
                    doDelete(em, entity, id);
                    break;
                case JPAWorkItemHandler.GET_ACTION :
                    ...
                    break;
                case JPAWorkItemHandler.UPDATE_ACTION :
                    doUpdate(em, entity);
                    break;
                case JPAWorkItemHandler.CREATE_ACTION :
                    em.persist(entity);
                    break;
                case JPAWorkItemHandler.QUERY_ACTION :
                    if (queryName == null) {
                        throw new IllegalArgumentException((("You must provide a '" + (JPAWorkItemHandler.P_QUERY)) + "' parameter to run named queries."));
                    } 
            ...
            JPAWorkItemHandler.logger.debug("Error performing JPA action ", e);
            throw e;
        } finally {
            em.close();
        }
        params.put(JPAWorkItemHandler.P_RESULT, entity);
        params.put(JPAWorkItemHandler.P_QUERY_RESULTS, queryResults);
    ...
    private List<Object> doQuery(EntityManager em, String queryName, Object queryParams) {
        JPAWorkItemHandler.logger.debug("About to run query {}", queryName);
        Map<String, Object> params;
        Query namedQuery = em.createQuery(queryName);
        if (queryParams == null) {
            JPAWorkItemHandler.logger.debug("No parameters were provided");
        } else {
            params = ((Map<String, Object>) (queryParams));
            JPAWorkItemHandler.logger.debug("Parameters {}", params);
