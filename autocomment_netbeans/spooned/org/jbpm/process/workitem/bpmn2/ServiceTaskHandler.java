...
SYNC, ASYNC, ONEWAY;    }
    ...
    public void executeWorkItem(WorkItem workItem, final WorkItemManager manager) {
        ...
        if ("##WebService".equalsIgnoreCase(implementation)) {
            ...
            ServiceTaskHandler.WSMode mode = ServiceTaskHandler.WSMode.valueOf(((workItem.getParameter("mode")) == null ? "SYNC" : ((String) (workItem.getParameter("mode"))).toUpperCase()));
            try {
                        ...
                        new Thread(new Runnable() {
                            public void run() {
                                try {
                                    ...
                                    ServiceTaskHandler.logger.error("Error encountered while invoking ws operation asynchronously ", e);
                                }
                            }
                        }).start();
    ...
    protected synchronized Client getWSClient(WorkItem workItem, String interfaceRef) {
        ...
        if (typedImports != null) {
            ...
            for (Bpmn2Import importObj : typedImports) {
                if (ServiceTaskHandler.WSDL_IMPORT_TYPE.equalsIgnoreCase(importObj.getType())) {
                    try {
                        ...
                        ServiceTaskHandler.logger.info(((("WS Client is created for {" + (importObj.getNamespace())) + "}") + interfaceRef));
                        return client;
                    } catch (Exception e) {
