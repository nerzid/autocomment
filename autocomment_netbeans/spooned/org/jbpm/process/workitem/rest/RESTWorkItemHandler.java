...
public class RESTWorkItemHandler extends AbstractLogOrThrowWorkItemHandler {
    ...
    private RESTWorkItemHandler.AuthenticationType type;
     ...
     */
    protected HttpResponse doRequestWithAuthorization(HttpClient httpclient, RequestBuilder requestBuilder, Map<String, Object> params, RESTWorkItemHandler.AuthenticationType type) {
        // no authorization
        if ((type == null) || (type == (RESTWorkItemHandler.AuthenticationType.NONE))) {
...
NONE, BASIC, FORM_BASED;    }
    ...
    protected HttpClient getHttpClient(Integer readTimeout, Integer connectTimeout) {
        if (RESTWorkItemHandler.HTTP_CLIENT_API_43) {
            RequestConfig config = RequestConfig.custom().setSocketTimeout(readTimeout).setConnectTimeout(connectTimeout).setConnectionRequestTimeout(connectTimeout).build();
