...
public class SendHtmlTest extends AbstractBaseTest {
    ...
    public void testConnectionEquals() {
        ...
        // null test
        Assert.assertTrue((!(connA.equals(null))));
        // different class test
        Assert.assertTrue((!(connA.equals("og"))));
        // extended class test
        SendHtmlTest.ExtendedConnection connExt = new SendHtmlTest.ExtendedConnection();
        Assert.assertTrue((!(connA.equals(connExt))));
        // null fields test
        Assert.assertTrue(connA.equals(connB));
    ...
    public void sendHtmlWithAuthentication() throws Exception {
        ...
        wiser.getServer().setAuthenticationHandlerFactory(new SendHtmlTest.TestAuthHandlerFactory());
        // Input
        String testMethodName = Thread.currentThread().getStackTrace()[1].getMethodName();
        ...
        // Setup email
        WorkItemImpl workItem = createEmailWorkItem(toAddress, fromAddress, testMethodName);
        Connection connection = new Connection(emailHost, emailPort, SendHtmlTest.authUsername, SendHtmlTest.authPassword);
        sendAndCheckThatMessagesAreSent(workItem, connection);
    }
    ...
    public void sendHtmlWithAuthenticationAndAttachments() throws Exception {
        ...
        Assert.assertEquals(workItem.getParameter("Subject"), message.getSubject());
        Assert.assertTrue(Arrays.equals(InternetAddress.parse(((String) (workItem.getParameter("To")))), message.getRecipients(RecipientType.TO)));
        Assert.assertTrue(((message.getContent()) instanceof Multipart));
        ...
        for (int i = 0; i < (multipart.getCount()); i++) {
            ...
            Assert.assertEquals("email.gif", bodyPart.getFileName());
        }
    }
    ...
    public void sendHtmlWithBadAuthentication() throws Exception {
        ...
        String fromAddress = "rgivens@kty.us.gov";
        checkBadAuthentication(toAddress, fromAddress, testMethodName, SendHtmlTest.authUsername, "bad password");
        checkBadAuthentication(toAddress, fromAddress, testMethodName, "badUserName", SendHtmlTest.authPassword);
    }
    ...
    public void useEmailWorkItemHandlerWithAuthentication() throws Exception {
        ...
        handler.setConnection(emailHost, emailPort, SendHtmlTest.authUsername, SendHtmlTest.authPassword);
        ...
        for (WiserMessage wiserMessage : messages) {
            ...
            Assert.assertEquals(workItem.getParameter("Subject"), message.getSubject());
            Assert.assertTrue(Arrays.equals(InternetAddress.parse(toAddress), message.getRecipients(RecipientType.TO)));
        }
    }
    ...
    private void checkBadAuthentication(String toAddress, String fromAddress, String testMethodName, String username, String password) {
        ...
        try {
            ...
            Assert.assertTrue(((("Unexpected exception of type " + (t.getClass().getSimpleName())) + ", not ") + (t.getClass().getSimpleName())), (t instanceof RuntimeException));
            Assert.assertNotNull("Expected RuntimeException to have a cause.", t.getCause());
            Throwable cause = t.getCause();
            Assert.assertNotNull("Expected cause to have a cause.", cause.getCause());
            cause = cause.getCause();
            Assert.assertTrue(((("Unexpected exception of type " + (cause.getClass().getSimpleName())) + ", not ") + (cause.getClass().getSimpleName())), (cause instanceof AuthenticationFailedException));
        }
    }
    ...
    private WorkItemImpl createEmailWorkItem(String toAddress, String fromAddress, String testMethodName) {
        ...
        workItem.setParameter("From", fromAddress);
        workItem.setParameter("Reply-To", fromAddress);
        String subject = (((SendHtmlTest.this.getClass().getSimpleName()) + " test message [") + (uniqueTestNum)) + "]";
        ...
        public TestAuthHandlerFactory() {
            UsernamePasswordValidator validator = new UsernamePasswordValidator() {
                public void login(String username, String password) throws LoginFailedException {
                    if ((!(SendHtmlTest.authUsername.equals(username))) || (!(SendHtmlTest.authPassword.equals(password)))) {
                        SendHtmlTest.logger.debug("Tried to login with user/password [{}/{}]", username, password);
                        throw new LoginFailedException(("Incorrect password for user " + (SendHtmlTest.authUsername)));
                    } 
                }
            };
            authHandleFactory.addFactory(new org.subethamail.smtp.auth.LoginAuthenticationHandlerFactory(validator));
            authHandleFactory.addFactory(new org.subethamail.smtp.auth.PlainAuthenticationHandlerFactory(validator));
        }
