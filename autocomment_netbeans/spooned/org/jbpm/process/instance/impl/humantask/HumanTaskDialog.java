...
public class HumanTaskDialog extends JDialog {
    ...
    private List<HumanTaskDialog.Result> results = new ArrayList<HumanTaskDialog.Result>();
    ...
    public HumanTaskDialog(HumanTaskHandler handler, WorkItem workItem) {
        ...
        HumanTaskDialog.this.handler = handler;
        HumanTaskDialog.this.workItem = workItem;
    ...
    private void initializeComponent() {
        ...
        for (Map.Entry<String, Object> entry : workItem.getParameters().entrySet()) {
            ...
            if ((((!("TaskName".equals(name))) && (!("Priority".equals(name)))) && (!("Comment".equals(name)))) && (!("ActorId".equals(name)))) {
                ...
                c.insets = new Insets(5, 5, 5, 5);
                panel.add(label, c);
                JTextField field = new JTextField(workItem.getParameter(name).toString());
    ...
    private void addResult() {
        ...
        if (("".equals(name)) || ("".equals(value))) {
            JOptionPane.showMessageDialog(HumanTaskDialog.this, "Name or value of result may not be null!", "Error", JOptionPane.ERROR_MESSAGE);
            return ;
        } 
        HumanTaskDialog.Result result = new HumanTaskDialog.Result(name, value);
        if (results.contains(result)) {
            JOptionPane.showMessageDialog(HumanTaskDialog.this, "Cannot add result more than once!", "Error", JOptionPane.ERROR_MESSAGE);
            return ;
        } 
        results.add(result);
        reloadResultList();
    ...
    private void complete() {
        ...
        if ((results.size()) > 0) {
            ...
            for (HumanTaskDialog.Result result : results) {
    ...
    public static class Result {
        ...
        public Result(String name, Object value) {
            HumanTaskDialog.Result.this.name = name;
            HumanTaskDialog.Result.this.value = value;
        }

        public String getName() {
            return HumanTaskDialog.Result.this.name;
        }

        public Object getValue() {
            return HumanTaskDialog.Result.this.value;
        }

        public String toString() {
            return ((HumanTaskDialog.Result.this.name) + " = ") + (HumanTaskDialog.Result.this.value);
        }

        public boolean equals(Object o) {
            if (o instanceof HumanTaskDialog.Result) {
                return ((HumanTaskDialog.Result) (o)).getName().equals(HumanTaskDialog.Result.this.name);
            } 
            return false;
        }

        public int hashCode() {
            return HumanTaskDialog.Result.this.name.hashCode();
        }
    }
}
