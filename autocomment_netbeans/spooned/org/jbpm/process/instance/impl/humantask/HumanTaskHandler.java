...
public class HumanTaskHandler extends JFrame implements WorkItemHandler {
    ...
    public WorkItem getSelectedWorkItem() {
        ...
        if (index != (-1)) {
            ...
            if (selected instanceof HumanTaskHandler.WorkItemWrapper) {
                return ((HumanTaskHandler.WorkItemWrapper) (selected)).getWorkItem();
            } 
        } 
        return null;
    }
    ...
    private void reloadWorkItemsList(String actor) {
        List<HumanTaskHandler.WorkItemWrapper> result = new ArrayList<HumanTaskHandler.WorkItemWrapper>();
        for (Iterator<WorkItem> iterator = workItems.keySet().iterator(); iterator.hasNext();) {
            WorkItem workItem = iterator.next();
            if (((actor == null) || ((workItem.getParameter("ActorId")) == null)) || (actor.equals(workItem.getParameter("ActorId")))) {
                result.add(new HumanTaskHandler.WorkItemWrapper(workItem));
            } 
        }
    ...
    private class WorkItemWrapper {
        ...
        public WorkItemWrapper(WorkItem workItem) {
            HumanTaskHandler.WorkItemWrapper.this.workItem = workItem;
        }
