...
public class ProcessRuntimeImpl implements InternalProcessRuntime {
    ...
    public ProcessRuntimeImpl(InternalKnowledgeRuntime kruntime) {
        ProcessRuntimeImpl.this.kruntime = kruntime;
        ...
        if (!((timerService.getTimerJobFactoryManager()) instanceof CommandServiceTimerJobFactoryManager)) {
            timerService.setTimerJobFactoryManager(new org.drools.core.time.impl.ThreadSafeTrackableTimeJobFactoryManager());
        } 
    ...
    private void initStartTimers() {
        ...
        for (Process process : processes) {
            ...
            if ((startNodes != null) && (!(startNodes.isEmpty()))) {
                kruntime.queueWorkingMemoryAction(new ProcessRuntimeImpl.RegisterStartTimerAction(p.getId(), startNodes, ProcessRuntimeImpl.this.timerManager));
    ...
    public ProcessRuntimeImpl(InternalWorkingMemory workingMemory) {
        ...
        ProcessRuntimeImpl.this.kruntime = ((InternalKnowledgeRuntime) (workingMemory.getKnowledgeRuntime()));
        initProcessInstanceManager();
        initSignalManager();
        timerManager = new TimerManager(kruntime, kruntime.getTimerService());
        processEventSupport = new ProcessEventSupport();
    ...
    private void initProcessInstanceManager() {
        ...
        try {
            processInstanceManager = ((ProcessInstanceManagerFactory) (loadClass(processInstanceManagerClass).newInstance())).createProcessInstanceManager(kruntime);
    ...
    private void initSignalManager() {
        ...
        try {
            signalManager = ((SignalManagerFactory) (loadClass(signalManagerClass).newInstance())).createSignalManager(kruntime);
        } catch (InstantiationException e) {
            throw new RuntimeException(e);
        } catch (IllegalAccessException e) {
            throw new RuntimeException(e);
    ...
    private ProcessInstance startProcess(final Process process, CorrelationKey correlationKey, Map<String, Object> parameters) {
        ...
        return conf.createProcessInstance(process, correlationKey, kruntime, parameters);
    }
                        ...
                        if (triggers != null) {
                            for (Trigger trigger : triggers) {
                                if (trigger instanceof EventTrigger) {
                                    ...
                                    ProcessRuntimeImpl.StartProcessEventListener listener = new ProcessRuntimeImpl.StartProcessEventListener(process.getId(), filters, trigger.getInMappings(), startNode.getEventTransformer());
    ...
    public void addEventListener(final ProcessEventListener listener) {
        ProcessRuntimeImpl.this.processEventSupport.addEventListener(listener);
    }

    public void removeEventListener(final ProcessEventListener listener) {
        ProcessRuntimeImpl.this.processEventSupport.removeEventListener(listener);
    }
    ...
    private class StartProcessEventListener implements EventListener {
        ...
        public StartProcessEventListener(String processId, List<EventFilter> eventFilters, Map<String, String> inMappings, EventTransformer eventTransformer) {
            ProcessRuntimeImpl.StartProcessEventListener.this.processId = processId;
            ProcessRuntimeImpl.StartProcessEventListener.this.eventFilters = eventFilters;
            ProcessRuntimeImpl.StartProcessEventListener.this.inMappings = inMappings;
            ProcessRuntimeImpl.StartProcessEventListener.this.eventTransformer = eventTransformer;
        }
        ...
        public void signalEvent(final String type, Object event) {
            ...
            if (manager != null) {
                ...
                ksession.execute(new ProcessRuntimeImpl.StartProcessWithTypeCommand(processId, params, type));
            } else {
    ...
    private class StartProcessWithTypeCommand implements GenericCommand<Void> {
        ...
        private StartProcessWithTypeCommand(String processId, Map<String, Object> params, String type) {
            ProcessRuntimeImpl.StartProcessWithTypeCommand.this.processId = processId;
            ProcessRuntimeImpl.StartProcessWithTypeCommand.this.params = params;
            ProcessRuntimeImpl.StartProcessWithTypeCommand.this.type = type;
        }
    ...
    public void setProcessEventSupport(ProcessEventSupport processEventSupport) {
        ProcessRuntimeImpl.this.processEventSupport = processEventSupport;
    }

    public void dispose() {
        ProcessRuntimeImpl.this.processEventSupport.reset();
        ProcessRuntimeImpl.this.timerManager.dispose();
        kruntime = null;
    }

    public void clearProcessInstances() {
        ProcessRuntimeImpl.this.processInstanceManager.clearProcessInstances();
    }

    public void clearProcessInstancesState() {
        ProcessRuntimeImpl.this.processInstanceManager.clearProcessInstancesState();
    }
    ...
    public static class RegisterStartTimerAction extends PropagationEntry.AbstractPropagationEntry implements WorkingMemoryAction {
        ...
        public RegisterStartTimerAction(String processId, List<StartNode> startNodes, TimerManager timerManager) {
            ProcessRuntimeImpl.RegisterStartTimerAction.this.processId = processId;
            ProcessRuntimeImpl.RegisterStartTimerAction.this.startNodes = startNodes;
            ProcessRuntimeImpl.RegisterStartTimerAction.this.timerManager = timerManager;
        }
