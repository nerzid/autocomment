...
import JBPMMessages.ProcessTimer;
...
public class TimerManager {
    ...
    public static final Job processJob = new TimerManager.ProcessJob();

    public static final Job startProcessJob = new TimerManager.StartProcessJob();

    public TimerManager(InternalKnowledgeRuntime kruntime, TimerService timerService) {
        TimerManager.this.kruntime = kruntime;
        TimerManager.this.timerService = timerService;
    }
    ...
    public void registerTimer(final TimerInstance timer, ProcessInstance processInstance) {
        try {
            ...
            if ((timer.getCronExpression()) != null) {
                ...
                trigger = new org.drools.core.time.impl.CronTrigger(timerService.getCurrentTime(), startTime, null, (-1), timer.getCronExpression(), null, null);
                // cron timers are by nature repeatable
                timer.setPeriod(1);
            } else {
                trigger = new org.drools.core.time.impl.IntervalTrigger(timerService.getCurrentTime(), null, null, timer.getRepeatLimit(), timer.getDelay(), timer.getPeriod(), null, null);
            }
            TimerManager.ProcessJobContext ctx = new TimerManager.ProcessJobContext(timer, trigger, processInstance.getId(), TimerManager.this.kruntime);
            JobHandle jobHandle = TimerManager.this.timerService.scheduleJob(TimerManager.processJob, ctx, trigger);
    ...
    public void registerTimer(final TimerInstance timer, String processId, Map<String, Object> params) {
        try {
            ...
            TimerManager.StartProcessJobContext ctx = new TimerManager.StartProcessJobContext(timer, trigger, processId, params, TimerManager.this.kruntime);
            JobHandle jobHandle = TimerManager.this.timerService.scheduleJob(TimerManager.startProcessJob, ctx, trigger);
            timer.setJobHandle(jobHandle);
            timers.put(timer.getId(), timer);
        } finally {
            kruntime.endOperation();
    ...
    public void internalAddTimer(final TimerInstance timer) {
        ...
        Trigger trigger = new org.drools.core.time.impl.IntervalTrigger(timerService.getCurrentTime(), null, null, (-1), delay, timer.getPeriod(), null, null);
        TimerManager.ProcessJobContext ctx = new TimerManager.ProcessJobContext(timer, trigger, timer.getProcessInstanceId(), TimerManager.this.kruntime);
        JobHandle jobHandle = TimerManager.this.timerService.scheduleJob(TimerManager.processJob, ctx, trigger);
        timer.setJobHandle(jobHandle);
        timers.put(timer.getId(), timer);
    }
    ...
    public TimerService getTimerService() {
        return TimerManager.this.timerService;
    }
    ...
    public Map<Long, TimerInstance> getTimerMap() {
        return TimerManager.this.timers;
    }
    ...
    public void internalSetTimerId(long timerId) {
        TimerManager.this.timerId = timerId;
    }
    ...
    public static class ProcessTimerInputMarshaller implements TimersInputMarshaller {
        public void deserialize(MarshallerReaderContext inCtx, Timer timer) throws ClassNotFoundException {
            ...
            TimerService ts = inCtx.getTimerService();
            ...
            TimerManager tm = ((InternalProcessRuntime) (inCtx.getProcessRuntime())).getTimerManager();
            ...
            if (!(tm.getTimerMap().containsKey(timerInstance.getId()))) {
                TimerManager.ProcessJobContext pctx = new TimerManager.ProcessJobContext(timerInstance, trigger, processInstanceId, inCtx.getKnowledgeRuntime());
    ...
    public static class ProcessJob implements Serializable , Job {
        private static final long serialVersionUID = 6004839244692770390L;

        public void execute(JobContext c) {
            TimerManager.ProcessJobContext ctx = ((TimerManager.ProcessJobContext) (c));
    ...
    public static class StartProcessJob implements Serializable , Job {
        private static final long serialVersionUID = 1039445333595469160L;

        public void execute(JobContext c) {
            TimerManager.StartProcessJobContext ctx = ((TimerManager.StartProcessJobContext) (c));
            InternalKnowledgeRuntime kruntime = ctx.getKnowledgeRuntime();
            try {
                ctx.getTimer().setLastTriggered(new Date(ctx.getKnowledgeRuntime().<SessionClock>getSessionClock().getCurrentTime()));
                // if there is no more trigger reset period on timer so its node instance can be removed
                ...
                TimerManager.logger.error((("Error when executing start process " + (ctx.getProcessId())) + " timer job"), e);
            }
        }
    }
    ...
    public static class ProcessJobContext implements JobContext {
        ...
        public ProcessJobContext(final TimerInstance timer, final Trigger trigger, final Long processInstanceId, final InternalKnowledgeRuntime kruntime) {
            TimerManager.ProcessJobContext.this.timer = timer;
            TimerManager.ProcessJobContext.this.trigger = trigger;
            TimerManager.ProcessJobContext.this.processInstanceId = processInstanceId;
            TimerManager.ProcessJobContext.this.kruntime = kruntime;
            TimerManager.ProcessJobContext.this.sessionId = timer.getSessionId();
        }
        ...
        public JobHandle getJobHandle() {
            return TimerManager.ProcessJobContext.this.jobHandle;
        }

        public void setJobHandle(JobHandle jobHandle) {
            TimerManager.ProcessJobContext.this.jobHandle = jobHandle;
        }
    ...
    public static class StartProcessJobContext extends TimerManager.ProcessJobContext {
        ...
        public StartProcessJobContext(TimerInstance timer, Trigger trigger, String processId, Map<String, Object> params, InternalKnowledgeRuntime kruntime) {
            ...
            TimerManager.StartProcessJobContext.this.processId = processId;
            TimerManager.StartProcessJobContext.this.paramaeters = params;
        }
        ...
        public void setParamaeters(Map<String, Object> paramaeters) {
            TimerManager.StartProcessJobContext.this.paramaeters = paramaeters;
        }
    }
    ...
    public static class OverdueTrigger implements Trigger {
        ...
        public OverdueTrigger(Trigger orig, InternalKnowledgeRuntime kruntime) {
            TimerManager.OverdueTrigger.this.orig = orig;
            TimerManager.OverdueTrigger.this.kruntime = kruntime;
        }
        ...
        public Date hasNextFireTime() {
            ...
            // overdue timer
            if (then < now) {
                return new Date((now + (TimerManager.OverdueTrigger.OVERDUE_DELAY)));
            } else {
