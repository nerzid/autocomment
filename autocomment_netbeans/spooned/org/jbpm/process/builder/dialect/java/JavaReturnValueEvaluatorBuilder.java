...
public class JavaReturnValueEvaluatorBuilder extends AbstractJavaProcessBuilder implements ReturnValueEvaluatorBuilder {
    ...
    public void build(final PackageBuildContext context, final ReturnValueConstraintEvaluator constraintNode, final ReturnValueDescr descr, final ContextResolver contextResolver) {
        ...
        // build return PackageBuildContext{context} to JavaReturnValueEvaluatorBuilder{}
        buildReturnValueEvaluator(context, constraintNode, descr, contextResolver, className, analysis);
    }
    ...
    protected void buildReturnValueEvaluator(final PackageBuildContext context, final ReturnValueConstraintEvaluator constraintNode, final ReturnValueDescr descr, final ContextResolver contextResolver, String className, AnalysisResult analysis) {
        Set<String> identifiers = analysis.getBoundIdentifiers().getGlobals().keySet();
        final Map map = createVariableContext(className, descr.getText(), ((ProcessBuildContext) (context)), ((String[]) (identifiers.toArray(new String[identifiers.size()]))), analysis.getNotBoundedIdentifiers(), contextResolver);
        // put String{"text"} to Map{map}
        map.put("text", descr.getText());
        // generate templates String{"returnValueEvaluatorMethod"} to JavaReturnValueEvaluatorBuilder{}
        generateTemplates("returnValueEvaluatorMethod", "returnValueEvaluatorInvoker", ((ProcessBuildContext) (context)), className, map, constraintNode, descr);
        // collect types String{"JavaReturnValue"} to JavaReturnValueEvaluatorBuilder{}
