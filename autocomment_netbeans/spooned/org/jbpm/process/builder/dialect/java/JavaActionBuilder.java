...
public class JavaActionBuilder extends AbstractJavaProcessBuilder implements ActionBuilder {
    public void build(final PackageBuildContext context, final DroolsAction action, final ActionDescr actionDescr, final ContextResolver contextResolver) {
        ...
        // build action PackageBuildContext{context} to JavaActionBuilder{}
    ...
    protected void buildAction(final PackageBuildContext context, final DroolsAction action, final ActionDescr actionDescr, final ContextResolver contextResolver, String className, AnalysisResult analysis) {
        Set<String> identifiers = analysis.getBoundIdentifiers().getGlobals().keySet();
        final Map map = createVariableContext(className, actionDescr.getText(), ((ProcessBuildContext) (context)), ((String[]) (identifiers.toArray(new String[identifiers.size()]))), analysis.getNotBoundedIdentifiers(), contextResolver);
        // put String{"text"} to Map{map}
        map.put("text", ProcessKnowledgeHelperFixer.fix(actionDescr.getText()));
        // generate templates String{"actionMethod"} to JavaActionBuilder{}
        generateTemplates("actionMethod", "actionInvoker", ((ProcessBuildContext) (context)), className, map, action, actionDescr);
        // collect types String{"JavaDialect"} to JavaActionBuilder{}
