...
public class StandaloneJtaStrategy implements PersistenceStrategy {
    ...
    public StandaloneJtaStrategy(EntityManagerFactory emf) {
        StandaloneJtaStrategy.this.emf = emf;
    }
    ...
    public Object joinTransaction(EntityManager em) {
        boolean newTx = false;
        UserTransaction ut = StandaloneJtaStrategy.findUserTransaction();
        if (ut == null) {
        ...
        return StandaloneJtaStrategy.USER_MANAGED_TRANSACTION;
    }
    ...
    protected static UserTransaction findUserTransaction() {
        ...
        try {
            ...
            for (String utLookup : StandaloneJtaStrategy.KNOWN_UT_JNDI_KEYS) {
                if (utLookup != null) {
                    try {
                        UserTransaction ut = ((UserTransaction) (context.lookup(utLookup)));
                        return ut;
                    } catch (NamingException e) {
                        StandaloneJtaStrategy.logger.debug("User Transaction not found in JNDI under {}", utLookup);
                    }
                } 
            }
            StandaloneJtaStrategy.logger.warn("No user transaction found under known names");
            return null;
        }
    }
    ...
    protected void commitTransaction(Object transaction) {
        if (transaction == (StandaloneJtaStrategy.USER_MANAGED_TRANSACTION)) {
            return ;
        } 
        ...
        try {
            ...
            StandaloneJtaStrategy.logger.error("Unable to commit transaction: ", e);
        }
    }
