...
public class JPAWorkingMemoryDbLogger extends AbstractAuditLogger {
    ...
    public JPAWorkingMemoryDbLogger(WorkingMemory workingMemory) {
        ...
        if (processRuntime != null) {
            processRuntime.addEventListener(((ProcessEventListener) (JPAWorkingMemoryDbLogger.this)));
        } 
    }
    ...
    public JPAWorkingMemoryDbLogger(KieSession session) {
        ...
        session.addEventListener(JPAWorkingMemoryDbLogger.this);
    }

    /* end of backward compatibility */
    public JPAWorkingMemoryDbLogger(EntityManagerFactory emf) {
        JPAWorkingMemoryDbLogger.this.emf = emf;
    }
    ...
    public void afterVariableChanged(ProcessVariableChangedEvent event) {
        List<VariableInstanceLog> variables = indexManager.index(getBuilder(), event);
    ...
    private void leaveTransaction(EntityManager em, Object transaction) {
        if (isJTA) {
            try {
                ...
                JPAWorkingMemoryDbLogger.logger.error("Unable to commit transaction: ", e);
            }
        } else {
            if (transaction != null) {
        ...
        if (!(sharedEM)) {
            try {
                ...
                JPAWorkingMemoryDbLogger.logger.error("Unable to close created EntityManager: {}", e.getMessage(), e);
            }
        } 
    }
    ...
    protected static UserTransaction findUserTransaction() {
        ...
        try {
            ...
            for (String utLookup : JPAWorkingMemoryDbLogger.KNOWN_UT_JNDI_KEYS) {
                if (utLookup != null) {
                    try {
                        UserTransaction ut = ((UserTransaction) (context.lookup(utLookup)));
                        return ut;
                    } catch (NamingException e) {
                        JPAWorkingMemoryDbLogger.logger.debug("User Transaction not found in JNDI under {}", utLookup);
                    }
                } 
            }
            JPAWorkingMemoryDbLogger.logger.warn("No user transaction found under known names");
            return null;
        }
    }
}
