...
public class GlobalTimerService implements InternalSchedulerService , TimerService {
    ...
    public GlobalTimerService(RuntimeManager manager, GlobalSchedulerService schedulerService) {
        GlobalTimerService.this.manager = manager;
        GlobalTimerService.this.schedulerService = schedulerService;
        GlobalTimerService.this.schedulerService.initScheduler(GlobalTimerService.this);
        try {
            GlobalTimerService.this.jobFactoryManager = ((TimerJobFactoryManager) (Class.forName("org.jbpm.persistence.timer.GlobalJPATimerJobFactoryManager").newInstance()));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public JobHandle scheduleJob(Job job, JobContext ctx, Trigger trigger) {
        if (ctx instanceof ProcessJobContext) {
            ...
            GlobalTimerService.GlobalJobHandle jobHandle = ((GlobalTimerService.GlobalJobHandle) (GlobalTimerService.this.schedulerService.scheduleJob(job, ctx, trigger)));
            if (jobHandle != null) {
    ...
    public boolean removeJob(JobHandle jobHandle) {
        ...
        if (handles == null) {
            ...
            return GlobalTimerService.this.schedulerService.removeJob(jobHandle);
        } 
    ...
    public void destroy() {
        ...
        for (List<GlobalTimerService.GlobalJobHandle> handles : activeTimers) {
            for (GlobalTimerService.GlobalJobHandle handle : handles) {
                GlobalTimerService.this.schedulerService.removeJob(handle);
            }
        }
    }
    ...
    public void internalSchedule(TimerJobInstance timerJobInstance) {
        if ((GlobalTimerService.this.schedulerService) instanceof InternalSchedulerService) {
            ((InternalSchedulerService) (GlobalTimerService.this.schedulerService)).internalSchedule(timerJobInstance);
        } else {
            throw new UnsupportedOperationException(("Unsupported scheduler operation internalSchedule on class " + (GlobalTimerService.this.schedulerService.getClass())));
        }
    }
    ...
    public TimerJobFactoryManager getTimerJobFactoryManager() {
        return GlobalTimerService.this.jobFactoryManager;
    }
    ...
    public void setTimerServiceId(String timerServiceId) {
        GlobalTimerService.this.timerServiceId = timerServiceId;
    }

    public JobHandle buildJobHandleForContext(NamedJobContext ctx) {
        return GlobalTimerService.this.schedulerService.buildJobHandleForContext(ctx);
    }
    ...
    public static class GlobalJobHandle extends DefaultJobHandle implements JobHandle {
        ...
        public long getTimerId() {
            JobContext ctx = GlobalTimerService.GlobalJobHandle.this.getTimerJobInstance().getJobContext();
        ...
        public long getSessionId() {
            if ((GlobalTimerService.GlobalJobHandle.this.getTimerJobInstance()) != null) {
                JobContext ctx = GlobalTimerService.GlobalJobHandle.this.getTimerJobInstance().getJobContext();
                if (ctx instanceof SelfRemovalJobContext) {
                    ctx = ((SelfRemovalJobContext) (ctx)).getJobContext();
                } 
    ...
    public static class DisposableCommandService implements CommandService {
        ...
        public DisposableCommandService(CommandService delegate, RuntimeManager manager, RuntimeEngine runtime, boolean retry) {
            GlobalTimerService.DisposableCommandService.this.delegate = delegate;
            GlobalTimerService.DisposableCommandService.this.manager = manager;
            GlobalTimerService.DisposableCommandService.this.runtime = runtime;
            GlobalTimerService.DisposableCommandService.this.retry = retry;
        }
