...
import Node.CONNECTION_DEFAULT_TYPE;
...
public class ForEachNodeFactory extends RuleFlowNodeContainerFactory {
    ...
    public ForEachNodeFactory collectionExpression(String collectionExpression) {
        // set collection String{collectionExpression} to ForEachNode{getForEachNode()}
    ...
    public ForEachNodeFactory variable(String variableName, DataType dataType) {
        // set variable String{variableName} to ForEachNode{getForEachNode()}
        getForEachNode().setVariable(variableName, dataType);
        return this;
    }

    public ForEachNodeFactory waitForCompletion(boolean waitForCompletion) {
        // set wait boolean{waitForCompletion} to ForEachNode{getForEachNode()}
    ...
    public RuleFlowNodeContainerFactory done() {
        if ((linkedIncomingNodeId) != (-1)) {
            getForEachNode().linkIncomingConnections(CONNECTION_DEFAULT_TYPE, linkedIncomingNodeId, CONNECTION_DEFAULT_TYPE);
        }
        if ((linkedOutgoingNodeId) != (-1)) {
            getForEachNode().linkOutgoingConnections(linkedOutgoingNodeId, CONNECTION_DEFAULT_TYPE, CONNECTION_DEFAULT_TYPE);
        }
        // add node ForEachNode{getForEachNode()} to NodeContainer{nodeContainer}
