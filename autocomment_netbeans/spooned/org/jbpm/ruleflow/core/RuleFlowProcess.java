...
public class RuleFlowProcess extends WorkflowProcessImpl {
    ...
    public RuleFlowProcess() {
        setType(RuleFlowProcess.RULEFLOW_TYPE);
    ...
    protected NodeContainer createNodeContainer() {
        return new RuleFlowProcess.WorkflowProcessNodeContainer();
    }

    public List<Node> getStartNodes() {
        return RuleFlowProcess.getStartNodes(RuleFlowProcess.this.getNodes());
    }
    ...
    public List<Node> getAutoStartNodes() {
        ...
        List<Node> nodes = Arrays.stream(getNodes()).filter(( n) -> (n.getIncomingConnections().isEmpty()) && ("true".equalsIgnoreCase(((String) (n.getMetaData().get("customAutoStart")))))).collect(Collectors.toList());
        return nodes;
    }
