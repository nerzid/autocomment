...
public class TransactionAwareSchedulerServiceInterceptor extends DelegateSchedulerServiceInterceptor {
    ...
    public TransactionAwareSchedulerServiceInterceptor(RuntimeEnvironment environment, RuntimeManager manager, GlobalSchedulerService schedulerService) {
        ...
        TransactionAwareSchedulerServiceInterceptor.this.environment = environment;
        TransactionAwareSchedulerServiceInterceptor.this.manager = manager;
    }
    ...
    public final void internalSchedule(final TimerJobInstance timerJobInstance) {
        ...
        if ((((tm.getStatus()) != (TransactionManager.STATUS_NO_TRANSACTION)) && ((tm.getStatus()) != (TransactionManager.STATUS_ROLLEDBACK))) && ((tm.getStatus()) != (TransactionManager.STATUS_COMMITTED))) {
            TransactionManagerHelper.registerTransactionSyncInContainer(tm, new TransactionAwareSchedulerServiceInterceptor.ScheduleTimerTransactionSynchronization(timerJobInstance, delegate));
            return ;
        } 
        super.internalSchedule(timerJobInstance);
    }
    ...
    private class ScheduleTimerTransactionSynchronization extends OrderedTransactionSynchronization {
        ...
        ScheduleTimerTransactionSynchronization(TimerJobInstance timerJobInstance, GlobalSchedulerService schedulerService) {
            ...
            TransactionAwareSchedulerServiceInterceptor.ScheduleTimerTransactionSynchronization.this.timerJobInstance = timerJobInstance;
            TransactionAwareSchedulerServiceInterceptor.ScheduleTimerTransactionSynchronization.this.schedulerService = schedulerService;
        }
        ...
        public void afterCompletion(int status) {
            if ((status == (TransactionManager.STATUS_COMMITTED)) && (!(timerJobInstance.getJobHandle().isCancel()))) {
                TransactionAwareSchedulerServiceInterceptor.ScheduleTimerTransactionSynchronization.this.schedulerService.internalSchedule(timerJobInstance);
            } 
        }

        @Override
        public int compareTo(OrderedTransactionSynchronization o) {
            if (o instanceof TransactionAwareSchedulerServiceInterceptor.ScheduleTimerTransactionSynchronization) {
                if (TransactionAwareSchedulerServiceInterceptor.ScheduleTimerTransactionSynchronization.this.timerJobInstance.equals(((TransactionAwareSchedulerServiceInterceptor.ScheduleTimerTransactionSynchronization) (o)).timerJobInstance)) {
                    return 0;
                } 
                return -1;
            } 
