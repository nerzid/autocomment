...
public class PerProcessInstanceRuntimeManagerTest extends AbstractBaseTest {
    ...
    public void testCreationOfSession() {
        ...
        Assert.assertTrue((ksession2Id == 2));
    ...
    public void testCreationOfSessionWithPersistence() {
        ...
        Assert.assertNotNull(ksession2);
        long ksession2Id = ksession2.getIdentifier();
        Assert.assertTrue((ksession2Id == 3));
        ProcessInstance pi1 = ksession.startProcess("UserTask");
        ProcessInstance pi2 = ksession2.startProcess("UserTask");
        // both processes started
        Assert.assertEquals(ProcessInstance.STATE_ACTIVE, pi1.getState());
        ...
        try {
            ...
            Assert.fail((("Session for this (" + (pi1.getId())) + ") process instance is no more accessible"));
        ...
        try {
            ...
            Assert.fail((("Session for this (" + (pi2.getId())) + ") process instance is no more accessible"));
        } catch (RuntimeException e) {
        }
        manager.close();
    }
    ...
    public void testExecuteCompleteWorkItemOnInvalidSessionWithPersistence() {
        ...
        try {
            ...
            Assert.fail((("Invalid session was used for (" + (pi2.getId())) + ") process instance"));
        } catch (RuntimeException e) {
        }
        manager.close();
    }
    ...
    public void testExecuteReusableSubprocess() {
        ...
        try {
            ksession.getWorkItemManager().completeWorkItem(1, null);
            Assert.fail((("Invalid session was used for subprocess of (" + (pi1.getId())) + ") process instance"));
        } catch (RuntimeException e) {
        }
    ...
    public void testCreationOfSessionTaskServiceNotConfigured() {
        RuntimeEnvironment environment = RuntimeEnvironmentBuilder.Factory.get().newEmptyBuilder().userGroupCallback(userGroupCallback).addAsset(ResourceFactory.newClassPathResource("BPMN2-ScriptTask.bpmn2"), ResourceType.BPMN2).addAsset(ResourceFactory.newClassPathResource("BPMN2-UserTask.bpmn2"), ResourceType.BPMN2).get();
        manager = RuntimeManagerFactory.Factory.get().newPerProcessInstanceRuntimeManager(environment);
        Assert.assertNotNull(manager);
        // ksession for process instance #1
        // since there is no process instance yet we need to get new session
    ...
    public void testCreationOfSessionWithCustomTaskListener() {
        ...
        ksession2 = runtime2.getKieSession();
        Assert.assertEquals(ksession2Id, ksession2.getIdentifier());
        Assert.assertEquals(2, addedTasks.size());
        manager.close();
    }
    ...
    public void testCreationOfSessionCustomTaskServiceFactory() {
        ...
        RuntimeEnvironment environment = RuntimeEnvironmentBuilder.Factory.get().newDefaultInMemoryBuilder().userGroupCallback(userGroupCallback).entityManagerFactory(emf).addAsset(ResourceFactory.newClassPathResource("BPMN2-ScriptTask.bpmn2"), ResourceType.BPMN2).addAsset(ResourceFactory.newClassPathResource("BPMN2-UserTask.bpmn2"), ResourceType.BPMN2).addEnvironmentEntry("org.kie.internal.runtime.manager.TaskServiceFactory", new TaskServiceFactory() {
            ...
            public TaskService newTaskService() {
                ...
                return HumanTaskServiceFactory.newTaskServiceConfigurator().entityManagerFactory(produceEntityManagerFactory()).listener(new org.jbpm.services.task.audit.JPATaskLifeCycleEventListener(true)).getTaskService();
            }
        ...
        Assert.assertTrue(customTaskServiceUsed.get());
    }
    ...
    public void testRestoreTimersAfterManagerClose() throws Exception {
        ...
        manager.disposeRuntimeEngine(runtime);
        manager.close();
        Assert.assertTrue(((timerExpirations.size()) > currentNumberOfTriggers));
    }

    @Test(expected = UnsupportedOperationException.class)
    public void testAuditServiceNotAvailable() {
        RuntimeEnvironment environment = RuntimeEnvironmentBuilder.Factory.get().newDefaultInMemoryBuilder().userGroupCallback(userGroupCallback).addAsset(ResourceFactory.newClassPathResource("BPMN2-ScriptTask.bpmn2"), ResourceType.BPMN2).addAsset(ResourceFactory.newClassPathResource("BPMN2-UserTask.bpmn2"), ResourceType.BPMN2).get();
        manager = RuntimeManagerFactory.Factory.get().newPerProcessInstanceRuntimeManager(environment);
        Assert.assertNotNull(manager);
        // ksession for process instance #1
        // since there is no process instance yet we need to get new session
    ...
    public void testIndependentSubprocessAbort() {
        ...
        Assert.assertNotNull(logs);
        Assert.assertEquals(1, logs.size());
        Assert.assertEquals(ProcessInstance.STATE_ABORTED, ((int) (logs.get(0).getStatus())));
        logs = logService.findProcessInstances("SubProcess");
        Assert.assertNotNull(logs);
        Assert.assertEquals(1, logs.size());
        Assert.assertEquals(ProcessInstance.STATE_ACTIVE, ((int) (logs.get(0).getStatus())));
        manager.close();
    }
    ...
    public void testSignalStartMultipleProcesses() {
        ...
        Assert.assertEquals(3, processInstanceLogs.size());
        manager.close();
    }
}
