...
public class InMemorySessionFactory implements SessionFactory {
    ...
    public InMemorySessionFactory(RuntimeEnvironment environment) {
        InMemorySessionFactory.this.environment = environment;
        InMemorySessionFactory.this.kbase = environment.getKieBase();
    }

    @Override
    public KieSession newKieSession() {
        KieSession ksession = kbase.newKieSession(environment.getConfiguration(), environment.getEnvironment());
        InMemorySessionFactory.this.sessions.put(ksession.getIdentifier(), ksession);
        return ksession;
    }
    ...
    public KieSession findKieSessionById(Long sessionId) {
        if (sessions.containsKey(sessionId)) {
            ...
            throw new org.kie.internal.runtime.manager.SessionNotFoundException((("Session with id " + sessionId) + " was not found"));
        }
    }
