...
public class ExecutorImpl implements Executor {
    ...
    public void setEventSupport(ExecutorEventSupport eventSupport) {
        ExecutorImpl.this.eventSupport = eventSupport;
    }

    public void setExecutorStoreService(ExecutorStoreService executorStoreService) {
        ExecutorImpl.this.executorStoreService = executorStoreService;
    }
    ...
    public void setConnectionFactoryName(String connectionFactoryName) {
        ExecutorImpl.this.connectionFactoryName = connectionFactoryName;
    }
    ...
    public void setQueueName(String queueName) {
        ExecutorImpl.this.queueName = queueName;
    }
    ...
    public void setConnectionFactory(ConnectionFactory connectionFactory) {
        ExecutorImpl.this.connectionFactory = connectionFactory;
    }
    ...
    public void setInterval(int interval) {
        ExecutorImpl.this.interval = interval;
    }
    ...
    public void setRetries(int retries) {
        ExecutorImpl.this.retries = retries;
    }
    ...
    public void setThreadPoolSize(int threadPoolSize) {
        ExecutorImpl.this.threadPoolSize = threadPoolSize;
    }
    ...
    public void setTimeunit(TimeUnit timeunit) {
        ExecutorImpl.this.timeunit = timeunit;
    }
    ...
    public void init() {
        if (!("true".equalsIgnoreCase(System.getProperty("org.kie.executor.disabled")))) {
            ExecutorImpl.logger.info(("Starting Executor Component ...\n" + (" \t - Thread Pool Size: {}" + ("\n" + (" \t - Interval: {} {} \n" + " \t - Retries per Request: {}\n")))), threadPoolSize, interval, timeunit.toString(), retries);
            int delayIncremental = 0;
            scheduler = Executors.newScheduledThreadPool(threadPoolSize);
            for (int i = 0; i < (threadPoolSize); i++) {
                long delay = 2000 + delayIncremental;
                long interval = TimeUnit.MILLISECONDS.convert(ExecutorImpl.this.interval, timeunit);
                ExecutorImpl.logger.debug("Starting executor thread with initial delay {} interval {} and time unit {}", delay, interval, TimeUnit.MILLISECONDS);
                handle.add(scheduler.scheduleAtFixedRate(executorStoreService.buildExecutorRunnable(), delay, interval, TimeUnit.MILLISECONDS));
                delayIncremental += ExecutorImpl.this.initialDelay;
            }
            if (useJMS) {
                try {
                    InitialContext ctx = new InitialContext();
                    if ((ExecutorImpl.this.connectionFactory) == null) {
                        ExecutorImpl.this.connectionFactory = ((ConnectionFactory) (ctx.lookup(connectionFactoryName)));
                    } 
                    if ((ExecutorImpl.this.queue) == null) {
                        ExecutorImpl.this.queue = ((Queue) (ctx.lookup(queueName)));
                    } 
                    ExecutorImpl.logger.info("Executor JMS based support successfully activated on queue {}", queue);
                } catch (Exception e) {
                    ExecutorImpl.logger.warn("Disabling JMS support in executor because: unable to initialize JMS configuration for executor due to {}", e.getMessage());
                    ExecutorImpl.logger.debug("JMS support executor failed due to {}", e.getMessage(), e);
                    // since it cannot be initialized disable jms
                    useJMS = false;
                }
            } 
        } else {
            throw new org.jbpm.executor.ExecutorNotStartedException();
        }
    }
     ...
     */
    public void destroy() {
        ExecutorImpl.logger.info(" >>>>> Destroying Executor !!!");
        if ((handle) != null) {
    ...
    public Long scheduleRequest(String commandId, Date date, CommandContext ctx) {
        ...
        if ((ctx.getData("priority")) != null) {
            ...
            if (priority < 0) {
                ExecutorImpl.logger.warn("Priority {} is not valid (cannot be less than 0) setting it to 0", priority);
                priority = 0;
            } else if (priority > 9) {
                ExecutorImpl.logger.warn("Priority {} is not valid (cannot be more than 9) setting it to 9", priority);
                priority = 9;
            } 
        } 
        ...
        if (ctx != null) {
            try {
                ...
                ExecutorImpl.logger.warn("Error serializing context data", e);
        ...
        try {
            ...
            if (useJMS) {
                ...
                if (currentTimestamp >= (date.getTime())) {
                    ExecutorImpl.logger.debug("Sending JMS message to trigger job execution for job {}", requestInfo.getId());
                    // send JMS message to trigger processing
                    sendMessage(String.valueOf(requestInfo.getId()), priority);
                } else {
                    ExecutorImpl.logger.debug("JMS message not sent for job {} as the job should not be executed immediately but at {}", requestInfo.getId(), date);
                }
            } 
            ExecutorImpl.logger.debug("Scheduled request for Command: {} - requestId: {} with {} retries", commandId, requestInfo.getId(), requestInfo.getRetries());
    ...
    public void cancelRequest(Long requestId) {
        ExecutorImpl.logger.debug("Before - Cancelling Request with Id: {}", requestId);
        ...
        ExecutorImpl.logger.debug("After - Cancelling Request with Id: {}", requestId);
    }
        ...
        try {
            ...
            if (producer != null) {
                try {
                    ...
                    ExecutorImpl.logger.warn("Error when closing producer", e);
                }
            } 
            if (queueSession != null) {
                try {
                    ...
                    ExecutorImpl.logger.warn("Error when closing queue session", e);
                }
            } 
            if (queueConnection != null) {
                try {
                    ...
                    ExecutorImpl.logger.warn("Error when closing queue connection", e);
                }
            } 
        }
    }
