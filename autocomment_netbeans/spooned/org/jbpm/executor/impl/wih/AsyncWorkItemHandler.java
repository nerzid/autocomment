...
public class AsyncWorkItemHandler implements WorkItemHandler {
    ...
    public AsyncWorkItemHandler(ExecutorService executorService) {
        AsyncWorkItemHandler.this.executorService = executorService;
    }

    public AsyncWorkItemHandler(ExecutorService executorService, String commandClass) {
        this(executorService);
        AsyncWorkItemHandler.this.commandClass = commandClass;
    }
    ...
    public void executeWorkItem(WorkItem workItem, WorkItemManager manager) {
        ...
        AsyncWorkItemHandler.logger.debug("Executing work item {} with built business key {}", workItem, businessKey);
        String cmdClass = ((String) (workItem.getParameter("CommandClass")));
        if (cmdClass == null) {
            cmdClass = AsyncWorkItemHandler.this.commandClass;
        } 
        AsyncWorkItemHandler.logger.debug("Command class for this execution is {}", cmdClass);
        ...
        AsyncWorkItemHandler.logger.trace("Command context {}", ctxCMD);
        Long requestId = executorService.scheduleRequest(cmdClass, scheduleDate, ctxCMD);
        AsyncWorkItemHandler.logger.debug("Request scheduled successfully with id {}", requestId);
        if (autoComplete) {
            AsyncWorkItemHandler.logger.debug("Auto completing work item with id {}", workItem.getId());
    ...
    public void abortWorkItem(WorkItem workItem, WorkItemManager manager) {
        String businessKey = buildBusinessKey(workItem);
        AsyncWorkItemHandler.logger.info("Looking up for not cancelled and not done requests for business key {}", businessKey);
        ...
        if (requests != null) {
            for (RequestInfo request : requests) {
                if ((((request.getStatus()) != (STATUS.CANCELLED)) && ((request.getStatus()) != (STATUS.DONE))) && ((request.getStatus()) != (STATUS.ERROR))) {
                    AsyncWorkItemHandler.logger.info("About to cancel request with id {} and business key {} request state {}", request.getId(), businessKey, request.getStatus());
