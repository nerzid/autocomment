...
public class InMemoryExecutorQueryServiceImpl implements ExecutorQueryService {
    ...
    public void setStoreService(InMemoryExecutorStoreService storeService) {
        InMemoryExecutorQueryServiceImpl.this.storeService = storeService;
    }
    ...
    public List<RequestInfo> getPendingRequests() {
        ...
        return ((List<RequestInfo>) (CollectionUtils.select(requests.values(), new InMemoryExecutorQueryServiceImpl.GetRequestsByStatus(STATUS.QUEUED, STATUS.RETRYING))));
    }
    ...
    public List<RequestInfo> getRequestByBusinessKey(String businessKey, QueryContext queryContext) {
        Map<Long, RequestInfo> requests = storeService.getRequests();
        List<RequestInfo> requestsByBusinessKey = ((List<RequestInfo>) (CollectionUtils.select(requests.values(), new InMemoryExecutorQueryServiceImpl.GetRequestsByKey(businessKey))));
    ...
    public List<RequestInfo> getRequestByCommand(String command, QueryContext queryContext) {
        Map<Long, RequestInfo> requests = storeService.getRequests();
        List<RequestInfo> requestsByCommand = ((List<RequestInfo>) (CollectionUtils.select(requests.values(), new InMemoryExecutorQueryServiceImpl.GetRequestsByCommand(command))));
    ...
    public List<RequestInfo> getQueuedRequests() {
        Map<Long, RequestInfo> requests = storeService.getRequests();
        return ((List<RequestInfo>) (CollectionUtils.select(requests.values(), new InMemoryExecutorQueryServiceImpl.GetRequestsByStatus(STATUS.QUEUED))));
    }

    @Override
    public List<RequestInfo> getCompletedRequests() {
        Map<Long, RequestInfo> requests = storeService.getProcessedRequests();
        return ((List<RequestInfo>) (CollectionUtils.select(requests.values(), new InMemoryExecutorQueryServiceImpl.GetRequestsByStatus(STATUS.DONE))));
    }

    @Override
    public List<RequestInfo> getInErrorRequests() {
        Map<Long, RequestInfo> requests = storeService.getProcessedRequests();
        return ((List<RequestInfo>) (CollectionUtils.select(requests.values(), new InMemoryExecutorQueryServiceImpl.GetRequestsWitError())));
    }

    @Override
    public List<RequestInfo> getCancelledRequests() {
        Map<Long, RequestInfo> requests = storeService.getProcessedRequests();
        return ((List<RequestInfo>) (CollectionUtils.select(requests.values(), new InMemoryExecutorQueryServiceImpl.GetRequestsByStatus(STATUS.CANCELLED))));
    }
    ...
    public List<RequestInfo> getRunningRequests() {
        Map<Long, RequestInfo> requests = storeService.getRequests();
        return ((List<RequestInfo>) (CollectionUtils.select(requests.values(), new InMemoryExecutorQueryServiceImpl.GetRequestsByStatus(STATUS.RUNNING))));
    }
    ...
    public List<RequestInfo> getRequestsByStatus(List<STATUS> statuses) {
        Map<Long, RequestInfo> requests = new HashMap<Long, RequestInfo>(storeService.getRequests());
        requests.putAll(storeService.getProcessedRequests());
        return ((List<RequestInfo>) (CollectionUtils.select(requests.values(), new InMemoryExecutorQueryServiceImpl.GetRequestsByStatus(statuses))));
    }
    ...
    private class GetRequestsByStatus implements Predicate {
        ...
        GetRequestsByStatus(STATUS... status) {
            InMemoryExecutorQueryServiceImpl.GetRequestsByStatus.this.statuses = Arrays.asList(status);
        }

        GetRequestsByStatus(List<STATUS> statuses) {
            InMemoryExecutorQueryServiceImpl.GetRequestsByStatus.this.statuses = statuses;
        }
    ...
    private class GetRequestsByKey implements Predicate {
        ...
        GetRequestsByKey(String key) {
            InMemoryExecutorQueryServiceImpl.GetRequestsByKey.this.key = key;
        }
    ...
    private class GetRequestsByCommand implements Predicate {
        ...
        GetRequestsByCommand(String command) {
            InMemoryExecutorQueryServiceImpl.GetRequestsByCommand.this.command = command;
        }
