    ...
    public void onMessage(Message message) {
        if (message instanceof TextMessage) {
            ...
            try {
                ...
                JmsAvailableJobsExecutor.logger.debug("received Text message with content {}", messageContent);
                ...
                JmsAvailableJobsExecutor.logger.debug("Found following job request for id {} : {}", requestId, request);
                if (request != null) {
                    JmsAvailableJobsExecutor.logger.debug("Request with id {} is in valid state, processing...", requestId);
                    executeGivenJob(request);
                }
            } catch (JMSException e) {
                JmsAvailableJobsExecutor.logger.error("JMS Error when processing job with id {} due to {}", requestId, e.getMessage(), e);
                throw new RuntimeException("Exception when receiving executor job request", e);
            } catch (Throwable e) {
                JmsAvailableJobsExecutor.logger.error("Error when processing job with id {} due to {} will ack JMS message and let exeuctor retry it", requestId, e.getMessage(), e);
            }
        }
    }
}
