    ...
    public ExecutionResults execute(CommandContext ctx) {
        ...
        try {
            ...
            if (executorService instanceof RequeueAware) {
                if (requestId != null) {
                    RequeueRunningJobsCommand.logger.info("Requeue jobs by id {}", requestId);
                    ((RequeueAware) (executorService)).requeueById(requestId);
                }else {
                    RequeueRunningJobsCommand.logger.info("Requeue jobs older than {}", olderThan);
                    ((RequeueAware) (executorService)).requeue(olderThan);
                }
            }else {
                RequeueRunningJobsCommand.logger.info("Executor Service is not capable of jobs requeue");
            }
        } catch (Exception e) {
            RequeueRunningJobsCommand.logger.error("Error while creating CDI bean from jbpm executor", e);
        }
        // info String{"Command executed on executor with data {}"} to Logger{RequeueRunningJobsCommand.logger}
        RequeueRunningJobsCommand.logger.info("Command executed on executor with data {}", ctx.getData());
