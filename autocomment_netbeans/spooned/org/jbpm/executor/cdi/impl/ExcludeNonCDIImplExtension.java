...
public class ExcludeNonCDIImplExtension implements Extension {
    ...
    final ProcessAnnotatedType<X> pat, BeanManager beanManager) {
        ...
        if ((pkg != null) && (excluded.contains(pkg.getName()))) {
            ExcludeNonCDIImplExtension.logger.debug("jBPM Exeutor CDI integration :: excluding package {}", pkg.getName());
            pat.veto();
        }else
            if ((javaClass != null) && (excluded.contains(javaClass.getName()))) {
                ExcludeNonCDIImplExtension.logger.debug(("jBPM Exeutor CDI integration :: excluding class {}" + (javaClass.getName())));
                pat.veto();
            }
