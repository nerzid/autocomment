...
public class HumanTaskServiceProducer {
    ...
    public CommandBasedTaskService produceTaskService() {
        ...
        if ((taskService) == null) {
            ...
            if (mode.equalsIgnoreCase("singleton")) {
                HumanTaskServiceProducer.this.taskService = ((CommandBasedTaskService) (configurator.getTaskService()));
            } else {
    ...
    protected void configureHumanTaskConfigurator(HumanTaskConfigurator configurator) {
        configurator.entityManagerFactory(emf).userGroupCallback(safeGet(userGroupCallback)).userInfo(safeGet(userInfo));
        ...
        try {
            for (TaskLifeCycleEventListener listener : taskListeners) {
                configurator.listener(listener);
                HumanTaskServiceProducer.logger.debug("Registering listener {}", listener);
            }
        } catch (Exception e) {
            HumanTaskServiceProducer.logger.debug("Cannot add listeners to task service due to {}", e.getMessage());
        }
    }
    ...
    protected <T> T safeGet(Instance<T> instance) {
        try {
            ...
            HumanTaskServiceProducer.logger.debug("About to set object {} on task service", object);
            return object;
            ...
            HumanTaskServiceProducer.logger.debug("Cannot get value of of instance {} due to {}", instance, e.getMessage());
        }
        return null;
    }
}
