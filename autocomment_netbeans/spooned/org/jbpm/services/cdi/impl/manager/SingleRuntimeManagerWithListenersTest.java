...
public class SingleRuntimeManagerWithListenersTest extends AbstractKieServicesBaseTest {
    ...
    private void testProcessStartOnManager(RuntimeEngine runtime) {
        ...
        Assert.assertNotNull(pListeners);
        ...
        Assert.assertTrue(listenerCLassNames.contains(DebugProcessEventListener.class.getName()));
        // JPAWorkingMemoryDbLogger one is always added to deal with user tasks
        Assert.assertTrue(listenerCLassNames.contains(JPAWorkingMemoryDbLogger.class.getName()));
        if ((((RuntimeEngineImpl) (runtime)).getManager()) instanceof PerProcessInstanceRuntimeManager) {
            Assert.assertEquals(3, pListeners.size());
        } else {
            Assert.assertEquals(2, pListeners.size());
        }
        ...
        Assert.assertEquals(3, listenerCLassNames.size());
        // JPATaskLifeCycleEventListener was added by custom producer
        Assert.assertTrue(listenerCLassNames.contains(JPATaskLifeCycleEventListener.class.getName()));
        // external one is always added to deal with user tasks
        Assert.assertTrue(listenerCLassNames.contains(ExternalTaskEventListener.class.getName()));
        // BAMTaskEventListener was added by custom producer
        Assert.assertTrue(listenerCLassNames.contains(BAMTaskEventListener.class.getName()));
