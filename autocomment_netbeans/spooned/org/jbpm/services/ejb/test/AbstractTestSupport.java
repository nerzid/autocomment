...
public abstract class AbstractTestSupport {
    ...
    protected static InternalKieModule createKieJar(KieServices ks, ReleaseId releaseId, List<String> resources, Map<String, String> extraResources) {
        ...
        kfs.writePomXML(AbstractTestSupport.getPom(releaseId));
        ...
        if (!(kieBuilder.buildAll().getResults().getMessages().isEmpty())) {
            for (Message message : kieBuilder.buildAll().getResults().getMessages()) {
                AbstractTestSupport.logger.error("Error Message: ({}) {}", message.getPath(), message.getText());
            }
    ...
    protected static void cleanupSingletonSessionId() {
        ...
        if (tempDir.exists()) {
            ...
            for (String file : jbpmSerFiles) {
                AbstractTestSupport.logger.debug("Temp dir to be removed {} file {}", tempDir, file);
