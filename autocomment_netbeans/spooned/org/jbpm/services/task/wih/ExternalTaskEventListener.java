...
public class ExternalTaskEventListener implements TaskLifeCycleEventListener {
    ...
    public void afterTaskCompletedEvent(TaskEvent event) {
        ...
        KieSession session = runtime.getKieSession();
        if (session != null) {
            ExternalTaskEventListener.logger.debug(((">> I've recieved an event for a known session (" + (task.getTaskData().getProcessSessionId())) + ")"));
            processTaskState(task);
        } else {
            ExternalTaskEventListener.logger.error((("EE: I've recieved an event but the session is not known by this handler ( " + (task.getTaskData().getProcessSessionId())) + ")"));
        }
    }
    ...
    public boolean equals(Object obj) {
        if ((ExternalTaskEventListener.this) == obj)
            return true;
    ...
    public int hashCode() {
        final int prime = 31;
        int result = 1;
        result = (prime * result) + (ExternalTaskEventListener.this.getClass().getName().hashCode());
        return result;
    }
}
