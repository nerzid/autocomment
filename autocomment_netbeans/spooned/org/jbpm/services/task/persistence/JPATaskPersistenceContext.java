...
public class JPATaskPersistenceContext implements TaskPersistenceContext {
    ...
    public JPATaskPersistenceContext(EntityManager em, boolean isJTA, boolean locking) {
        JPATaskPersistenceContext.this.em = em;
        this.isJTA = isJTA;
        this.pessimisticLocking = locking;
        JPATaskPersistenceContext.logger.debug("TaskPersistenceManager configured with em {}, isJTA {}, pessimistic locking {}", em, isJTA, locking);
    }

    // Package level getters ------------------------------------------------------------------------------------------------------
    EntityManager getEntityManager() {
        return JPATaskPersistenceContext.this.em;
    }
    ...
    public Task findTask(Long taskId) {
        check();
        Task task = null;
        if (JPATaskPersistenceContext.this.pessimisticLocking) {
            return JPATaskPersistenceContext.this.em.find(TaskImpl.class, taskId, LockModeType.PESSIMISTIC_FORCE_INCREMENT);
        } 
        task = JPATaskPersistenceContext.this.em.find(TaskImpl.class, taskId);
        return task;
    }
    ...
    public Task persistTask(Task task) {
        check();
        JPATaskPersistenceContext.this.em.persist(task);
        if (JPATaskPersistenceContext.this.pessimisticLocking) {
            JPATaskPersistenceContext.this.em.flush();
            return JPATaskPersistenceContext.this.em.find(TaskImpl.class, task.getId(), LockModeType.PESSIMISTIC_FORCE_INCREMENT);
        } 
        return task;
    }
    ...
    public Task updateTask(Task task) {
        check();
        return JPATaskPersistenceContext.this.em.merge(task);
    }
    ...
    public Group findGroup(String groupId) {
        check();
        if (JPATaskPersistenceContext.this.pessimisticLocking) {
            return JPATaskPersistenceContext.this.em.find(GroupImpl.class, groupId, LockModeType.PESSIMISTIC_WRITE);
        } 
        return JPATaskPersistenceContext.this.em.find(GroupImpl.class, groupId);
    }
    ...
    public Group persistGroup(Group group) {
        check();
        try {
            JPATaskPersistenceContext.this.em.persist(group);
            if (JPATaskPersistenceContext.this.pessimisticLocking) {
                JPATaskPersistenceContext.this.em.flush();
                return JPATaskPersistenceContext.this.em.find(GroupImpl.class, group.getId(), LockModeType.PESSIMISTIC_WRITE);
            } 
    ...
    public Group updateGroup(Group group) {
        check();
        return JPATaskPersistenceContext.this.em.merge(group);
    }
    ...
    public User findUser(String userId) {
        check();
        if (JPATaskPersistenceContext.this.pessimisticLocking) {
            return JPATaskPersistenceContext.this.em.find(UserImpl.class, userId, LockModeType.PESSIMISTIC_WRITE);
        } 
        return JPATaskPersistenceContext.this.em.find(UserImpl.class, userId);
    }
    ...
    public User persistUser(User user) {
        check();
        try {
            JPATaskPersistenceContext.this.em.persist(user);
            if (JPATaskPersistenceContext.this.pessimisticLocking) {
                JPATaskPersistenceContext.this.em.flush();
                return JPATaskPersistenceContext.this.em.find(UserImpl.class, user.getId(), LockModeType.PESSIMISTIC_WRITE);
            } 
        } catch (EntityExistsException e) {
    ...
    public User updateUser(User user) {
        check();
        return JPATaskPersistenceContext.this.em.merge(user);
    }
    ...
    public OrganizationalEntity findOrgEntity(String orgEntityId) {
        check();
        if (JPATaskPersistenceContext.this.pessimisticLocking) {
            return JPATaskPersistenceContext.this.em.find(OrganizationalEntityImpl.class, orgEntityId, LockModeType.PESSIMISTIC_WRITE);
        } 
        return JPATaskPersistenceContext.this.em.find(OrganizationalEntityImpl.class, orgEntityId);
    }
    ...
    public OrganizationalEntity persistOrgEntity(OrganizationalEntity orgEntity) {
        ...
        if (!(StringUtils.isEmpty(orgEntity.getId()))) {
            try {
                JPATaskPersistenceContext.this.em.persist(orgEntity);
                if (JPATaskPersistenceContext.this.pessimisticLocking) {
                    JPATaskPersistenceContext.this.em.flush();
                    return JPATaskPersistenceContext.this.em.find(OrganizationalEntityImpl.class, orgEntity.getId(), LockModeType.PESSIMISTIC_WRITE);
                } 
            } catch (EntityExistsException e) {
    ...
    public OrganizationalEntity updateOrgEntity(OrganizationalEntity orgEntity) {
        check();
        return JPATaskPersistenceContext.this.em.merge(orgEntity);
    }
    ...
    public Content findContent(Long contentId) {
        check();
        if (JPATaskPersistenceContext.this.pessimisticLocking) {
            return JPATaskPersistenceContext.this.em.find(ContentImpl.class, contentId, LockModeType.PESSIMISTIC_WRITE);
        } 
        return JPATaskPersistenceContext.this.em.find(ContentImpl.class, contentId);
    }

    @Override
    public Content persistContent(Content content) {
        check();
        JPATaskPersistenceContext.this.em.persist(content);
        if (JPATaskPersistenceContext.this.pessimisticLocking) {
            JPATaskPersistenceContext.this.em.flush();
            return JPATaskPersistenceContext.this.em.find(ContentImpl.class, content.getId(), LockModeType.PESSIMISTIC_WRITE);
        } 
        return content;
    }
    ...
    public Content updateContent(Content content) {
        check();
        return JPATaskPersistenceContext.this.em.merge(content);
    }
    ...
    public Attachment findAttachment(Long attachmentId) {
        check();
        if (JPATaskPersistenceContext.this.pessimisticLocking) {
            return JPATaskPersistenceContext.this.em.find(AttachmentImpl.class, attachmentId, LockModeType.PESSIMISTIC_WRITE);
        } 
        return JPATaskPersistenceContext.this.em.find(AttachmentImpl.class, attachmentId);
    }

    @Override
    public Attachment persistAttachment(Attachment attachment) {
        check();
        JPATaskPersistenceContext.this.em.persist(attachment);
        if (JPATaskPersistenceContext.this.pessimisticLocking) {
            JPATaskPersistenceContext.this.em.flush();
            return JPATaskPersistenceContext.this.em.find(AttachmentImpl.class, attachment.getId(), LockModeType.PESSIMISTIC_WRITE);
        } 
        return attachment;
    }
    ...
    public Attachment updateAttachment(Attachment attachment) {
        check();
        return JPATaskPersistenceContext.this.em.merge(attachment);
    }
    ...
    public Comment findComment(Long commentId) {
        check();
        if (JPATaskPersistenceContext.this.pessimisticLocking) {
            return JPATaskPersistenceContext.this.em.find(CommentImpl.class, commentId, LockModeType.PESSIMISTIC_WRITE);
        } 
        return JPATaskPersistenceContext.this.em.find(CommentImpl.class, commentId);
    }

    @Override
    public Comment persistComment(Comment comment) {
        check();
        JPATaskPersistenceContext.this.em.persist(comment);
        if (JPATaskPersistenceContext.this.pessimisticLocking) {
            JPATaskPersistenceContext.this.em.flush();
            return JPATaskPersistenceContext.this.em.find(CommentImpl.class, comment.getId(), LockModeType.PESSIMISTIC_WRITE);
        } 
        return comment;
    }
    ...
    public Comment updateComment(Comment comment) {
        check();
        return JPATaskPersistenceContext.this.em.merge(comment);
    }
    ...
    public Deadline findDeadline(Long deadlineId) {
        check();
        if (JPATaskPersistenceContext.this.pessimisticLocking) {
            return JPATaskPersistenceContext.this.em.find(DeadlineImpl.class, deadlineId, LockModeType.PESSIMISTIC_WRITE);
        } 
        return JPATaskPersistenceContext.this.em.find(DeadlineImpl.class, deadlineId);
    }

    @Override
    public Deadline persistDeadline(Deadline deadline) {
        check();
        JPATaskPersistenceContext.this.em.persist(deadline);
        if (JPATaskPersistenceContext.this.pessimisticLocking) {
            JPATaskPersistenceContext.this.em.flush();
            return JPATaskPersistenceContext.this.em.find(DeadlineImpl.class, deadline.getId(), LockModeType.PESSIMISTIC_WRITE);
        } 
        return deadline;
    }
    ...
    public Deadline updateDeadline(Deadline deadline) {
        check();
        return JPATaskPersistenceContext.this.em.merge(deadline);
    }
    ...
    public <T> T queryInTransaction(String queryName, Class<T> clazz) {
        check();
        Query query = JPATaskPersistenceContext.this.em.createNamedQuery(queryName);
    ...
    public <T> T queryStringInTransaction(String queryString, Class<T> clazz) {
        check();
        Query query = JPATaskPersistenceContext.this.em.createQuery(queryString);
        return ((T) (query.getResultList()));
    }
    ...
    public <T> T queryStringWithParametersInTransaction(String queryString, Map<String, Object> params, Class<T> clazz) {
        ...
        // logging
        JPATaskPersistenceContext.logger.debug("QUERY:\n {}", queryString);
        if (JPATaskPersistenceContext.logger.isDebugEnabled()) {
            ...
            JPATaskPersistenceContext.logger.debug(paramsStr.toString());
        } 
        Query query = JPATaskPersistenceContext.this.em.createQuery(queryString);
        return queryStringWithParameters(params, false, LockModeType.NONE, clazz, query);
    }
    ...
    public int executeUpdateString(String updateString) {
        check();
        Query query = JPATaskPersistenceContext.this.em.createQuery(updateString);
    ...
    public <T> T persist(T object) {
        check();
        JPATaskPersistenceContext.this.em.persist(object);
        if (JPATaskPersistenceContext.this.pessimisticLocking) {
            JPATaskPersistenceContext.this.em.flush();
            Object primaryKey = getFieldValueWithAnnotation(object, Id.class);
            return ((T) (JPATaskPersistenceContext.this.em.find(object.getClass(), primaryKey, LockModeType.PESSIMISTIC_WRITE)));
        } 
        return object;
    }
    ...
    public <T> T find(Class<T> entityClass, Object primaryKey) {
        check();
        if (JPATaskPersistenceContext.this.pessimisticLocking) {
            return JPATaskPersistenceContext.this.em.find(entityClass, primaryKey, LockModeType.PESSIMISTIC_WRITE);
        } 
        return JPATaskPersistenceContext.this.em.find(entityClass, primaryKey);
    }
    ...
    public <T> T merge(T entity) {
        check();
        return JPATaskPersistenceContext.this.em.merge(entity);
    }
        ...
        if ((params != null) && (!(params.isEmpty()))) {
            for (Map.Entry<String, Object> paramEntry : params.entrySet()) {
                ...
                if (QueryParameterIdentifiers.FIRST_RESULT.equals(name)) {
                    ...
                    continue;
                } else if (QueryParameterIdentifiers.MAX_RESULTS.equals(name)) {
                    if (((Integer) (paramEntry.getValue())) > (-1)) {
                        query.setMaxResults(((Integer) (paramEntry.getValue())));
                    } 
                    continue;
                } else if (QueryParameterIdentifiers.FLUSH_MODE.equals(name)) {
    ...
    public boolean isOpen() {
        if ((JPATaskPersistenceContext.this.em) == null) {
            return false;
        } 
        return JPATaskPersistenceContext.this.em.isOpen();
    }
    ...
    public void joinTransaction() {
        ...
        if (JPATaskPersistenceContext.this.isJTA) {
            JPATaskPersistenceContext.this.em.joinTransaction();
        } 
    }
    ...
    public void close() {
        check();
        JPATaskPersistenceContext.this.em.close();
    }
    ...
    protected Query getQueryByName(String queryName, Map<String, Object> params) {
        String queryStr = JPATaskPersistenceContext.querymanager.getQuery(queryName, params);
        Query query = null;
        if (queryStr != null) {
            query = JPATaskPersistenceContext.this.em.createQuery(queryStr);
        } else {
            query = JPATaskPersistenceContext.this.em.createNamedQuery(queryName);
        }
        return query;
    }
    ...
    private Object getFieldValueWithAnnotation(Object object, Class<? extends Annotation> annotation) {
        try {
            ...
            JPATaskPersistenceContext.logger.error("Unable to find primary key of class {} sure to {}", object.getClass(), e.getMessage());
        }
        return null;
    }
    ...
    public Long findTaskIdByContentId(Long contentId) {
        check();
        CriteriaBuilder builder = JPATaskPersistenceContext.this.em.getCriteriaBuilder();
    ...
    private TaskSummaryQueryCriteriaUtil queryUtil = new TaskSummaryQueryCriteriaUtil(JPATaskPersistenceContext.this);
