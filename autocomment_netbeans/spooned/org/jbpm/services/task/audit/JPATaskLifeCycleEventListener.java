...
public class JPATaskLifeCycleEventListener extends PersistableEventListener implements TaskLifeCycleEventListener {
    ...
    public void afterTaskStartedEvent(TaskEvent event) {
        ...
        try {
            ...
            persistenceContext.persist(new org.jbpm.services.task.audit.impl.model.TaskEventImpl(ti.getId(), TaskEventType.STARTED, ti.getTaskData().getProcessInstanceId(), ti.getTaskData().getWorkItemId(), userId));
            AuditTaskImpl auditTaskImpl = getAuditTask(event, persistenceContext, ti);
            if (auditTaskImpl == null) {
                JPATaskLifeCycleEventListener.logger.warn("Unable find audit task entry for task id {} '{}', skipping audit task update", ti.getId(), ti.getName());
                return ;
            } 
    ...
    public void afterTaskActivatedEvent(TaskEvent event) {
        ...
        try {
            ...
            persistenceContext.persist(new org.jbpm.services.task.audit.impl.model.TaskEventImpl(ti.getId(), TaskEventType.ACTIVATED, ti.getTaskData().getProcessInstanceId(), ti.getTaskData().getWorkItemId(), userId));
            AuditTaskImpl auditTaskImpl = getAuditTask(event, persistenceContext, ti);
            if (auditTaskImpl == null) {
                JPATaskLifeCycleEventListener.logger.warn("Unable find audit task entry for task id {} '{}', skipping audit task update", ti.getId(), ti.getName());
                return ;
    ...
    public void afterTaskClaimedEvent(TaskEvent event) {
        ...
        try {
            ...
            persistenceContext.persist(new org.jbpm.services.task.audit.impl.model.TaskEventImpl(ti.getId(), TaskEventType.CLAIMED, ti.getTaskData().getProcessInstanceId(), ti.getTaskData().getWorkItemId(), userId));
            AuditTaskImpl auditTaskImpl = getAuditTask(event, persistenceContext, ti);
            if (auditTaskImpl == null) {
                JPATaskLifeCycleEventListener.logger.warn("Unable find audit task entry for task id {} '{}', skipping audit task update", ti.getId(), ti.getName());
                return ;
    ...
    public void afterTaskSkippedEvent(TaskEvent event) {
        ...
        try {
            ...
            persistenceContext.persist(new org.jbpm.services.task.audit.impl.model.TaskEventImpl(ti.getId(), TaskEventType.SKIPPED, ti.getTaskData().getProcessInstanceId(), ti.getTaskData().getWorkItemId(), userId));
            AuditTaskImpl auditTaskImpl = getAuditTask(event, persistenceContext, ti);
            if (auditTaskImpl == null) {
                JPATaskLifeCycleEventListener.logger.warn("Unable find audit task entry for task id {} '{}', skipping audit task update", ti.getId(), ti.getName());
                return ;
    ...
    public void afterTaskStoppedEvent(TaskEvent event) {
        ...
        try {
            ...
            persistenceContext.persist(new org.jbpm.services.task.audit.impl.model.TaskEventImpl(ti.getId(), TaskEventType.STOPPED, ti.getTaskData().getProcessInstanceId(), ti.getTaskData().getWorkItemId(), userId));
            AuditTaskImpl auditTaskImpl = getAuditTask(event, persistenceContext, ti);
            if (auditTaskImpl == null) {
                JPATaskLifeCycleEventListener.logger.warn("Unable find audit task entry for task id {} '{}', skipping audit task update", ti.getId(), ti.getName());
                return ;
    ...
    public void afterTaskCompletedEvent(TaskEvent event) {
        ...
        try {
            ...
            persistenceContext.persist(new org.jbpm.services.task.audit.impl.model.TaskEventImpl(ti.getId(), TaskEventType.COMPLETED, ti.getTaskData().getProcessInstanceId(), ti.getTaskData().getWorkItemId(), userId));
            AuditTaskImpl auditTaskImpl = getAuditTask(event, persistenceContext, ti);
            if (auditTaskImpl == null) {
                JPATaskLifeCycleEventListener.logger.warn("Unable find audit task entry for task id {} '{}', skipping audit task update", ti.getId(), ti.getName());
                return ;
    ...
    public void afterTaskFailedEvent(TaskEvent event) {
        ...
        try {
            ...
            persistenceContext.persist(new org.jbpm.services.task.audit.impl.model.TaskEventImpl(ti.getId(), TaskEventType.FAILED, ti.getTaskData().getProcessInstanceId(), ti.getTaskData().getWorkItemId(), userId));
            AuditTaskImpl auditTaskImpl = getAuditTask(event, persistenceContext, ti);
            if (auditTaskImpl == null) {
                JPATaskLifeCycleEventListener.logger.warn("Unable find audit task entry for task id {} '{}', skipping audit task update", ti.getId(), ti.getName());
                return ;
    ...
    public void afterTaskAddedEvent(TaskEvent event) {
        ...
        try {
            ...
            persistenceContext.persist(new org.jbpm.services.task.audit.impl.model.TaskEventImpl(ti.getId(), TaskEventType.ADDED, ti.getTaskData().getProcessInstanceId(), ti.getTaskData().getWorkItemId(), userId));
        } finally {
            cleanup(persistenceContext);
        }
    }
    ...
    public void afterTaskExitedEvent(TaskEvent event) {
        ...
        try {
            ...
            persistenceContext.persist(new org.jbpm.services.task.audit.impl.model.TaskEventImpl(ti.getId(), TaskEventType.EXITED, ti.getTaskData().getProcessInstanceId(), ti.getTaskData().getWorkItemId(), userId));
            AuditTaskImpl auditTaskImpl = getAuditTask(event, persistenceContext, ti);
            if (auditTaskImpl == null) {
                JPATaskLifeCycleEventListener.logger.warn("Unable find audit task entry for task id {} '{}', skipping audit task update", ti.getId(), ti.getName());
                return ;
    ...
    public void afterTaskResumedEvent(TaskEvent event) {
        ...
        try {
            ...
            persistenceContext.persist(new org.jbpm.services.task.audit.impl.model.TaskEventImpl(ti.getId(), TaskEventType.RESUMED, ti.getTaskData().getProcessInstanceId(), ti.getTaskData().getWorkItemId(), userId));
            AuditTaskImpl auditTaskImpl = getAuditTask(event, persistenceContext, ti);
            if (auditTaskImpl == null) {
                JPATaskLifeCycleEventListener.logger.warn("Unable find audit task entry for task id {} '{}', skipping audit task update", ti.getId(), ti.getName());
                return ;
    ...
    public void afterTaskSuspendedEvent(TaskEvent event) {
        ...
        try {
            ...
            persistenceContext.persist(new org.jbpm.services.task.audit.impl.model.TaskEventImpl(ti.getId(), TaskEventType.SUSPENDED, ti.getTaskData().getProcessInstanceId(), ti.getTaskData().getWorkItemId(), userId));
            AuditTaskImpl auditTaskImpl = getAuditTask(event, persistenceContext, ti);
            if (auditTaskImpl == null) {
                JPATaskLifeCycleEventListener.logger.warn("Unable find audit task entry for task id {} '{}', skipping audit task update", ti.getId(), ti.getName());
                return ;
    ...
    public void afterTaskForwardedEvent(TaskEvent event) {
        ...
        try {
            ...
            persistenceContext.persist(new org.jbpm.services.task.audit.impl.model.TaskEventImpl(ti.getId(), TaskEventType.FORWARDED, ti.getTaskData().getProcessInstanceId(), ti.getTaskData().getWorkItemId(), userId));
            AuditTaskImpl auditTaskImpl = getAuditTask(event, persistenceContext, ti);
            if (auditTaskImpl == null) {
                JPATaskLifeCycleEventListener.logger.warn("Unable find audit task entry for task id {} '{}', skipping audit task update", ti.getId(), ti.getName());
                return ;
    ...
    public void afterTaskDelegatedEvent(TaskEvent event) {
        ...
        try {
            ...
            persistenceContext.persist(new org.jbpm.services.task.audit.impl.model.TaskEventImpl(ti.getId(), TaskEventType.DELEGATED, ti.getTaskData().getProcessInstanceId(), ti.getTaskData().getWorkItemId(), userId));
            AuditTaskImpl auditTaskImpl = getAuditTask(event, persistenceContext, ti);
            if (auditTaskImpl == null) {
                JPATaskLifeCycleEventListener.logger.warn("Unable find audit task entry for task id {} '{}', skipping audit task update", ti.getId(), ti.getName());
                return ;
    ...
    public void afterTaskNominatedEvent(TaskEvent event) {
        ...
        try {
            ...
            persistenceContext.persist(new org.jbpm.services.task.audit.impl.model.TaskEventImpl(ti.getId(), TaskEventType.NOMINATED, userId, new Date()));
            AuditTaskImpl auditTaskImpl = getAuditTask(event, persistenceContext, ti);
            if (auditTaskImpl == null) {
                JPATaskLifeCycleEventListener.logger.warn("Unable find audit task entry for task id {} '{}', skipping audit task update", ti.getId(), ti.getName());
                return ;
    ...
    protected AuditTaskImpl getAuditTask(TaskEvent event, TaskPersistenceContext persistenceContext, Task ti) {
        AuditTaskImpl auditTaskImpl = persistenceContext.queryWithParametersInTransaction("getAuditTaskById", true, persistenceContext.addParametersToMap("taskId", ti.getId()), ClassUtil.<AuditTaskImpl>castClass(AuditTaskImpl.class));
    ...
    public void beforeTaskReleasedEvent(TaskEvent event) {
        ...
        try {
            ...
            persistenceContext.persist(new org.jbpm.services.task.audit.impl.model.TaskEventImpl(ti.getId(), TaskEventType.RELEASED, ti.getTaskData().getProcessInstanceId(), ti.getTaskData().getWorkItemId(), userId));
            AuditTaskImpl auditTaskImpl = getAuditTask(event, persistenceContext, ti);
            if (auditTaskImpl == null) {
                JPATaskLifeCycleEventListener.logger.warn("Unable find audit task entry for task id {} '{}', skipping audit task update", ti.getId(), ti.getName());
                return ;
    ...
    public boolean equals(Object obj) {
        if ((JPATaskLifeCycleEventListener.this) == obj)
            return true;
    ...
    public int hashCode() {
        final int prime = 31;
        int result = 1;
        result = (prime * result) + (JPATaskLifeCycleEventListener.this.getClass().getName().hashCode());
        return result;
    }
    ...
    public void afterTaskUpdatedEvent(TaskEvent event) {
        ...
        try {
            ...
            if ((((ti.getDescription()) != null) && (!(ti.getDescription().equals(auditTaskImpl.getDescription())))) || (((ti.getDescription()) == null) && ((auditTaskImpl.getDescription()) != null))) {
                ...
                persistenceContext.persist(new org.jbpm.services.task.audit.impl.model.TaskEventImpl(ti.getId(), TaskEventType.UPDATED, ti.getTaskData().getProcessInstanceId(), ti.getTaskData().getWorkItemId(), userId, message));
            } 
    ...
    public void afterTaskOutputVariableChangedEvent(TaskEvent event, Map<String, Object> variables) {
        ...
        JPATaskLifeCycleEventListener.logger.debug("Deleted {} output variables logs for task id {}", removed, task.getId());
    ...
    protected void indexAndPersistVariables(Task task, Map<String, Object> variables, TaskPersistenceContext persistenceContext, VariableType type) {
        ...
        for (Map.Entry<String, Object> variable : variables.entrySet()) {
            if ((JPATaskLifeCycleEventListener.SKIPPED_TASK_VARIABLES.contains(variable.getKey())) || ((variable.getValue()) == null)) {
                continue;
            } 
