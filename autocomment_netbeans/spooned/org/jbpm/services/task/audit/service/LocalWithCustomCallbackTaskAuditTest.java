...
public class LocalWithCustomCallbackTaskAuditTest extends HumanTaskServicesBaseTest {
    ...
    private LocalWithCustomCallbackTaskAuditTest.NullGroupsUserGroupCallback callback;
    ...
    public void setup() {
        ...
        callback = new LocalWithCustomCallbackTaskAuditTest.NullGroupsUserGroupCallback();
        LocalWithCustomCallbackTaskAuditTest.this.taskService = ((InternalTaskService) (HumanTaskServiceFactory.newTaskServiceConfigurator().entityManagerFactory(emf).listener(new org.jbpm.services.task.audit.JPATaskLifeCycleEventListener(true)).listener(new org.jbpm.services.task.lifecycle.listeners.BAMTaskEventListener(true)).userGroupCallback(callback).getTaskService()));
        LocalWithCustomCallbackTaskAuditTest.this.taskAuditService = TaskAuditServiceFactory.newTaskAuditServiceConfigurator().setTaskService(taskService).getTaskAuditService();
    }
    ...
    public void testGroupTasks() {
        Task task = new org.jbpm.services.task.utils.TaskFluent().setName("This is my task name").addPotentialUser("salaboy").addPotentialGroup("Knights Templer").setAdminUser("Administrator").getTask();
        ...
        List<AuditTask> allGroupAuditTasksByUser = taskAuditService.getAllGroupAuditTasksByUser("salaboy", new org.kie.internal.query.QueryFilter(0, 0));
