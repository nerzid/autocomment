...
public class LocalTaskAuditTest extends TaskAuditBaseTest {
    ...
    public void setup() {
        ...
        LocalTaskAuditTest.this.taskService = ((InternalTaskService) (HumanTaskServiceFactory.newTaskServiceConfigurator().entityManagerFactory(emf).listener(new org.jbpm.services.task.audit.JPATaskLifeCycleEventListener(true)).listener(new org.jbpm.services.task.lifecycle.listeners.BAMTaskEventListener(true)).getTaskService()));
        LocalTaskAuditTest.this.taskAuditService = TaskAuditServiceFactory.newTaskAuditServiceConfigurator().setTaskService(taskService).getTaskAuditService();
    }
