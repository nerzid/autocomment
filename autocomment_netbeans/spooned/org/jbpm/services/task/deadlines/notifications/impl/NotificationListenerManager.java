...
public class NotificationListenerManager {
    ...
    private NotificationListenerManager() {
        for (NotificationListener listener : NotificationListenerManager.listenersLoaded) {
    ...
    public void broadcast(NotificationEvent event, UserInfo userInfo) {
        for (NotificationListener listener : listeners) {
            try {
                NotificationListenerManager.logger.debug("Sending notification {} to {} with params {}", event, listener, userInfo);
                listener.onNotification(event, userInfo);
            } catch (Exception e) {
                NotificationListenerManager.logger.warn("Exception encountered while notifying listener {} with event {} - error {}", listener, event, e.getMessage());
            }
        }
    }

    public static NotificationListenerManager get() {
        return NotificationListenerManager.INSTANCE;
    }
}
