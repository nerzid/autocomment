...
public class LDAPUserInfoImpl extends AbstractLDAPUserGroupInfo implements UserInfo {
    ...
    private static final String[] REQUIRED_PROPERTIES = new String[]{ LDAPUserInfoImpl.USER_CTX , LDAPUserInfoImpl.ROLE_CTX , LDAPUserInfoImpl.USER_FILTER , LDAPUserInfoImpl.ROLE_FILTER };
    ...
    public LDAPUserInfoImpl(boolean activate) {
        super(LDAPUserInfoImpl.REQUIRED_PROPERTIES, LDAPUserInfoImpl.DEFAULT_PROPERTIES_NAME);
    }
    ...
    public LDAPUserInfoImpl(Properties config) {
        super(LDAPUserInfoImpl.REQUIRED_PROPERTIES, config);
    }

    @Override
    public String getDisplayName(OrganizationalEntity entity) {
        return getAttributeValueForEntity(entity, LDAPUserInfoImpl.NAME_ATTR_ID, LDAPUserInfoImpl.DEFAULT_NAME_ATTR_ID);
    }

    @Override
    public Iterator<OrganizationalEntity> getMembersForGroup(Group group) {
        String roleContext = getConfigProperty(LDAPUserInfoImpl.ROLE_CTX);
        String roleFilter = getConfigProperty(LDAPUserInfoImpl.ROLE_MEMBERS_FILTER, getConfigProperty(LDAPUserInfoImpl.ROLE_FILTER));
        String roleAttrId = getConfigProperty(LDAPUserInfoImpl.MEMBER_ATTR_ID, LDAPUserInfoImpl.DEFAULT_MEMBER_ATTR_ID);
        ...
        return memberIds.stream().filter(( memberId) -> memberId != null).map(( memberId) -> ((OrganizationalEntity) (TaskModelProvider.getFactory().newUser(memberId)))).collect(Collectors.toList()).iterator();
    }
    ...
    public String getEmailForEntity(OrganizationalEntity entity) {
        return getAttributeValueForEntity(entity, LDAPUserInfoImpl.EMAIL_ATTR_ID, LDAPUserInfoImpl.DEFAULT_EMAIL_ATTR_ID);
    }

    @Override
    public String getLanguageForEntity(OrganizationalEntity entity) {
        String result = getAttributeValueForEntity(entity, LDAPUserInfoImpl.LANG_ATTR_ID, LDAPUserInfoImpl.DEFAULT_LANG_ATTR_ID);
        return result == null ? LDAPUserInfoImpl.DEFAULT_LOCALE : result;
    }

    private String getAttributeValueForEntity(OrganizationalEntity entity, String attributeName, String defaultValue) {
        String context = getConfigPropertyByEntity(entity, LDAPUserInfoImpl.USER_CTX, LDAPUserInfoImpl.ROLE_CTX);
        String filter = getConfigPropertyByEntity(entity, LDAPUserInfoImpl.USER_FILTER, LDAPUserInfoImpl.ROLE_FILTER);
    ...
    private boolean isEntityIdDn() {
        return Boolean.parseBoolean(getConfigProperty(LDAPUserInfoImpl.IS_ENTITY_ID_DN, LDAPUserInfoImpl.DEFAULT_ENTITY_ID_DN));
    }
}
