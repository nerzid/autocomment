...
public class NominateTaskCommand extends UserGroupCallbackTaskCommand<Void> {
    ...
    public void setPotentialOwners(List<OrganizationalEntity> potentialOwners) {
        NominateTaskCommand.this.potentialOwners = AbstractJaxbTaskObject.convertListFromInterfaceToJaxbImpl(potentialOwners, OrganizationalEntity.class, JaxbOrganizationalEntity.class);
    }

    public Void execute(Context cntxt) {
        TaskContext context = ((TaskContext) (cntxt));
        doCallbackUserOperation(userId, context);
        List<OrganizationalEntity> realPotOwners = JaxbOrganizationalEntity.convertListFromJaxbImplToInterface(potentialOwners);
