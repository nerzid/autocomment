...
public class ExecuteReminderCommand extends TaskCommand<Void> {
    ...
    public ExecuteReminderCommand(long taskId, String fromUser) {
        this.taskId = taskId;
        ExecuteReminderCommand.this.fromUser = fromUser;
    }
    ...
    public Void execute(Context context) {
        ...
        try {
            ...
            if ((resultList == null) || ((resultList.size()) == 0)) {
                ...
                if (taskData != null) {
                    ...
                    if ((DeadlineType.START.isValidStatus(taskData.getStatus())) || (DeadlineType.END.isValidStatus(taskData.getStatus()))) {
                        ...
                        ExecuteReminderCommand.logger.debug("Sending an Email");
            ...
            ExecuteReminderCommand.logger.error("Error when executing deadlines", e);
        }
        return null;
    }
    ...
    private List<DeadlineSummary> getAlldeadlines(TaskPersistenceContext persistenceContext, TaskData taskData) {
        ...
        if (((Status.InProgress) == (taskData.getStatus())) || ((Status.Suspended) == (taskData.getStatus()))) {
            resultList = persistenceContext.queryWithParametersInTransaction("UnescalatedEndDeadlinesByTaskIdForReminder", persistenceContext.addParametersToMap("taskId", taskId), ClassUtil.<List<DeadlineSummary>>castClass(List.class));
        } else {
            // get no-started notification
            resultList = persistenceContext.queryWithParametersInTransaction("UnescalatedStartDeadlinesByTaskIdForReminder", persistenceContext.addParametersToMap("taskId", taskId), ClassUtil.<List<DeadlineSummary>>castClass(List.class));
        }
        return resultList;
    }
    ...
    private String buildDefafultEmailBody(TaskData taskData, Task task) {
        ...
        String body = ((String) (TemplateRuntime.eval(ExecuteReminderCommand.defaultEmailBody, vars)));
        return body;
    }
    ...
    private String buildDefafultEmailSubject(TaskData taskData, Task task) {
        ...
        vars.put("taskName", task.getName());
        vars.put("processId", taskData.getProcessId());
        String body = ((String) (TemplateRuntime.eval(ExecuteReminderCommand.defaultEmailSubject, vars)));
        return body;
    }
}
