...
public class BAMTaskEventListener extends PersistableEventListener {
    ...
    public void afterTaskStartedEvent(TaskEvent event) {
        updateTask(event, new BAMTaskEventListener.BAMTaskWorker() {
            @Override
     ...
     */
    protected BAMTaskSummaryImpl updateTask(TaskEvent event, BAMTaskEventListener.BAMTaskWorker worker) {
     ...
     * @return The created or updated bam task summary instance.
     */
    protected BAMTaskSummaryImpl createTask(TaskEvent event, Status newStatus, BAMTaskEventListener.BAMTaskWorker worker) {
        ...
        try {
            if (ti == null) {
                BAMTaskEventListener.logger.error("The task instance does not exist.");
                return result;
            } 
    ...
    protected BAMTaskSummaryImpl updateTask(TaskEvent event, Status newStatus, BAMTaskEventListener.BAMTaskWorker worker) {
        BAMTaskSummaryImpl result = null;
        Task ti = event.getTask();
        TaskPersistenceContext persistenceContext = getPersistenceContext(((TaskContext) (event.getTaskContext())).getPersistenceContext());
        try {
            ...
            if (result == null) {
                BAMTaskEventListener.logger.warn("Unable find bam task entry for task id {} '{}', skipping bam task update", ti.getId(), ti.getName());
                return null;
            } 
    ...
    public boolean equals(Object obj) {
        if ((BAMTaskEventListener.this) == obj)
            return true;
    ...
    public int hashCode() {
        final int prime = 31;
        int result = 1;
        result = (prime * result) + (BAMTaskEventListener.this.getClass().getName().hashCode());
        return result;
    }
