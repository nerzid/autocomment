...
public abstract class EmailDeadlinesBaseTest extends HumanTaskServicesBaseTest {
    ...
    protected Wiser getWiser() {
        return EmailDeadlinesBaseTest.this.wiser;
    }
    ...
    public void testDelayedEmailNotificationOnDeadline() throws Exception {
        ...
        Assert.assertEquals(0, getWiser().getMessages().size());
        countDownListener.waitTillCompleted();
        for (WiserMessage msg : getWiser().getMessages()) {
            EmailDeadlinesBaseTest.logger.info(msg.getEnvelopeReceiver());
        }
        // 1 email with two recipients should now exist
        Assert.assertEquals(2, getWiser().getMessages().size());
        ...
        Assert.assertEquals(myBody, msg.getContent());
        Assert.assertEquals(mySubject, msg.getSubject());
        Assert.assertEquals("from@domain.com", ((InternetAddress) (msg.getFrom()[0])).getAddress());
        Assert.assertEquals("replyTo@domain.com", ((InternetAddress) (msg.getReplyTo()[0])).getAddress());
        final Address[] recipients = msg.getRecipients(RecipientType.TO);
        Assert.assertNotNull(recipients);
        Assert.assertTrue((recipients == 2));
    ...
    public void testDelayedEmailNotificationOnDeadlineContentSingleObject() throws Exception {
        ...
        Assert.assertTrue(list.contains("darth@domain.com"));
        MimeMessage msg = ((WiserMessage) (getWiser().getMessages().get(0))).getMimeMessage();
        Assert.assertEquals("'singleobject'", msg.getContent());
        Assert.assertEquals("'singleobject'", msg.getSubject());
        Assert.assertEquals("from@domain.com", ((InternetAddress) (msg.getFrom()[0])).getAddress());
        Assert.assertEquals("replyTo@domain.com", ((InternetAddress) (msg.getReplyTo()[0])).getAddress());
        final Address[] recipients = msg.getRecipients(RecipientType.TO);
        Assert.assertNotNull(recipients);
