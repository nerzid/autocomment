...
public class LdapSearcher {
        ...
        try {
            ...
            if (ldapResult != null) {
                try {
                    ...
                    LdapSearcher.log.error("Failed to close LDAP results enumeration", ex);
                }
            } 
            if (ldapContext != null) {
                try {
                    ...
                    LdapSearcher.log.error("Failed to close LDAP context", ex);
                }
            } 
        }
        return LdapSearcher.this;
    }
    ...
    public List<String> getAttributeResults(String attributeId) {
        return searchResults.stream().map(( searchResult) -> getAttribute(searchResult, attributeId)).collect(Collectors.toList());
    }
    ...
    private String getAttribute(SearchResult searchResult, String attributeId) {
        ...
        if (entry == null) {
            LdapSearcher.log.warn("The attribute with ID '{}' has not been found.", attributeId);
            return null;
        } 
        try {
            return entry.get().toString();
        } catch (NamingException ex) {
            LdapSearcher.log.error("Failed to get attribute value", ex);
            return null;
        }
    }

    private LdapContext buildLdapContext() throws NamingException {
        config.putIfAbsent(Context.INITIAL_CONTEXT_FACTORY, LdapSearcher.DEFAULT_INITIAL_CONTEXT_FACTORY);
        config.putIfAbsent(Context.SECURITY_AUTHENTICATION, LdapSearcher.DEFAULT_SECURITY_AUTHENTICATION);
        ...
        if (LdapSearcher.log.isDebugEnabled()) {
            LdapSearcher.log.debug("Using following InitialLdapContext properties:");
            LdapSearcher.log.debug("Initial Context Factory: {}", config.getProperty(Context.INITIAL_CONTEXT_FACTORY));
            LdapSearcher.log.debug("Authentication Type: {}", config.getProperty(Context.SECURITY_AUTHENTICATION));
            LdapSearcher.log.debug("Protocol: {}", config.getProperty(Context.SECURITY_PROTOCOL));
            LdapSearcher.log.debug("Provider URL: {}", config.getProperty(Context.PROVIDER_URL));
            LdapSearcher.log.debug("User DN: {}", config.getProperty(Context.SECURITY_PRINCIPAL));
            LdapSearcher.log.debug("Password: {}", config.getProperty(Context.SECURITY_CREDENTIALS));
        } 
    ...
    private SearchControls createSearchControls() {
        ...
        String searchScope = config.getProperty(LdapSearcher.SEARCH_SCOPE);
        if (searchScope != null) {
    ...
    private int parseSearchScope(String searchScope) {
        LdapSearcher.log.debug("Search scope: {}", searchScope);
        try {
            return LdapSearcher.SearchScope.valueOf(searchScope).ordinal();
        } catch (IllegalArgumentException ex) {
            return LdapSearcher.SearchScope.ONELEVEL_SCOPE.ordinal();
        }
    }
