...
public class DistincVsJoinPerformanceTest extends HumanTaskServicesBaseTest {
    ...
    public void setup() {
        ...
        DistincVsJoinPerformanceTest.this.taskService = ((InternalTaskService) (HumanTaskServiceFactory.newTaskServiceConfigurator().entityManagerFactory(emf).getTaskService()));
    }
    ...
    public void performanceTest() {
        ...
        joinTotal /= 20;
        selectTotal /= 20;
        Assert.assertTrue((((("Join [" + joinTotal) + "ms] took longer than Select [") + selectTotal) + "ms]!"), (joinTotal < selectTotal));
    }
    ...
    private String[] createOriginalAndExpectedKeys(Attribute embeddedAttr, PluralAttribute listAttr) {
        String originalKey = ((((embeddedAttr.getDeclaringType().getJavaType().getName()) + ".") + (embeddedAttr.getName())) + ".") + (listAttr.getName());
        String copyKey = ((listAttr.getDeclaringType().getJavaType().getName()) + ".") + (listAttr.getName());
        String[] keys = new String[]{ originalKey , copyKey };
        return keys;
    }
    ...
    private long timeQueryExecution(EntityManager em, CriteriaQuery query, Query realQuery, int total) {
        ...
        Assert.assertEquals("query results", total, results.size());
    ...
    private TaskImpl addTask(long workItemId, long procInstId, String busAdmin, String potOwner, String name, String deploymentId) {
        ...
        str += ((((((((((("peopleAssignments = (with ( new PeopleAssignments() ) { " + "taskStakeholders = [new User('") + (DistincVsJoinPerformanceTest.stakeHolder)) + "')],") + "businessAdministrators = [new User('") + busAdmin) + "')],") + "potentialOwners = [new ") + potOwnerType) + "('") + potOwner) + "')]") + " }),";
        str += ("name =  '" + name) + "' })";
