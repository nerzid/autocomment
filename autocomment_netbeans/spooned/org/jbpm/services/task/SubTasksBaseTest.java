...
public abstract class SubTasksBaseTest extends HumanTaskServicesBaseTest {
    ...
    public void noActionStrategy() throws Exception {
        ...
        taskService.addTask(parentTask, new HashMap<String, java.lang.Object>());
        ...
        Assert.assertEquals(parentTask1.getTaskData().getStatus(), Status.Reserved);
        ...
        taskService.addTask(child1Task, new HashMap<String, java.lang.Object>());
        long child1TaskId = child1Task.getId();
        // Test if the task is succesfully created
        Assert.assertEquals(1, taskService.getPendingSubTasksByParent(taskParentId));
        Task childTask1 = taskService.getTaskById(child1TaskId);
        Assert.assertEquals(taskParentId, childTask1.getTaskData().getParentId());
        ...
        Assert.assertEquals(Status.Completed, childTask1.getTaskData().getStatus());
        parentTask1 = taskService.getTaskById(taskParentId);
        Assert.assertEquals(Status.InProgress, parentTask1.getTaskData().getStatus());
        Assert.assertEquals(0, taskService.getPendingSubTasksByParent(taskParentId));
        taskService.complete(taskParentId, "salaboy", null);
        parentTask1 = taskService.getTaskById(taskParentId);
        Assert.assertEquals(Status.Completed, parentTask1.getTaskData().getStatus());
    }
    ...
    public void onSubtaskCompletionAutoCompleteParentStrategyWithLotsOfTasks() throws Exception {
        ...
        try {
            ...
            Query query = em.createNativeQuery((((("select SEQUENCE_NAME from INFORMATION_SCHEMA.COLUMNS " + "where TABLE_NAME = '") + (tableName.toUpperCase())) + "' ") + " and SEQUENCE_NAME IS NOT null"));
    ...
    public void onParentAbortCompleteAllSubTasksStrategy() throws Exception {
        ...
        parentTask1 = taskService.getTaskById(taskParentId);
        Assert.assertEquals(0, taskService.getPendingSubTasksByParent(taskParentId));
        Assert.assertEquals(Status.Obsolete, parentTask1.getTaskData().getStatus());
        childTask1 = taskService.getTaskById(child1TaskId);
        Assert.assertEquals(Status.Obsolete, childTask1.getTaskData().getStatus());
    }
}
