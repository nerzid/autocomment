...
public final class ChecklistItemFactory {
    public static ChecklistItem createChecklistItem(Task task) {
        return new DefaultChecklistItem(ChecklistItemFactory.getText(task.getNames()), ChecklistItemFactory.getStatus(task.getTaskData().getStatus()), task.getId(), "HumanTaskNode", ChecklistItemFactory.getActors(task), task.getPriority(), task.getTaskData().getProcessId(), task.getTaskData().getProcessInstanceId(), ChecklistItemFactory.getText(task.getDescriptions()));
    }
    ...
    private static Status getStatus(Status status) {
        switch (status) {
            case Completed :
