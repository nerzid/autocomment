...
public class JPAProcessInstanceManager implements ProcessInstanceManager {
    ...
    private transient volatile Map<Long, ProcessInstance> processInstances = new ConcurrentHashMap<Long, ProcessInstance>();

    public void setKnowledgeRuntime(InternalKnowledgeRuntime kruntime) {
        JPAProcessInstanceManager.this.kruntime = kruntime;
    }

    public void addProcessInstance(ProcessInstance processInstance, CorrelationKey correlationKey) {
        ProcessInstanceInfo processInstanceInfo = new ProcessInstanceInfo(processInstance, JPAProcessInstanceManager.this.kruntime.getEnvironment());
        ProcessPersistenceContext context = ((ProcessPersistenceContextManager) (JPAProcessInstanceManager.this.kruntime.getEnvironment().get(EnvironmentName.PERSISTENCE_CONTEXT_MANAGER))).getProcessPersistenceContext();
    ...
    public void internalAddProcessInstance(ProcessInstance processInstance) {
        if ((((ConcurrentHashMap<Long, ProcessInstance>) (processInstances)).putIfAbsent(processInstance.getId(), processInstance)) != null) {
    ...
    public ProcessInstance getProcessInstance(long id, boolean readOnly) {
        ...
        TransactionManager txm = ((TransactionManager) (JPAProcessInstanceManager.this.kruntime.getEnvironment().get(EnvironmentName.TRANSACTION_MANAGER)));
        org.jbpm.process.instance.ProcessInstance processInstance = null;
        processInstance = ((org.jbpm.process.instance.ProcessInstance) (JPAProcessInstanceManager.this.processInstances.get(id)));
        if (processInstance != null) {
            if ((((WorkflowProcessInstanceImpl) (processInstance)).isPersisted()) && (!readOnly)) {
                ProcessPersistenceContextManager ppcm = ((ProcessPersistenceContextManager) (JPAProcessInstanceManager.this.kruntime.getEnvironment().get(EnvironmentName.PERSISTENCE_CONTEXT_MANAGER)));
        ...
        processInstance = ((org.jbpm.process.instance.ProcessInstance) (processInstanceInfo.getProcessInstance(kruntime, JPAProcessInstanceManager.this.kruntime.getEnvironment())));
        if (!readOnly) {
            processInstanceInfo.updateLastReadDate();
            TransactionManagerHelper.addToUpdatableSet(txm, processInstanceInfo);
        } 
