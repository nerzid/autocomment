...
public class WorkItemPersistenceTest extends AbstractBaseTest {
    ...
    public void testCancelNonRegisteredWorkItemHandler() {
        ...
        parameters.put("Person", new org.jbpm.persistence.session.objects.Person("John Doe"));
        ProcessInstance processInstance = ksession.startProcess("org.drools.actions", parameters);
        long processInstanceId = processInstance.getId();
        Assert.Assert.assertEquals(ProcessInstance.STATE_ACTIVE, processInstance.getState());
        ksession.getWorkItemManager().registerWorkItemHandler(workName, null);
        try {
            ksession.abortProcessInstance(processInstanceId);
            Assert.Assert.fail((("should fail if WorkItemHandler for" + workName) + "is not registered"));
        } catch (WorkItemHandlerNotFoundException wihnfe) {
        }
        Assert.Assert.assertEquals(ProcessInstance.STATE_ABORTED, processInstance.getState());
    }
    ...
    public void testHumanTask() {
        List<ProcessInstanceInfo> procInstInfoList = WorkItemPersistenceTest.retrieveProcessInstanceInfo(emf);
        ...
        switch (state) {
            case ProcessInstance.STATE_ABORTED :
                WorkItemPersistenceTest.logger.debug("STATE_ABORTED");
                break;
            case ProcessInstance.STATE_ACTIVE :
                WorkItemPersistenceTest.logger.debug("STATE_ACTIVE");
                break;
            case ProcessInstance.STATE_COMPLETED :
                WorkItemPersistenceTest.logger.debug("STATE_COMPLETED");
                break;
            case ProcessInstance.STATE_PENDING :
                WorkItemPersistenceTest.logger.debug("STATE_PENDING");
                break;
            case ProcessInstance.STATE_SUSPENDED :
                WorkItemPersistenceTest.logger.debug("STATE_SUSPENDED");
                break;
            default :
                WorkItemPersistenceTest.logger.debug("Unknown state: {}", state);
        }
        procInstInfoList = WorkItemPersistenceTest.retrieveProcessInstanceInfo(emf);
    ...
    public static ArrayList<ProcessInstanceInfo> retrieveProcessInstanceInfo(EntityManagerFactory emf) {
        ...
        for (Object resultObject : mdList) {
            ...
            WorkItemPersistenceTest.logger.trace("> {}", procInstInfo);
        }
        txm.commit(txOwner);
