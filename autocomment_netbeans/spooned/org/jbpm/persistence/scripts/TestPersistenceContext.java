...
public final class TestPersistenceContext {
    ...
    public void init(final PersistenceUnit persistenceUnit) {
        context = PersistenceUtil.setupWithPoolingDataSource(persistenceUnit.getName(), persistenceUnit.getDataSourceName());
        ...
        Object tm = TestPersistenceContext.this.environment.get(EnvironmentName.TRANSACTION_MANAGER);
        transactionManager = new JtaTransactionManager(environment.get(EnvironmentName.TRANSACTION), environment.get(EnvironmentName.TRANSACTION_SYNCHRONIZATION_REGISTRY), tm);
    }
        ...
        try {
            for (File script : sqlScripts) {
                if ((type == null) || (script.getName().startsWith(type))) {
                    TestPersistenceContext.logger.debug("Executing script {}", script.getName());
                    ...
                    for (String command : scriptCommands) {
                        TestPersistenceContext.logger.debug(command);
    ...
    public void createSomeTask() {
        ...
        peopleAssignments.setPotentialOwners(new ArrayList<org.kie.api.task.model.OrganizationalEntity>());
        peopleAssignments.setBusinessAdministrators(new ArrayList<org.kie.api.task.model.OrganizationalEntity>());
        peopleAssignments.setExcludedOwners(new ArrayList<org.kie.api.task.model.OrganizationalEntity>());
        peopleAssignments.setRecipients(new ArrayList<org.kie.api.task.model.OrganizationalEntity>());
        peopleAssignments.setTaskStakeholders(new ArrayList<org.kie.api.task.model.OrganizationalEntity>());
    ...
    public StatefulKnowledgeSession loadPersistedSession(final Long sessionId, final String processIdForKieBase) {
        testIsInitialized();
        return JPAKnowledgeService.loadStatefulKnowledgeSession(sessionId, createKieBase(processIdForKieBase), null, environment);
    }
     ...
     */
    public void persistOldProcessAndSession(final Integer sessionId, final String processId, final Long processInstanceId) {
        testIsInitialized();
        final boolean txOwner = transactionManager.begin();
    ...
    private int getStoredEntitiesCount(final String entityClassName) {
        ...
        final EntityManager entityManager = entityManagerFactory.createEntityManager();
        try {
            final List entitiesList = entityManager.createQuery((("SELECT p FROM " + entityClassName) + " p")).getResultList();
     ...
     */
    private ProcessInstanceInfo getOldProcessInstanceInfo(final String processId, final Long processInstanceId) throws ParseException {
