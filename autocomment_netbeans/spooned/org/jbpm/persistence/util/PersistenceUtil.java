...
public class PersistenceUtil {
    ...
    private static PersistenceUtil.TestH2Server h2Server = new PersistenceUtil.TestH2Server();
    ...
    public static HashMap<String, Object> setupWithPoolingDataSource(final String persistenceUnitName, String dataSourceName) {
        ...
        context.put(EnvironmentName.ENTITY_MANAGER_FACTORY, emf);
        return context;
    }
    ...
    public static void cleanUp(Map<String, Object> context) {
        if (context != null) {
            ...
            Object emfObject = context.remove(EnvironmentName.ENTITY_MANAGER_FACTORY);
            if (emfObject != null) {
                try {
            ...
            Object ds1Object = context.remove(PersistenceUtil.DATASOURCE);
            if (ds1Object != null) {
                try {
    ...
    public static PoolingDataSource setupPoolingDataSource(Properties dsProps, String datasourceName, boolean startServer) {
        ...
        if ((driverClass.startsWith("org.h2")) || (driverClass.startsWith("org.hsqldb"))) {
            if (startServer) {
                PersistenceUtil.h2Server.start();
            } 
        } 
    ...
    private static Properties getDefaultProperties() {
        if ((PersistenceUtil.defaultProperties) == null) {
            ...
            String[] defaultPropArr = new String[]{ "" , "" , "" , "jdbc:h2:tcp://localhost/target/jbpm-test" , "sa" , "" , "org.h2.Driver" , "bitronix.tm.resource.jdbc.lrc.LrcXADataSource" , "16" , "true" };
            Assert.assertTrue("Unequal number of keys for default properties", ((keyArr.length) == (defaultPropArr.length)));
            PersistenceUtil.defaultProperties = new Properties();
            for (int i = 0; i < (keyArr.length); ++i) {
                PersistenceUtil.defaultProperties.put(keyArr[i], defaultPropArr[i]);
            }
        } 
        return PersistenceUtil.defaultProperties;
    }
    ...
    public static Properties getDatasourceProperties() {
        String propertiesNotFoundMessage = ("Unable to load datasource properties [" + (PersistenceUtil.DATASOURCE_PROPERTIES)) + "]";
        boolean propertiesNotFound = false;
        // Central place to set additional H2 properties
        System.setProperty("h2.lobInDatabase", "true");
        InputStream propsInputStream = PersistenceUtil.class.getResourceAsStream(PersistenceUtil.DATASOURCE_PROPERTIES);
        Assert.assertNotNull(propertiesNotFoundMessage, propsInputStream);
        ...
        if (propsInputStream != null) {
            try {
                ...
                PersistenceUtil.logger.warn("Unable to find properties, using default H2 properties: {}", ioe.getMessage());
                PersistenceUtil.logger.warn("Stacktrace:", ioe);
            }
        } else {
            propertiesNotFound = true;
        }
    ...
    public static Object getValueOfField(String fieldname, Object source) {
        ...
        try {
            ...
            Assert.fail(((((("Unable to retrieve " + fieldname) + " field from ") + sourceClassName) + ": ") + (e.getCause())));
        } catch (NoSuchFieldException e) {
            Assert.fail(((((("Unable to retrieve " + fieldname) + " field from ") + sourceClassName) + ": ") + (e.getCause())));
        }
        Assert.assertNotNull((("." + fieldname) + " field is null!?!"), field);
        Object fieldValue = null;
        try {
            fieldValue = field.get(source);
        } catch (IllegalArgumentException e) {
            Assert.fail(((((("Unable to retrieve value of " + fieldname) + " from ") + sourceClassName) + ": ") + (e.getCause())));
    ...
    public static Environment createEnvironment(Map<String, Object> context) {
        ...
        env.set(EnvironmentName.GLOBALS, new org.drools.core.base.MapGlobalResolver());
        return env;
    }
