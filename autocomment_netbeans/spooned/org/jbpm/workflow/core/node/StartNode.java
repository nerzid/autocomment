...
public class StartNode extends ExtendedNodeImpl implements Mappable {
    ...
    public void setTriggers(List<Trigger> triggers) {
        StartNode.this.triggers = triggers;
    }

    public String[] getActionTypes() {
        return StartNode.EVENT_TYPES;
    }

    public void validateAddIncomingConnection(final String type, final Connection connection) {
        throw new UnsupportedOperationException((((("A start node [" + (StartNode.this.getMetaData("UniqueId"))) + ", ") + (StartNode.this.getName())) + "] may not have an incoming connection!"));
    }
    ...
    public void validateAddOutgoingConnection(final String type, final Connection connection) {
        ...
        if (!(org.jbpm.workflow.core.Node.CONNECTION_DEFAULT_TYPE.equals(type))) {
            throw new IllegalArgumentException((((("A start node [" + (StartNode.this.getMetaData("UniqueId"))) + ", ") + (StartNode.this.getName())) + "] only accepts default outgoing connection type!"));
        } 
        if ((getTo()) != null) {
            throw new IllegalArgumentException((((("A start node [" + (StartNode.this.getMetaData("UniqueId"))) + ", ") + (StartNode.this.getName())) + "] cannot have more than one outgoing connection!"));
        } 
    }
    ...
    public void setInterrupting(boolean isInterrupting) {
        StartNode.this.isInterrupting = isInterrupting;
    }

    @Override
    public void addInMapping(String parameterName, String variableName) {
        throw new IllegalArgumentException((((("A start event [" + (StartNode.this.getMetaData("UniqueId"))) + ", ") + (StartNode.this.getName())) + "] does not support input mappings"));
    }
    ...
    public void setOutMappings(Map<String, String> outMapping) {
        StartNode.this.outMapping = new LinkedList<DataAssociation>();
    ...
    public Map<String, String> getOutMappings() {
        ...
        for (DataAssociation assoc : outMapping) {
            if ((((assoc.getSources().size()) == 1) && (((assoc.getAssignments()) == null) || ((assoc.getAssignments().size()) == 0))) && ((assoc.getTransformation()) == null)) {
    ...
    public void setTimer(Timer timer) {
        StartNode.this.timer = timer;
    }

    public void setEventTransformer(EventTransformer transformer) {
        StartNode.this.transformer = transformer;
    }
