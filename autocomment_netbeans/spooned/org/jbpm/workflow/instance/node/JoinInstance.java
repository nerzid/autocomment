...
public class JoinInstance extends NodeInstanceImpl {
    ...
    public void internalTrigger(final NodeInstance from, String type) {
        ...
        switch (join.getType()) {
            case Join.TYPE_XOR :
                ...
                break;
            case Join.TYPE_AND :
                Integer count = ((Integer) (JoinInstance.this.triggers.get(from.getNodeId())));
                if (count == null) {
                    JoinInstance.this.triggers.put(from.getNodeId(), 1);
                } else {
                    JoinInstance.this.triggers.put(from.getNodeId(), ((count.intValue()) + 1));
                }
            ...
            case Join.TYPE_N_OF_M :
                count = ((Integer) (JoinInstance.this.triggers.get(from.getNodeId())));
                if (count == null) {
                    JoinInstance.this.triggers.put(from.getNodeId(), 1);
                } else {
                    JoinInstance.this.triggers.put(from.getNodeId(), ((count.intValue()) + 1));
                ...
                for (final Connection connection : getJoin().getDefaultIncomingConnections()) {
                    if ((JoinInstance.this.triggers.get(connection.getFrom().getId())) != null) {
                        counter++;
                    } 
                }
                String n = join.getN();
    ...
    private void decreaseAllTriggers() {
        ...
        for (final Connection connection : getJoin().getDefaultIncomingConnections()) {
            final Integer count = ((Integer) (JoinInstance.this.triggers.get(connection.getFrom().getId())));
            if ((count.intValue()) == 1) {
                JoinInstance.this.triggers.remove(connection.getFrom().getId());
            } else {
                JoinInstance.this.triggers.put(connection.getFrom().getId(), ((count.intValue()) - 1));
            }
        }
    }
    ...
    public void internalSetTriggers(Map<Long, Integer> triggers) {
        JoinInstance.this.triggers = triggers;
    }
}
