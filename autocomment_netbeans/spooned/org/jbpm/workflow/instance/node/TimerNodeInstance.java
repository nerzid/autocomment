...
public class TimerNodeInstance extends StateBasedNodeInstance implements EventListener {
    ...
    public void internalSetTimerId(long timerId) {
        TimerNodeInstance.this.timerId = timerId;
    }
        ...
        while (matcher.find()) {
            ...
            if ((replacements.get(paramName)) == null) {
                ...
                if (variableScopeInstance != null) {
                    ...
                    try {
                        Object variableValue = MVELSafeHelper.getEvaluator().eval(paramName, new org.jbpm.workflow.instance.impl.NodeInstanceResolverFactory(TimerNodeInstance.this));
                        String variableValueString = variableValue == null ? "" : variableValue.toString();
                        replacements.put(paramName, variableValueString);
                    } catch (Throwable t) {
                        TimerNodeInstance.logger.error("Could not find variable scope for variable {}", paramName);
                        TimerNodeInstance.logger.error("when trying to replace variable in processId for sub process {}", getNodeName());
                        TimerNodeInstance.logger.error("Continuing without setting process id.");
                    }
                }
            } 
        }
    ...
    public void cancel() {
        ((InternalProcessRuntime) (getProcessInstance().getKnowledgeRuntime().getProcessRuntime())).getTimerManager().cancelTimer(timerId);
        super.cancel();
    }
    ...
    public void removeEventListeners() {
        ...
        ((WorkflowProcessInstance) (getProcessInstance())).removeEventListener("timerTriggered", TimerNodeInstance.this, false);
    }
