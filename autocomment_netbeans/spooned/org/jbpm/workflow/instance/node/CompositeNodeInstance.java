...
import CompositeNode.CompositeNodeEnd;
import CompositeNode.CompositeNodeStart;
...
import CompositeNode.NodeAndType;
...
import org.jbpm.workflow.instance.org.jbpm.workflow.instance.NodeInstanceContainer;

/**
 * Runtime counterpart of a composite node.
 */
public class CompositeNodeInstance extends StateBasedNodeInstance implements EventBasedNodeInstanceInterface , EventNodeInstanceInterface , NodeInstanceContainer {
    ...
    // set during NodeInstance creation (*NodeFactory)
    ...
    public void setProcessInstance(WorkflowProcessInstance processInstance) {
        ...
        CompositeNodeInstance.this.singleNodeInstanceCounter = ((WorkflowProcessInstanceImpl) (processInstance)).internalGetNodeInstanceCounter();
        ...
        for (Node node : getCompositeNode().getNodes()) {
            if (node instanceof EventNode) {
                ...
                for (String type : events) {
                    getProcessInstance().addEventListener(type, DummyEventListener.EMPTY_EVENT_LISTENER, true);
                }
            } 
        }
    }
    ...
    public void internalTrigger(final NodeInstance from, String type) {
        ...
        if (nodeAndType != null) {
            ...
            for (Iterator<Connection> iterator = connections.iterator(); iterator.hasNext();) {
                ...
                if (((connection.getFrom()) instanceof CompositeNode) && ((from == null) || ((((CompositeNode.CompositeNodeStart) (connection.getFrom())).getInNode().getId()) == (from.getNodeId())))) {
    ...
    protected void internalTriggerOnlyParent(final NodeInstance from, String type) {
        super.internalTrigger(from, type);
    }
    ...
    public void addNodeInstance(final NodeInstance nodeInstance) {
        ...
        CompositeNodeInstance.this.nodeInstances.add(nodeInstance);
    }

    public void removeNodeInstance(final NodeInstance nodeInstance) {
        CompositeNodeInstance.this.nodeInstances.remove(nodeInstance);
    }
    ...
    public NodeInstance getFirstNodeInstance(final long nodeId) {
        for (final Iterator<NodeInstance> iterator = CompositeNodeInstance.this.nodeInstances.iterator(); iterator.hasNext();) {
    ...
    public NodeInstance getNodeInstance(final Node node) {
        ...
        if (node instanceof CompositeNode) {
            CompositeNodeInstance.CompositeNodeStartInstance nodeInstance = new CompositeNodeInstance.CompositeNodeStartInstance();
            nodeInstance.setNodeId(node.getId());
            nodeInstance.setNodeInstanceContainer(CompositeNodeInstance.this);
            nodeInstance.setProcessInstance(getProcessInstance());
            return nodeInstance;
        } else if (node instanceof CompositeNode) {
            CompositeNodeInstance.CompositeNodeEndInstance nodeInstance = new CompositeNodeInstance.CompositeNodeEndInstance();
            nodeInstance.setNodeId(node.getId());
            nodeInstance.setNodeInstanceContainer(CompositeNodeInstance.this);
            nodeInstance.setProcessInstance(getProcessInstance());
            return nodeInstance;
        ...
        NodeInstanceImpl nodeInstance = ((NodeInstanceImpl) (conf.getNodeInstance(actualNode, getProcessInstance(), CompositeNodeInstance.this)));
    ...
    public void signalEvent(String type, Object event) {
        List<NodeInstance> currentView = new ArrayList<NodeInstance>(CompositeNodeInstance.this.nodeInstances);
    ...
    public class CompositeNodeStartInstance extends NodeInstanceImpl {
        ...
        public CompositeNodeStart getCompositeNodeStart() {
    ...
    public class CompositeNodeEndInstance extends NodeInstanceImpl {
        ...
        public CompositeNodeEnd getCompositeNodeEnd() {
    ...
    public void nodeInstanceCompleted(NodeInstance nodeInstance, String outType) {
        ...
        if ((nodeInstance instanceof EndNodeInstance) || (nodeInstance instanceof EventSubProcessNodeInstance)) {
            if (((WorkflowProcess) (getProcessInstance().getProcess())).isAutoComplete()) {
    ...
    public void setState(final int state) {
        CompositeNodeInstance.this.state = state;
    ...
    public int getState() {
        return CompositeNodeInstance.this.state;
    }
    ...
    public void setCurrentLevel(int currentLevel) {
        CompositeNodeInstance.this.currentLevel = currentLevel;
    }
