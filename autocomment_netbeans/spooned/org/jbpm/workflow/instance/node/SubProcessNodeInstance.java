 ...
 */
public class SubProcessNodeInstance extends StateBasedNodeInstance implements ContextInstanceContainer , EventListener {
    ...
    public void internalTrigger(final NodeInstance from, String type) {
        ...
        for (Iterator<DataAssociation> iterator = getSubProcessNode().getInAssociations().iterator(); iterator.hasNext();) {
            ...
            if ((mapping.getTransformation()) != null) {
                ...
                if (variableScopeInstance != null) {
                    ...
                    try {
                        parameterValue = MVELSafeHelper.getEvaluator().eval(mapping.getSources().get(0), new org.jbpm.workflow.instance.impl.NodeInstanceResolverFactory(SubProcessNodeInstance.this));
                    } catch (Throwable t) {
                        parameterValue = VariableUtil.resolveVariable(mapping.getSources().get(0), SubProcessNodeInstance.this);
                        if ((parameterValue != null) && (!(parameterValue.equals(mapping.getSources().get(0))))) {
                            parameters.put(mapping.getTarget(), parameterValue);
                        } else {
                            SubProcessNodeInstance.logger.error("Could not find variable scope for variable {}", mapping.getSources().get(0));
                            SubProcessNodeInstance.logger.error("when trying to execute SubProcess node {}", getSubProcessNode().getName());
                            SubProcessNodeInstance.logger.error("Continuing without setting parameter.");
                        }
                    }
                }
            }
        ...
        while (matcher.find()) {
            ...
            if ((replacements.get(paramName)) == null) {
                ...
                if (variableScopeInstance != null) {
                    ...
                    try {
                        Object variableValue = MVELSafeHelper.getEvaluator().eval(paramName, new org.jbpm.workflow.instance.impl.NodeInstanceResolverFactory(SubProcessNodeInstance.this));
                        String variableValueString = variableValue == null ? "" : variableValue.toString();
                        replacements.put(paramName, variableValueString);
                    } catch (Throwable t) {
                        SubProcessNodeInstance.logger.error("Could not find variable scope for variable {}", paramName);
                        SubProcessNodeInstance.logger.error("when trying to replace variable in processId for sub process {}", getNodeName());
                        SubProcessNodeInstance.logger.error("Continuing without setting process id.");
                    }
                }
            } 
        }
        ...
        if (process == null) {
            SubProcessNodeInstance.logger.error("Could not find process {}", processId);
            SubProcessNodeInstance.logger.error("Aborting process");
            ...
            SubProcessNodeInstance.this.processInstanceId = processInstance.getId();
    ...
    public void internalSetProcessInstanceId(long processInstanceId) {
        SubProcessNodeInstance.this.processInstanceId = processInstanceId;
    }
    ...
    private void addProcessListener() {
        getProcessInstance().addEventListener(("processInstanceCompleted:" + (processInstanceId)), SubProcessNodeInstance.this, true);
    }

    public void removeEventListeners() {
        super.removeEventListeners();
        getProcessInstance().removeEventListener(("processInstanceCompleted:" + (processInstanceId)), SubProcessNodeInstance.this, true);
    }
                ...
                if ((mapping.getTransformation()) != null) {
                    ...
                    if (transformer != null) {
                        ...
                        if ((variableScopeInstance != null) && (parameterValue != null)) {
                            ...
                            SubProcessNodeInstance.logger.warn("Could not find variable scope for variable {}", mapping.getTarget());
                            SubProcessNodeInstance.logger.warn("Continuing without setting variable.");
                        }
                    } 
                } else {
                    VariableScopeInstance variableScopeInstance = ((VariableScopeInstance) (resolveContextInstance(VariableScope.VARIABLE_SCOPE, mapping.getTarget())));
                    if (variableScopeInstance != null) {
                        ...
                        if (value == null) {
                            try {
                                value = MVELSafeHelper.getEvaluator().eval(mapping.getSources().get(0), new org.jbpm.workflow.instance.impl.VariableScopeResolverFactory(subProcessVariableScopeInstance));
                            } catch (Throwable t) {
                                // do nothing
                            }
                        } 
                        ...
                        SubProcessNodeInstance.logger.error("Could not find variable scope for variable {}", mapping.getTarget());
                        SubProcessNodeInstance.logger.error("when trying to complete SubProcess node {}", getSubProcessNode().getName());
                        SubProcessNodeInstance.logger.error("Continuing without setting variable.");
                    }
                }
            }
        } 
    ...
    public List<ContextInstance> getContextInstances(String contextId) {
        return SubProcessNodeInstance.this.subContextInstances.get(contextId);
    }

    @Override
    public void addContextInstance(String contextId, ContextInstance contextInstance) {
        List<ContextInstance> list = SubProcessNodeInstance.this.subContextInstances.get(contextId);
        if (list == null) {
            list = new ArrayList<ContextInstance>();
            SubProcessNodeInstance.this.subContextInstances.put(contextId, list);
        } 
    ...
    public ContextInstance getContextInstance(Context context) {
        ...
        ContextInstance contextInstance = ((ContextInstance) (conf.getContextInstance(context, SubProcessNodeInstance.this, ((ProcessInstance) (getProcessInstance())))));
    ...
    protected Map<String, Object> getSourceParameters(DataAssociation association) {
        ...
        for (String sourceParam : association.getSources()) {
            ...
            if (variableScopeInstance != null) {
                ...
                try {
                    parameterValue = MVELSafeHelper.getEvaluator().eval(sourceParam, new org.jbpm.workflow.instance.impl.NodeInstanceResolverFactory(SubProcessNodeInstance.this));
                } catch (Throwable t) {
                    SubProcessNodeInstance.logger.warn("Could not find variable scope for variable {}", sourceParam);
                }
            }
            if (parameterValue != null) {
