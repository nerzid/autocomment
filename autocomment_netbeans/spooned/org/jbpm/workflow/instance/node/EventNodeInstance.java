 ...
 */
public class EventNodeInstance extends ExtendedNodeInstanceImpl implements EventBasedNodeInstanceInterface , EventNodeInstanceInterface {
    ...
    private class VariableExternalEventListener implements Serializable , EventListener {
        private static final long serialVersionUID = 5L;

        private String eventType;

        VariableExternalEventListener(String eventType) {
            EventNodeInstance.VariableExternalEventListener.this.eventType = eventType;
        }
    ...
    public void addEventListeners() {
        ...
        if (isVariableExpression(getEventNode().getType())) {
            getProcessInstance().addEventListener(eventType, new EventNodeInstance.VariableExternalEventListener(eventType), true);
        } else {
    ...
    private boolean isVariableExpression(String eventType) {
        ...
        Matcher matcher = EventNodeInstance.PARAMETER_MATCHER.matcher(eventType);
        if (matcher.find()) {
            return true;
        } 
        return false;
    ...
    private String resolveVariable(String s) {
        ...
        Matcher matcher = EventNodeInstance.PARAMETER_MATCHER.matcher(s);
        while (matcher.find()) {
