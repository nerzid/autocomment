...
import JBPMMessages.ProcessInstance.NodeInstance.Builder;
...
import JBPMMessages.IterationLevel;
import org.jbpm.workflow.instance.node.JoinInstance;
import JBPMMessages.ProcessInstance.NodeInstanceContent.JoinNode.JoinTrigger;
...
public abstract class AbstractProtobufProcessInstanceMarshaller implements ProcessInstanceMarshaller {
    // Output methods
    public org.kie.api.runtime.process.ProcessInstance writeProcessInstance(MarshallerWriteContext context, org.kie.api.runtime.process.ProcessInstance processInstance) throws IOException {
        WorkflowProcessInstanceImpl workFlow = ((WorkflowProcessInstanceImpl) (processInstance));
        JBPMMessages.ProcessInstance.Builder _instance = JBPMMessages.ProcessInstance.newBuilder().setId(workFlow.getId()).setProcessId(workFlow.getProcessId()).setState(workFlow.getState()).setNodeInstanceCounter(workFlow.getNodeInstanceCounter()).setProcessType(workFlow.getProcess().getType()).setParentProcessInstanceId(workFlow.getParentProcessInstanceId()).setSignalCompletion(workFlow.isSignalCompletion());
    ...
    public NodeInstance writeNodeInstance(MarshallerWriteContext context, NodeInstance nodeInstance) throws IOException {
        JBPMMessages.ProcessInstance.NodeInstance.Builder _node = JBPMMessages.ProcessInstance.NodeInstance.newBuilder().setId(nodeInstance.getId()).setNodeId(nodeInstance.getNodeId()).setLevel(((org.jbpm.workflow.instance.NodeInstance) (nodeInstance)).getLevel());
        _node.setContent(writeNodeInstanceContent(_node, nodeInstance, context));
        return _node.build();
    }

    protected NodeInstanceContent writeNodeInstanceContent(Builder _node, NodeInstance nodeInstance, MarshallerWriteContext context) throws IOException {
        ...
        if (nodeInstance instanceof RuleSetNodeInstance) {
            ...
            if (nodeInstance instanceof DynamicNodeInstance) {
                _type = NodeInstanceType.DYNAMIC_NODE;
            } else if (nodeInstance instanceof EventSubProcessNodeInstance) {
                _type = NodeInstanceType.EVENT_SUBPROCESS_NODE;
            } else {
                _type = NodeInstanceType.COMPOSITE_CONTEXT_NODE;
            }
    ...
    // Input methods
    public org.kie.api.runtime.process.ProcessInstance readProcessInstance(MarshallerReaderContext context) throws IOException {
        InternalKnowledgeBase ruleBase = context;
        InternalWorkingMemory wm = context;
        JBPMMessages.ProcessInstance _instance = ((JBPMMessages.ProcessInstance) (context));
        if (_instance == null) {
    ...
    public NodeInstance readNodeInstance(MarshallerReaderContext context, NodeInstanceContainer nodeInstanceContainer, WorkflowProcessInstance processInstance) throws IOException {
        JBPMMessages.ProcessInstance.NodeInstance _node = ((JBPMMessages.ProcessInstance.NodeInstance) (context));
        ...
        switch (_node.getContent().getType()) {
            ...
            case DYNAMIC_NODE :
                if ((_node.getContent().getComposite().getVariableCount()) > 0) {
                    Context variableScope = ((org.jbpm.process.core.Process) (((ProcessInstance) (processInstance)).getProcess())).getDefaultContext(VariableScope.VARIABLE_SCOPE);
    ...
    protected NodeInstanceImpl readNodeInstanceContent(NodeInstance _node, MarshallerReaderContext context, WorkflowProcessInstance processInstance) throws IOException {
