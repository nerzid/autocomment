...
public class FormProviderServiceImplTest {
    ...
    public void testGetFormDisplayTaskWithoutProcess() {
        ...
        assertEquals("", form);
        verify(dataService, never()).getProcessesByDeploymentIdProcessId(Matchers.anyString(), Matchers.anyString());
        verify(formProvider).render(Mockito.eq(task.getName()), Mockito.eq(task), Mockito.isNull(ProcessDefinition.class), Mockito.anyMap());
    }
    ...
    public void testGetFormDisplayTask() {
        ...
        verify(formProvider).render(Mockito.eq(task.getName()), Mockito.eq(task), Mockito.eq(processDefinition), Mockito.anyMap());
    }
}
