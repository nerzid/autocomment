...
public class DeploymentIdsPreprocessor implements DataSetPreprocessor {
    ...
    public DeploymentIdsPreprocessor(DeploymentRolesManager deploymentRolesManager, IdentityProvider identityProvider, String columnId) {
        DeploymentIdsPreprocessor.this.deploymentRolesManager = deploymentRolesManager;
        DeploymentIdsPreprocessor.this.identityProvider = identityProvider;
        DeploymentIdsPreprocessor.this.columnId = columnId;
    }
    ...
    public void preprocess(DataSetLookup lookup) {
        ...
        if ((lookup.getFirstFilterOp()) != null) {
            lookup.getFirstFilterOp().addFilterColumn(FilterFactory.in(columnId, deploymentIds));
        } else {
            DataSetFilter filter = new DataSetFilter();
            filter.addFilterColumn(FilterFactory.in(columnId, deploymentIds));
