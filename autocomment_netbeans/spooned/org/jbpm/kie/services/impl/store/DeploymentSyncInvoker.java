...
public class DeploymentSyncInvoker {
    ...
    public DeploymentSyncInvoker(DeploymentSynchronizer synchronizer, Long delay, Long period, TimeUnit timeUnit) {
        this.synchronizer = synchronizer;
        DeploymentSyncInvoker.this.delay = delay;
        DeploymentSyncInvoker.this.period = period;
        DeploymentSyncInvoker.this.timeUnit = timeUnit;
    }
    ...
    public void stop() {
        ...
        if ((DeploymentSyncInvoker.this.executor) != null) {
            if ((future) != null) {
                DeploymentSyncInvoker.this.future.cancel(false);
            } 
            DeploymentSyncInvoker.this.executor.shutdown();
            try {
                if (!(DeploymentSyncInvoker.this.executor.awaitTermination(10, TimeUnit.SECONDS))) {
                    DeploymentSyncInvoker.this.executor.shutdownNow();
                } 
