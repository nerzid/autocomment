...
public class KModuleDeploymentService extends AbstractDeploymentService {
    ...
    public KModuleDeploymentService() {
        try {
            processIdXPathExpression = XPathFactory.newInstance().newXPath().compile(KModuleDeploymentService.PROCESS_ID_XPATH);
        } catch (XPathExpressionException e) {
            KModuleDeploymentService.logger.error("Unable to parse '{}' XPath expression due to {}", KModuleDeploymentService.PROCESS_ID_XPATH, e.getMessage());
        }
    }
        ...
        try {
            ...
            if (StringUtils.isEmpty(kbaseName)) {
                ...
                if (defaultKBaseModel != null) {
                    ...
                    kbaseName = KModuleDeploymentService.DEFAULT_KBASE_NAME;
                }
            } 
            ...
            if ((module.getKieDependencies()) != null) {
                ...
                for (InternalKieModule depModule : dependencies) {
                    KModuleDeploymentService.logger.debug(("Processing dependency module " + (depModule.getReleaseId())));
            ...
            KModuleDeploymentService.logger.warn("Unexpected error while deploying unit {}", unit.getIdentifier(), e);
        ...
        if ((remoteableClasses != null) && (!(remoteableClasses.isEmpty()))) {
            for (String className : remoteableClasses) {
                ...
                try {
                    ...
                    KModuleDeploymentService.logger.debug("Loaded {} into the classpath from deployment descriptor {}", className, kieContainer.getReleaseId().toExternalForm());
    ...
    private void addClassToDeployedUnit(Class deploymentClass, DeployedUnitImpl deployedUnit) {
        if (deploymentClass != null) {
            ...
            if ((limitClasses != null) && limitClasses) {
                ...
                KModuleDeploymentService.logger.debug("Loaded {} onto the classpath from deployment {}", deploymentClass.getName(), unit.getIdentifier());
    ...
    public void setBpmn2Service(DefinitionService bpmn2Service) {
        KModuleDeploymentService.this.bpmn2Service = bpmn2Service;
    }

    public void setMerger(DeploymentDescriptorMerger merger) {
        KModuleDeploymentService.this.merger = merger;
    }

    public void setFormManagerService(FormManagerService formManagerService) {
        KModuleDeploymentService.this.formManagerService = formManagerService;
    }

    public void setExecutorService(ExecutorService executorService) {
        KModuleDeploymentService.this.executorService = executorService;
    }
    ...
    protected String getProcessId(String processSource) {
        try {
            ...
            KModuleDeploymentService.logger.error("Unable to find process id from process source due to {}", e.getMessage());
            return null;
        }
    }
}
