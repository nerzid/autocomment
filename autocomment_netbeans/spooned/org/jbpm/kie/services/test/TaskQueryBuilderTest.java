...
public class TaskQueryBuilderTest extends AbstractKieServicesBaseTest {
    ...
    @Before
    public void prepare() {
        TaskQueryBuilderTest.this.dataSourceJNDIname = getDataSourceJNDI();
        configureServices();
        TaskQueryBuilderTest.logger.debug("Preparing kjar");
    ...
    public void testGetTaskPageInstances() {
        ...
        for (int i = 0; i < 10; ++i) {
            ...
            Assert.assertNotNull(pids[i]);
        }
        List<TaskSummary> taskInstanceLogs = runtimeDataService.taskSummaryQuery(correctUser).ascending(OrderBy.processInstanceId).maxResults(4).offset(6).build().getResultList();
        Assert.assertNotNull(taskInstanceLogs);
        Assert.assertEquals(4, taskInstanceLogs.size());
        int pid = 6;
        for (TaskSummary ts : taskInstanceLogs) {
            Assert.assertEquals(((long) (pids[pid])), ((long) (ts.getProcessInstanceId())));
            pid++;
        }
    }
    ...
    public void testGetTaskIdGroupsInstances() {
        ...
        List<TaskSummary> allTaskInstanceLogs = runtimeDataService.taskSummaryQuery(correctUser).ascending(OrderBy.taskId).build().getResultList();
        Assert.assertNotNull(allTaskInstanceLogs);
        long firstTaskId = allTaskInstanceLogs.get(0).getId();
        List<TaskSummary> taskInstanceLogs = runtimeDataService.taskSummaryQuery(correctUser).newGroup().taskIdRange(firstTaskId, (firstTaskId + 1)).endGroup().or().newGroup().taskIdRange((firstTaskId + 3), (firstTaskId + 4)).endGroup().build().getResultList();
        Assert.assertNotNull(taskInstanceLogs);
        Assert.assertEquals(4, taskInstanceLogs.size());
        long tid = firstTaskId;
        for (TaskSummary ts : taskInstanceLogs) {
            Assert.assertEquals(tid, ((long) (ts.getId())));
            tid++;
    ...
    public void testGetDescendingTaskInstances() {
        ...
        List<TaskSummary> taskInstanceLogs = runtimeDataService.taskSummaryQuery(correctUser).descending(OrderBy.processInstanceId).build().getResultList();
        Assert.assertNotNull(taskInstanceLogs);
        Assert.assertEquals(3, taskInstanceLogs.size());
        int pos = 2;
        for (TaskSummary ts : taskInstanceLogs) {
            Assert.assertEquals(((long) (pids[pos])), ((long) (ts.getProcessInstanceId())));
            pos--;
        }
    }
    ...
    public void testGetTaskVariables() {
        ...
        List<TaskSummary> taskInstanceLogs = runtimeDataService.taskSummaryQuery("katy").ascending(OrderBy.processInstanceId).newGroup().variableName("name").variableValue("Shallan").endGroup().or().newGroup().variableName("name").variableValue("Kaladin").endGroup().build().getResultList();
        Assert.assertNotNull(taskInstanceLogs);
        Assert.assertEquals(2, taskInstanceLogs.size());
        int pos = 1;
        for (TaskSummary ts : taskInstanceLogs) {
            Assert.assertEquals(((long) (pids[pos])), ((long) (ts.getProcessInstanceId())));
            pos++;
        }
        taskInstanceLogs = runtimeDataService.taskSummaryQuery("katy").variableName("name").variableValue("Dalinar Kholin", "Shallan").build().getResultList();
        Assert.assertNotNull(taskInstanceLogs);
        Assert.assertEquals(2, taskInstanceLogs.size());
        taskInstanceLogs = // Kaladin
        runtimeDataService.taskSummaryQuery("katy").variableName("name").regex().variableValue("*lad*").build().getResultList();
        Assert.assertNotNull(taskInstanceLogs);
        Assert.assertEquals(1, taskInstanceLogs.size());
    }
}
