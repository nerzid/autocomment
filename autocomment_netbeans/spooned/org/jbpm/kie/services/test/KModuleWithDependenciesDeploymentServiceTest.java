...
public class KModuleWithDependenciesDeploymentServiceTest extends AbstractKieServicesBaseTest {
    ...
    public void prepare() {
        ...
        ReleaseId releaseId = ks.newReleaseId(KModuleWithDependenciesDeploymentServiceTest.GROUP_ID, KModuleWithDependenciesDeploymentServiceTest.ARTIFACT_ID, KModuleWithDependenciesDeploymentServiceTest.VERSION);
        ...
        ReleaseId dataReleaseId = ks.newReleaseId(KModuleWithDependenciesDeploymentServiceTest.DATA_GROUP_ID, KModuleWithDependenciesDeploymentServiceTest.DATA_ARTIFACT_ID, KModuleWithDependenciesDeploymentServiceTest.DATA_VERSION);
    ...
    public void testDeploymentOfProcesses() throws Exception {
        ...
        KModuleDeploymentUnit deploymentUnit = new KModuleDeploymentUnit(KModuleWithDependenciesDeploymentServiceTest.GROUP_ID, KModuleWithDependenciesDeploymentServiceTest.ARTIFACT_ID, KModuleWithDependenciesDeploymentServiceTest.VERSION, "defaultKieBase", "defaultKieSession");
        ...
        Assert.assertEquals("org.jbpm.test:jbpm-module:1.0.0:defaultKieBase:defaultKieSession", deployed.getDeploymentUnit().getIdentifier());
        Assert.assertTrue((deployed instanceof DeployedUnitImpl));
        Assert.assertEquals(2, ((DeployedUnitImpl) (deployed)).getDeployedClasses().size());
        List<String> classnames = new ArrayList<String>();
        for (Class<?> clazz : ((DeployedUnitImpl) (deployed)).getDeployedClasses()) {
            classnames.add(clazz.getName());
        }
        Assert.assertTrue(classnames.contains("org.jbpm.data.rest.CustomDataObject"));
        Assert.assertTrue(classnames.contains("org.jbpm.test.Person"));
