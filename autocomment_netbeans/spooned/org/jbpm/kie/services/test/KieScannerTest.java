...
public class KieScannerTest {
    ...
    public void setUp() throws Exception {
        KieScannerTest.this.fileManager = new FileManager();
        KieScannerTest.this.fileManager.setUp();
        kPom = createKPom(fileManager, releaseId);
    }

    @After
    public void tearDown() throws Exception {
        KieScannerTest.this.fileManager.tearDown();
    }
    ...
    public void testKScanner() throws Exception {
        ...
        Assert.assertTrue(((processInstance2.getState()) == (ProcessInstance.STATE_COMPLETED)));
        Assert.assertEquals(1, results.size());
        Assert.assertEquals("Morning", results.get(0));
        results.clear();
    ...
    private InternalKieModule createJbpmKieJar(String message) {
        ...
        Assert.assertTrue(kieBuilder.buildAll().getResults().getMessages().isEmpty());
    ...
    protected KieFileSystem createKieFileSystemWithKProject(KieServices ks, boolean isdefault) {
        ...
        KieSessionModel ksession1 = kieBaseModel1.newKieSessionModel("KSession1").setDefault(isdefault).setType(KieSessionModel.KieSessionType.STATEFUL).setClockType(ClockTypeOption.get("realtime"));
