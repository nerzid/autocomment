...
public class DeploymentServiceTest extends AbstractKieServicesBaseTest {
    ...
    public void testDeploymentOfAllProcesses() {
        Assert.assertNotNull(deploymentService);
        // deploy first unit
        DeploymentUnit deploymentUnitGeneral = new org.jbpm.kie.services.impl.KModuleDeploymentUnit(GROUP_ID, ARTIFACT_ID, VERSION);
        ...
        // deploy second unit
        DeploymentUnit deploymentUnitSupport = new org.jbpm.kie.services.impl.KModuleDeploymentUnit(GROUP_ID, "support", VERSION);
    ...
    public void testDuplicatedDeployment() {
        Assert.assertNotNull(deploymentService);
        DeploymentUnit deploymentUnit = new org.jbpm.kie.services.impl.KModuleDeploymentUnit(GROUP_ID, ARTIFACT_ID, VERSION);
        Assert.assertFalse(deploymentService.isDeployed(deploymentUnit.getIdentifier()));
        deploymentService.deploy(deploymentUnit);
        units.add(deploymentUnit);
        Assert.assertTrue(deploymentService.isDeployed(deploymentUnit.getIdentifier()));
    ...
    public void testDeploymentOfMultipleVersions() {
        Assert.assertNotNull(deploymentService);
        DeploymentUnit deploymentUnit = new org.jbpm.kie.services.impl.KModuleDeploymentUnit(GROUP_ID, ARTIFACT_ID, VERSION);
        DeploymentUnit deploymentUnit3 = new org.jbpm.kie.services.impl.KModuleDeploymentUnit(GROUP_ID, ARTIFACT_ID, "1.1.0-SNAPSHOT");
        deploymentService.deploy(deploymentUnit);
        units.add(deploymentUnit);
        ...
        Assert.assertEquals(1, ((DeployedUnitImpl) (deployed3)).getDeployedClasses().size());
        Assert.assertNotNull(runtimeDataService);
        Collection<ProcessDefinition> processes = runtimeDataService.getProcesses(new org.kie.api.runtime.query.QueryContext());
        Assert.assertNotNull(processes);
