...
public class ProcessBuilderImpl implements org.drools.compiler.compiler.ProcessBuilder {
    ...
    public ProcessBuilderImpl(KnowledgeBuilderImpl packageBuilder) {
        ProcessBuilderImpl.this.knowledgeBuilder = packageBuilder;
    ...
    public void configurePackageBuilder(KnowledgeBuilder packageBuilder) {
        ...
        if ((conf.getSemanticModules().getSemanticModule(ProcessSemanticModule.URI)) == null) {
            conf.addSemanticModule(new org.jbpm.compiler.xml.ProcessSemanticModule());
        } 
    }
    ...
    public void buildProcess(final Process process, Resource resource) {
        ...
        if (validator == null) {
            ProcessBuilderImpl.logger.warn("Could not find validator for process {}.", ((Process) (process)).getType());
            ProcessBuilderImpl.logger.warn("Continuing without validation of the process {} [{}]", process.getName(), process.getId());
        } else {
            ProcessValidationError[] errors = validator.validateProcess(((WorkflowProcess) (process)));
            if (errors != 0) {
                hasErrors = true;
                for (int i = 0; i < errors; i++) {
                    ProcessBuilderImpl.this.errors.add(new org.drools.compiler.compiler.ParserError(resource, errors[i].toString(), (-1), (-1)));
                }
            } 
        }
        if (!hasErrors) {
            ...
            try {
                ...
                // should never occur
                ProcessBuilderImpl.logger.error("IOException during addPackageFromDRL", e);
            } catch (DroolsParserException e) {
                // should never occur
                ProcessBuilderImpl.logger.error("DroolsParserException during addPackageFromDRL", e);
            }
            PackageRegistry pkgRegistry = ProcessBuilderImpl.this.knowledgeBuilder.getPackageRegistry(process.getPackageName());
            if (pkgRegistry != null) {
                ...
                if (p != null) {
                    if (validator != null) {
                        ...
                        if (validator.compilationSupported()) {
                            ...
                            ProcessBuildContext buildContext = new ProcessBuildContext(ProcessBuilderImpl.this.knowledgeBuilder, p, process, processDescr, dialectRegistry, dialect);
                        ...
                        if (duplicateProcess != null) {
                            ...
                            if ((((resource == null) || (duplicatedResource == null)) || ((duplicatedResource.getSourcePath()) == null)) || (duplicatedResource.getSourcePath().equals(resource.getSourcePath()))) {
                                ProcessBuilderImpl.this.errors.add(new org.drools.compiler.compiler.DuplicateProcess(process, ProcessBuilderImpl.this.knowledgeBuilder.getBuilderConfiguration()));
                            } else {
                                ProcessBuilderImpl.this.errors.add(new org.drools.compiler.compiler.ParserError(resource, ("Process with same id already exists: " + (process.getId())), (-1), (-1)));
                            }
                        } 
            ...
            for (Context context : exceptionScopes) {
                ...
                for (ExceptionHandler exceptionHandler : exceptionScope.getExceptionHandlers().values()) {
                    if (exceptionHandler instanceof ActionExceptionHandler) {
                        ...
                        dialect.getActionBuilder().build(buildContext, action, actionDescr, ((org.jbpm.process.core.impl.ProcessImpl) (buildContext.getProcess())));
                    } 
                }
            }
        } 
    ...
    public void buildNodes(WorkflowProcess process, ProcessBuildContext context) {
        ...
        if (!(context.getErrors().isEmpty())) {
            ProcessBuilderImpl.this.errors.addAll(context.getErrors());
        } 
        ProcessDialectRegistry.getDialect(JavaDialect.ID).addProcess(context);
    }

    private void processNodes(org.kie.api.definition.process.Node[] nodes, Process process, ProcessDescr processDescr, ProcessBuildContext context) {
        for (org.kie.api.definition.process.Node node : nodes) {
            ...
            if (node instanceof NodeContainer) {
                processNodes(((NodeContainer) (node)).getNodes(), process, processDescr, context);
            } 
    ...
    public List<Process> addProcessFromXml(final Resource resource) throws IOException {
        ...
        try {
            ...
            if (processes != null) {
                ...
                ProcessBuilderImpl.this.errors.add(new org.drools.compiler.compiler.ProcessLoadError(resource, "unable to parse xml", null));
            }
        } catch (FactoryConfigurationError e1) {
            ProcessBuilderImpl.this.errors.add(new org.drools.compiler.compiler.ProcessLoadError(resource, "FactoryConfigurationError ", e1.getException()));
        } catch (Exception e2) {
            e2.printStackTrace();
            ProcessBuilderImpl.this.errors.add(new org.drools.compiler.compiler.ProcessLoadError(resource, "unable to parse xml", e2));
        } finally {
            reader.close();
        }
        return processes;
    ...
    private String createSplitRule(Process process, Connection connection, String constraint) {
        return (((((((((((((("rule \"RuleFlow-Split-" + (process.getId())) + "-") + (((Node) (connection.getFrom())).getUniqueId())) + "-") + (((Node) (connection.getTo())).getUniqueId())) + "-") + (connection.getToType())) + "\"  @Propagation(EAGER) \n") + "      ruleflow-group \"DROOLS_SYSTEM\" \n") + "    when \n") + "      ") + constraint) + "\n") + "    then \n") + "end \n\n";
    }

    private String createMilestoneRule(Process process, MilestoneNode milestone) {
        return (((((((((("rule \"RuleFlow-Milestone-" + (process.getId())) + "-") + (milestone.getUniqueId())) + "\" @Propagation(EAGER) \n") + "      ruleflow-group \"DROOLS_SYSTEM\" \n") + "    when \n") + "      ") + (milestone.getConstraint())) + "\n") + "    then \n") + "end \n\n";
    }
    ...
    private String createStateRule(Process process, StateNode state, ConnectionRef key, Constraint constraint) {
        if (((constraint.getConstraint()) == null) || ((constraint.getConstraint().trim().length()) == 0)) {
            ...
            return (((((((((((((("rule \"RuleFlowStateNode-" + (process.getId())) + "-") + (state.getUniqueId())) + "-") + (key.getNodeId())) + "-") + (key.getToType())) + "\" @Propagation(EAGER) \n") + "      ruleflow-group \"DROOLS_SYSTEM\" \n") + "    when \n") + "      ") + (state.getConstraints().get(key).getConstraint())) + "\n") + "    then \n") + "end \n\n";
        }
    }
    ...
    private String createEventStateRule(Process process, EventNode event) {
        ...
        if ((condition == null) || ((condition.trim().length()) == 0)) {
            ...
            return (((((((((((("rule \"RuleFlowStateEvent-" + (process.getId())) + "-") + (event.getUniqueId())) + "-") + attachedTo) + "\" @Propagation(EAGER) \n") + "      ruleflow-group \"DROOLS_SYSTEM\" \n") + "    when \n") + "      ") + condition) + "\n") + "    then \n") + "end \n\n";
        }
    }
    ...
    private String createEventSubprocessStateRule(Process process, CompositeNode compositeNode, ConstraintTrigger trigger) {
        ...
        if ((condition == null) || ((condition.trim().length()) == 0)) {
            ...
            return (((((((((("rule \"RuleFlowStateEventSubProcess-" + (process.getId())) + "-") + (compositeNode.getUniqueId())) + "\" @Propagation(EAGER) \n") + "      ruleflow-group \"DROOLS_SYSTEM\" \n") + "    when \n") + "      ") + condition) + "\n") + "    then \n") + "end \n\n";
        }
    }
    ...
    private String createStartConstraintRule(Process process, NodeContainer nodeContainer, ConstraintTrigger trigger) {
        ...
        String result = ((((((("rule \"RuleFlow-Start-" + (process.getId())) + "\" @Propagation(EAGER) \n") + ((trigger.getHeader()) == null ? "" : ("        " + (trigger.getHeader())) + " \n")) + "    when\n") + "        ") + (trigger.getConstraint())) + "\n") + "    then\n";
