...
public class RuleFlowMigrator {
    ...
    public static String portRFToCurrentVersion(String xml) throws Exception {
        return RuleFlowMigrator.portToCurrentVersion(xml, RuleFlowMigrator.XSL_RF_FROM_4_TO_5);
    }
    ...
    public static String portRFMToCurrentVersion(String xml) throws Exception {
        return RuleFlowMigrator.portToCurrentVersion(xml, RuleFlowMigrator.XSL_RFM_FROM_4_TO_5);
    }
    ...
    private static String portToCurrentVersion(String xml, String xsl) throws Exception {
        // convert it.
        String version5XML = RuleFlowMigrator.XSLTransformation.transform(xsl, xml, null);
        // Add the namespace attribute to the process element as it is not added by the XSL transformation.
        version5XML = version5XML.replaceAll("<process ", RuleFlowMigrator.PROCESS_ELEMENT_WITH_NAMESPACE);
        return version5XML;
    }
    ...
    public static final void main(String[] args) {
        try {
            if ((args.length) != 2) {
                RuleFlowMigrator.logger.info("usage: RuleFileMigrator source_file dest_file");
                System.exit(1);
            } 
            ...
            if (result != null) {
                RuleFlowMigrator.logger.info("Ruleflow migrated from version 4.0 to 5.0");
                FileWriter fw = new FileWriter(outFile);
                fw.write(result);
                fw.flush();
                fw.close();
            } else {
                RuleFlowMigrator.logger.info("No Ruleflow Migration Reguired - Ruleflow is version 5.0");
            }
        } catch (Throwable t) {
            t.printStackTrace();
        }
    ...
    private static class XSLTransformation {
        public static String transform(String stylesheet, String srcXMLString, HashMap<String, String> params) throws Exception {
            ...
            RuleFlowMigrator.XSLTransformation.transform(stylesheet, src, result, params);
            return writer.toString();
        }

        public static void transform(String stylesheet, Source src, Result res, HashMap<String, String> params) throws Exception {
            Transformer transformer = RuleFlowMigrator.XSLTransformation.getTransformer(stylesheet);
        ...
        private static Transformer getTransformer(String stylesheet) throws Exception {
            ...
            try {
                InputStream in = RuleFlowMigrator.XSLTransformation.class.getResourceAsStream(stylesheet);
