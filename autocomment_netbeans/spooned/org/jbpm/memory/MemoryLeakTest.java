...
public class MemoryLeakTest {
    ...
    public static void beforeClass() {
        MemoryLeakTest.testContext = setupWithPoolingDataSource(JBPM_PERSISTENCE_UNIT_NAME);
    }

    @AfterClass
    public static void afterClass() {
        cleanUp(MemoryLeakTest.testContext);
    }

    @Before
    public void before() {
        env = createEnvironment(MemoryLeakTest.testContext);
    }
    ...
    private KieBase createKnowledgeBase() {
        ...
        if (!(kbuilder.getErrors().isEmpty())) {
            ...
            while (errIter.hasNext()) {
                ...
                MemoryLeakTest.logger.warn(((((err.getMessage()) + " (") + (lines.toString())) + ")"));
            }
    ...
    private void createKnowledgeSessionStartProcessEtc(KieBase kbase) {
        MemoryLeakTest.logger.info(("session count=" + (kbase.getKieSessions().size())));
        ...
        try {
            ...
            ProcessInstance processInstance = ksession.createProcessInstance(MemoryLeakTest.PROCESS_NAME, processParams);
    ...
    private void addEventListenersToSession(StatefulKnowledgeSession session) {
        session.addEventListener(new DefaultProcessEventListener() {
            ...
            public void afterProcessStarted(ProcessStartedEvent event) {
                MemoryLeakTest.logger.info((">>> Firing All the Rules after process started! " + event));
