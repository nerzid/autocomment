...
public class StartEventTest extends JbpmBpmn2TestCase {
    ...
    public void testTimerStartCycleLegacy() throws Exception {
        ...
        StartEventTest.logger.debug(("About to start ###### " + (new Date())));
    ...
    public void testTimerStartDateISO() throws Exception {
        ...
        byte[] content = IoUtils.readBytesFromInputStream(StartEventTest.this.getClass().getResourceAsStream("/BPMN2-TimerStartDate.bpmn2"));
    ...
    public void testSignalStartDynamic() throws Exception {
        ...
        ksession.addEventListener(new DefaultProcessEventListener() {
            public void beforeProcessStarted(ProcessStartedEvent event) {
                StartEventTest.logger.info("{}", event.getProcessInstance().getId());
                list.add(event.getProcessInstance().getId());
            }
        });
        ksession.signalEvent("MySignal", "NewValue");
    ...
    public void testTimerCycle() throws Exception {
        ...
        ksession = createKnowledgeSession(kbase);
        ksession.addEventListener(countDownListener);
        StartEventTest.StartCountingListener listener = new StartEventTest.StartCountingListener();
