...
public class ProcessHandler extends BaseAbstractHandler implements Handler {
    ...
    public ProcessHandler() {
        if (((ProcessHandler.this.validParents) == null) && ((ProcessHandler.this.validPeers) == null)) {
            ...
            ProcessHandler.this.validParents.add(Definitions.class);
            this.validPeers = new HashSet();
            ProcessHandler.this.validPeers.add(null);
            ProcessHandler.this.validPeers.add(ItemDefinition.class);
            ProcessHandler.this.validPeers.add(Message.class);
            ProcessHandler.this.validPeers.add(Interface.class);
            ProcessHandler.this.validPeers.add(Escalation.class);
            ProcessHandler.this.validPeers.add(Error.class);
            ProcessHandler.this.validPeers.add(Signal.class);
            ProcessHandler.this.validPeers.add(DataStore.class);
            ProcessHandler.this.validPeers.add(RuleFlowProcess.class);
    ...
    public Object end(final String uri, final String localName, final ExtensibleXmlParser parser) throws SAXException {
        ...
        List<IntermediateLink> throwLinks = ((List<IntermediateLink>) (process.getMetaData(ProcessHandler.LINKS)));
        ProcessHandler.linkIntermediateLinks(process, throwLinks);
        List<SequenceFlow> connections = ((List<SequenceFlow>) (process.getMetaData(ProcessHandler.CONNECTIONS)));
        ProcessHandler.linkConnections(process, connections);
        ProcessHandler.linkBoundaryEvents(process);
        // This must be done *after* linkConnections(process, connections)
        // because it adds hidden connections for compensations
        List<Association> associations = ((List<Association>) (process.getMetaData(ProcessHandler.ASSOCIATIONS)));
        ProcessHandler.linkAssociations(((Definitions) (process.getMetaData("Definitions"))), process, associations);
        ...
        if (null != links) {
            ...
            for (IntermediateLink throwLink : throwLinks) {
                ...
                for (IntermediateLink catchLink : linksWithSharedNames) {
                    ...
                    if ((t != null) && (c != null)) {
                        org.jbpm.workflow.core.Connection result = new ConnectionImpl(t, NodeImpl.CONNECTION_DEFAULT_TYPE, c, NodeImpl.CONNECTION_DEFAULT_TYPE);
    ...
    public static void linkConnections(NodeContainer nodeContainer, List<SequenceFlow> connections) {
        if (connections != null) {
            for (SequenceFlow connection : connections) {
                ...
                org.jbpm.workflow.core.Connection result = new ConnectionImpl(source, NodeImpl.CONNECTION_DEFAULT_TYPE, target, NodeImpl.CONNECTION_DEFAULT_TYPE);
                ...
                if ("true".equals(System.getProperty("jbpm.enable.multi.con"))) {
                    ...
                    if (constraint != null) {
                        nodeImpl.addConstraint(new org.jbpm.workflow.core.impl.ConnectionRef(target.getId(), NodeImpl.CONNECTION_DEFAULT_TYPE), constraint);
                    } 
                } else if (source instanceof Split) {
                    Split split = ((Split) (source));
                    Constraint constraint = ProcessHandler.buildConstraint(connection, split);
                    split.addConstraint(new org.jbpm.workflow.core.impl.ConnectionRef(target.getId(), NodeImpl.CONNECTION_DEFAULT_TYPE), constraint);
                } 
            }
        } 
    }
    ...
    private static void linkBoundaryEscalationEvent(NodeContainer nodeContainer, Node node, String attachedTo, Node attachedNode) {
        ...
        DroolsConsequenceAction action = new DroolsConsequenceAction("java", ((((((((ProcessHandler.PROCESS_INSTANCE_SIGNAL_EVENT) + "Escalation-") + attachedTo) + "-") + escalationCode) + "\", kcontext.getVariable(\"") + variable) + "\"));"));
        ...
        if (cancelActivity) {
            ...
            cancelAction.setMetaData("Action", new org.jbpm.process.instance.impl.CancelNodeInstanceAction(attachedTo));
    ...
    private static void linkBoundaryErrorEvent(NodeContainer nodeContainer, Node node, String attachedTo, Node attachedNode) {
        ...
        ActionExceptionHandler exceptionHandler = new ActionExceptionHandler();
        String variable = ((EventNode) (node)).getVariableName();
        DroolsConsequenceAction action = new DroolsConsequenceAction("java", ((((((((ProcessHandler.PROCESS_INSTANCE_SIGNAL_EVENT) + "Error-") + attachedTo) + "-") + errorCode) + "\", kcontext.getVariable(\"") + variable) + "\"));"));
        exceptionHandler.setAction(action);
        exceptionHandler.setFaultVariable(variable);
    ...
    private static void linkBoundaryTimerEvent(NodeContainer nodeContainer, Node node, String attachedTo, Node attachedNode) {
        ...
        if (timeDuration != null) {
            ...
            compositeNode.addTimer(timer, new DroolsConsequenceAction("java", ((((((((ProcessHandler.PROCESS_INSTANCE_SIGNAL_EVENT) + "Timer-") + attachedTo) + "-") + timeDuration) + "-") + (node.getId())) + "\", kcontext.getNodeInstance().getId());")));
            ...
            compositeNode.addTimer(timer, new DroolsConsequenceAction("java", (((((((((ProcessHandler.PROCESS_INSTANCE_SIGNAL_EVENT) + "Timer-") + attachedTo) + "-") + timeCycle) + ((timer.getPeriod()) == null ? "" : "###" + (timer.getPeriod()))) + "-") + (node.getId())) + "\", kcontext.getNodeInstance().getId());")));
        } else if (timeDate != null) {
            timer.setDate(timeDate);
            timer.setTimeType(Timer.TIME_DATE);
            compositeNode.addTimer(timer, new DroolsConsequenceAction("java", ((((((((ProcessHandler.PROCESS_INSTANCE_SIGNAL_EVENT) + "Timer-") + attachedTo) + "-") + timeDate) + "-") + (node.getId())) + "\", kcontext.getNodeInstance().getId());")));
        } 
        if (cancelActivity) {
            List<DroolsAction> actions = ((EventNode) (node)).getActions(EndNode.EVENT_NODE_EXIT);
            if (actions == null) {
    ...
    private static void linkBoundaryCompensationEvent(NodeContainer nodeContainer, Node node, String attachedTo, Node attachedNode) {
        ...
        if (activityRef != null) {
            ProcessHandler.logger.warn("Attribute activityRef={} will be IGNORED since this is a Boundary Compensation Event.", activityRef);
        } 
    ...
    private static void linkBoundarySignalEvent(NodeContainer nodeContainer, Node node, String attachedTo, Node attachedNode) {
        ...
        if (cancelActivity) {
            ...
            action.setMetaData("Action", new org.jbpm.process.instance.impl.CancelNodeInstanceAction(attachedTo));
            actions.add(action);
            ((EventNode) (node)).setActions(EndNode.EVENT_NODE_EXIT, actions);
        } 
    }
        ...
        if (associations != null) {
            for (Association association : associations) {
                ...
                if ((source == null) || (target == null)) {
                    ...
                    ProcessHandler.checkBoundaryEventCompensationHandler(association, sourceNode, targetNode);
                    ...
                    if (compensationObject == null) {
                        ...
                        ProcessHandler.logger.warn("Setting {} attribute to true for node {}", isForCompensation, targetRef);
    ...
    private static void checkBoundaryEventCompensationHandler(Association association, Node source, Node target) {
        ...
        Map<String, List<Connection>> connectionsMap = targetNode.getOutgoingConnections();
        ConnectionImpl outgoingConnection = null;
        for (String connectionType : connectionsMap.keySet()) {
            List<Connection> connections = connectionsMap.get(connectionType);
                                ...
                                for (EventFilter filter : filters) {
                                    if (filter instanceof EventTypeFilter) {
                                        ...
                                        if ((type.startsWith("Error-")) || (type.startsWith("Escalation"))) {
                                            ...
                                            ActionExceptionHandler exceptionHandler = new ActionExceptionHandler();
                                            DroolsConsequenceAction action = new DroolsConsequenceAction("java", (((((ProcessHandler.PROCESS_INSTANCE_SIGNAL_EVENT) + signalType) + "\", ") + (faultVariable == null ? "null" : ("kcontext.getVariable(\"" + faultVariable) + "\")")) + ");"));
                                            exceptionHandler.setAction(action);
                                            ...
                                            if (isForCompensationObj == null) {
                                                ...
                                                ProcessHandler.logger.warn((("Overriding empty or false value of \"isForCompensation\" attribute on Event Sub-Process [" + (eventSubProcessNode.getMetaData("UniqueId"))) + "] and setting it to true."));
                                            } 
                                            ...
                                            ProcessHandler.addCompensationScope(process, eventSubProcessNode, parentSubProcess, compensationHandlerId);
                                        } 
                                    } 
                                }
    ...
    protected static void addCompensationScope(final RuleFlowProcess process, final Node node, final NodeContainer parentContainer, final String compensationHandlerId) {
    ...
    protected void handleIntermediateOrEndThrowCompensationEvent(ExtendedNodeImpl throwEventNode) {
        if ((throwEventNode.getMetaData("compensation-activityRef")) != null) {
            ...
            DroolsConsequenceAction compensationAction = new DroolsConsequenceAction("java", ((((ProcessHandler.PROCESS_INSTANCE_SIGNAL_EVENT) + "Compensation\", \"") + compensationEvent) + "\");"));
