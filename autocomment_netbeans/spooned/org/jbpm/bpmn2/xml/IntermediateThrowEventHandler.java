...
public class IntermediateThrowEventHandler extends AbstractNodeHandler {
    ...
    public Object end(final String uri, final String localName, final ExtensibleXmlParser parser) throws SAXException {
        ...
        if ((node.getAction()) == null) {
            node.setAction(new org.jbpm.workflow.core.impl.DroolsConsequenceAction("mvel", ""));
    ...
    protected void handleLinkNode(Element element, Node node, org.w3c.dom.Node xmlLinkNode, ExtensibleXmlParser parser) {
        ...
        node.setMetaData(IntermediateThrowEventHandler.LINK_NAME, name);
        ...
        while (null != xmlNode) {
            ...
            if (IntermediateThrowEventHandler.LINK_TARGET.equals(nodeName)) {
                ...
                node.setMetaData(IntermediateThrowEventHandler.LINK_TARGET, target);
            } 
            if (IntermediateThrowEventHandler.LINK_SOURCE.equals(nodeName)) {
                String source = xmlNode.getTextContent();
                ArrayList<String> sources = ((ArrayList<String>) (node.getMetaData().get(IntermediateThrowEventHandler.LINK_SOURCE)));
                ...
                // to do the xml dump
                sources.add(source);
                node.setMetaData(IntermediateThrowEventHandler.LINK_SOURCE, sources);
            } 
            xmlNode = xmlNode.getNextSibling();
        }
    ...
    public void handleSignalNode(final Node node, final Element element, final String uri, final String localName, final ExtensibleXmlParser parser) throws SAXException {
        ...
        while (xmlNode != null) {
            ...
            if ("dataInput".equals(nodeName)) {
                ...
                actionNode.setAction(new org.jbpm.workflow.core.impl.DroolsConsequenceAction("java", ((((((((((" Object tVariable = " + (variable == null ? "null" : variable)) + ";") + "org.jbpm.workflow.core.node.Transformation transformation = (org.jbpm.workflow.core.node.Transformation)kcontext.getNodeInstance().getNode().getMetaData().get(\"Transformation\");") + "if (transformation != null) {") + "  tVariable = new org.jbpm.process.core.event.EventTransformerImpl(transformation)") + "  .transformEvent(") + (variable == null ? "null" : variable)) + ");") + "}") + signalExpression)));
            } 
            xmlNode = xmlNode.getNextSibling();
        }
    }
