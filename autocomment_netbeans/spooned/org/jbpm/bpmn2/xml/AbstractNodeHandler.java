...
public abstract class AbstractNodeHandler extends BaseAbstractHandler implements Handler {
    ...
    protected void initValidParents() {
        AbstractNodeHandler.this.validParents = new HashSet<java.lang.Class<?>>();
        AbstractNodeHandler.this.validParents.add(NodeContainer.class);
    }

    protected void initValidPeers() {
        AbstractNodeHandler.this.validPeers = new HashSet<java.lang.Class<?>>();
        AbstractNodeHandler.this.validPeers.add(null);
        AbstractNodeHandler.this.validPeers.add(Lane.class);
        AbstractNodeHandler.this.validPeers.add(Variable.class);
        AbstractNodeHandler.this.validPeers.add(Node.class);
        AbstractNodeHandler.this.validPeers.add(SequenceFlow.class);
        AbstractNodeHandler.this.validPeers.add(Lane.class);
        AbstractNodeHandler.this.validPeers.add(Association.class);
    }
    ...
    protected void endNode(final StringBuilder xmlDump) {
        xmlDump.append(("/>" + (AbstractNodeHandler.EOL)));
    }

    protected void endNode(final String name, final StringBuilder xmlDump) {
        xmlDump.append(((("    </" + name) + ">") + (AbstractNodeHandler.EOL)));
    }
    ...
    protected void writeExtensionElements(Node node, final StringBuilder xmlDump) {
        if (containsExtensionElements(node)) {
            xmlDump.append(("      <extensionElements>" + (AbstractNodeHandler.EOL)));
    ...
    public static void writeScript(final DroolsAction action, String type, final StringBuilder xmlDump) {
        if (action instanceof DroolsConsequenceAction) {
            ...
            if (consequence != null) {
                xmlDump.append((((((">" + (AbstractNodeHandler.EOL)) + "          <tns:script>") + (XmlDumper.replaceIllegalChars(consequence.trim()))) + "</tns:script>") + (AbstractNodeHandler.EOL)));
                xmlDump.append(((("        </tns:" + type) + "-script>") + (AbstractNodeHandler.EOL)));
            } else {
                xmlDump.append(("/>" + (AbstractNodeHandler.EOL)));
            }
        } else {
    ...
    protected DataType getDataType(String itemSubjectRef, Map<String, ItemDefinition> itemDefinitions, ClassLoader cl) {
        ...
        if (itemDefinition != null) {
            ...
            if (("java.lang.Boolean".equals(structureRef)) || ("Boolean".equals(structureRef))) {
                ...
                dataType = new org.drools.core.process.core.datatype.impl.type.ObjectDataType(structureRef);
            } else {
    ...
    protected void handleThrowCompensationEventNode(final Node node, final Element element, final String uri, final String localName, final ExtensibleXmlParser parser) {
    ...
    protected void writeVariableName(EventNode eventNode, StringBuilder xmlDump) {
        if ((eventNode.getVariableName()) != null) {
            xmlDump.append(((("      <dataOutput id=\"" + (XmlBPMNProcessDumper.getUniqueNodeId(eventNode))) + "_Output\" name=\"event\" />") + (AbstractNodeHandler.EOL)));
            xmlDump.append(("      <dataOutputAssociation>" + (AbstractNodeHandler.EOL)));
            xmlDump.append(((((((("      <sourceRef>" + (XmlBPMNProcessDumper.getUniqueNodeId(eventNode))) + "_Output</sourceRef>") + (AbstractNodeHandler.EOL)) + "      <targetRef>") + (XmlDumper.replaceIllegalChars(eventNode.getVariableName()))) + "</targetRef>") + (AbstractNodeHandler.EOL)));
            xmlDump.append(("      </dataOutputAssociation>" + (AbstractNodeHandler.EOL)));
            xmlDump.append(("      <outputSet>" + (AbstractNodeHandler.EOL)));
            xmlDump.append(((("        <dataOutputRefs>" + (XmlBPMNProcessDumper.getUniqueNodeId(eventNode))) + "_Output</dataOutputRefs>") + (AbstractNodeHandler.EOL)));
            xmlDump.append(("      </outputSet>" + (AbstractNodeHandler.EOL)));
        } 
    }

    protected String getSignalExpression(NodeImpl node, String signalName, String variable) {
        String signalExpression = AbstractNodeHandler.RUNTIME_SIGNAL_EVENT;
        String scope = ((String) (node.getMetaData("customScope")));
        if ("processInstance".equalsIgnoreCase(scope)) {
            signalExpression = (((((AbstractNodeHandler.PROCESS_INSTANCE_SIGNAL_EVENT) + "org.jbpm.process.instance.impl.util.VariableUtil.resolveVariable(\"") + signalName) + "\", kcontext.getNodeInstance()), ") + (variable == null ? "null" : variable)) + ");";
        } else if (("runtimeManager".equalsIgnoreCase(scope)) || ("project".equalsIgnoreCase(scope))) {
            signalExpression = (((((AbstractNodeHandler.RUNTIME_MANAGER_SIGNAL_EVENT) + "org.jbpm.process.instance.impl.util.VariableUtil.resolveVariable(\"") + signalName) + "\", kcontext.getNodeInstance()), ") + (variable == null ? "null" : variable)) + ");";
        } else if ("external".equalsIgnoreCase(scope)) {
            signalExpression = (((((((((((((((((((((("org.drools.core.process.instance.impl.WorkItemImpl workItem = new org.drools.core.process.instance.impl.WorkItemImpl();" + (AbstractNodeHandler.EOL)) + "workItem.setName(\"External Send Task\");") + (AbstractNodeHandler.EOL)) + "workItem.setNodeInstanceId(kcontext.getNodeInstance().getId());") + (AbstractNodeHandler.EOL)) + "workItem.setProcessInstanceId(kcontext.getProcessInstance().getId());") + (AbstractNodeHandler.EOL)) + "workItem.setNodeId(kcontext.getNodeInstance().getNodeId());") + (AbstractNodeHandler.EOL)) + "workItem.setDeploymentId((String) kcontext.getKnowledgeRuntime().getEnvironment().get(\"deploymentId\"));") + (AbstractNodeHandler.EOL)) + "workItem.setParameter(\"Signal\", org.jbpm.process.instance.impl.util.VariableUtil.resolveVariable(\"") + signalName) + "\", kcontext.getNodeInstance()));") + (AbstractNodeHandler.EOL)) + "workItem.setParameter(\"SignalProcessInstanceId\", kcontext.getVariable(\"SignalProcessInstanceId\"));") + (AbstractNodeHandler.EOL)) + "workItem.setParameter(\"SignalWorkItemId\", kcontext.getVariable(\"SignalWorkItemId\"));") + (AbstractNodeHandler.EOL)) + "workItem.setParameter(\"SignalDeploymentId\", kcontext.getVariable(\"SignalDeploymentId\"));") + (AbstractNodeHandler.EOL)) + (variable == null ? "" : (("workItem.setParameter(\"Data\", " + variable) + ");") + (AbstractNodeHandler.EOL))) + "((org.drools.core.process.instance.WorkItemManager) kcontext.getKnowledgeRuntime().getWorkItemManager()).internalExecuteWorkItem(workItem);";
        } else {
    ...
    protected String checkSignalAndConvertToRealSignalNam(ExtensibleXmlParser parser, String signalName) {
        ...
        Set<String> signalNames = ((Set<String>) (buildData.getMetaData(AbstractNodeHandler.SIGNAL_NAMES)));
        if (signalNames == null) {
            signalNames = new HashSet<>();
            buildData.setMetaData(AbstractNodeHandler.SIGNAL_NAMES, signalNames);
        } 
