...
public class CallActivityHandler extends AbstractNodeHandler {
    ...
    protected void writeIO(SubProcessNode subProcessNode, StringBuilder xmlDump) {
        ...
        for (Map.Entry<String, String> entry : subProcessNode.getInMappings().entrySet()) {
            ...
            xmlDump.append(((((((((("        <sourceRef>" + (XmlDumper.replaceIllegalChars(entry.getValue()))) + "</sourceRef>") + (EOL)) + "        <targetRef>") + (XmlBPMNProcessDumper.getUniqueNodeId(subProcessNode))) + "_") + (XmlDumper.replaceIllegalChars(entry.getKey()))) + "Input</targetRef>") + (EOL)));
            xmlDump.append(("      </dataInputAssociation>" + (EOL)));
        }
        for (Map.Entry<String, String> entry : subProcessNode.getOutMappings().entrySet()) {
            xmlDump.append(("      <dataOutputAssociation>" + (EOL)));
            xmlDump.append(((((((((("        <sourceRef>" + (XmlBPMNProcessDumper.getUniqueNodeId(subProcessNode))) + "_") + (XmlDumper.replaceIllegalChars(entry.getKey()))) + "Output</sourceRef>") + (EOL)) + "        <targetRef>") + (entry.getValue())) + "</targetRef>") + (EOL)));
            xmlDump.append(("      </dataOutputAssociation>" + (EOL)));
        }
    }
}
