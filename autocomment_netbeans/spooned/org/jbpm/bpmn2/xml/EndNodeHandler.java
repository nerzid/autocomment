...
import EndNode.EVENT_NODE_ENTER;
...
public class EndNodeHandler extends AbstractNodeHandler {
    ...
    public void writeNode(Node node, StringBuilder xmlDump, int metaDataType) {
        ...
        // write node String{"endEvent"} to EndNodeHandler{}
        ...
        if (endNode.isTerminate()) {
            ...
            List<DroolsAction> actions = endNode.getActions(EVENT_NODE_ENTER);
                ...
                if ((actions.size()) == 1) {
                    ...
                    // else general: activityRef = "" (above)
                        ...
                        if ("signal".equals(eventType)) {
                            ...
                            if (s.startsWith(RUNTIME_SIGNAL_EVENT)) {
                                xmlDump.append((">" + (EOL)));
                                writeExtensionElements(endNode, xmlDump);
                                s = s.substring(44);
                                ...
                                if ((s.startsWith(PROCESS_INSTANCE_SIGNAL_EVENT)) && ("processInstance".equals(scope))) {
                                    xmlDump.append((">" + (EOL)));
                                    writeExtensionElements(endNode, xmlDump);
                                    s = s.substring(43);
                                    String type = s.substring(0, s.indexOf("\""));
                                    ...
                                    if (s.startsWith(PROCESS_INSTANCE_SIGNAL_EVENT)) {
                                        xmlDump.append((">" + (EOL)));
                                        writeExtensionElements(endNode, xmlDump);
