...
public class UserTaskHandler extends TaskHandler {
    ...
    protected void handleNode(final org.jbpm.workflow.core.Node node, final Element element, final String uri, final String localName, final ExtensibleXmlParser parser) throws SAXException {
    ...
    protected void writeIO(WorkItemNode workItemNode, StringBuilder xmlDump) {
        ...
        for (Map.Entry<String, String> entry : workItemNode.getInMappings().entrySet()) {
            ...
            xmlDump.append(((((((((("        <sourceRef>" + (XmlDumper.replaceIllegalChars(entry.getValue()))) + "</sourceRef>") + (EOL)) + "        <targetRef>") + (XmlBPMNProcessDumper.getUniqueNodeId(workItemNode))) + "_") + (XmlDumper.replaceIllegalChars(entry.getKey()))) + "Input</targetRef>") + (EOL)));
            xmlDump.append(("      </dataInputAssociation>" + (EOL)));
        }
        for (Map.Entry<String, Object> entry : workItemNode.getWork().getParameters().entrySet()) {
            if ((!("ActorId".equals(entry.getKey()))) && ((entry.getValue()) != null)) {
        ...
        for (Map.Entry<String, String> entry : workItemNode.getOutMappings().entrySet()) {
            ...
            xmlDump.append(((((((((("        <sourceRef>" + (XmlBPMNProcessDumper.getUniqueNodeId(workItemNode))) + "_") + (XmlDumper.replaceIllegalChars(entry.getKey()))) + "Output</sourceRef>") + (EOL)) + "        <targetRef>") + (XmlDumper.replaceIllegalChars(entry.getValue()))) + "</targetRef>") + (EOL)));
            xmlDump.append(("      </dataOutputAssociation>" + (EOL)));
        }
    }
}
