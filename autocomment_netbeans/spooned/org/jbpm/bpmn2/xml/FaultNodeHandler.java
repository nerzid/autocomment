...
public class FaultNodeHandler extends AbstractNodeHandler {
    ...
    public void writeNode(Node node, StringBuilder xmlDump, int metaDataType) {
        ...
        if ((faultNode.getFaultVariable()) != null) {
            ...
            xmlDump.append(((((((("        <sourceRef>" + (XmlDumper.replaceIllegalChars(faultNode.getFaultVariable()))) + "</sourceRef>") + (EOL)) + "        <targetRef>") + (XmlBPMNProcessDumper.getUniqueNodeId(faultNode))) + "_Input</targetRef>") + (EOL)));
            xmlDump.append(("      </dataInputAssociation>" + (EOL)));
