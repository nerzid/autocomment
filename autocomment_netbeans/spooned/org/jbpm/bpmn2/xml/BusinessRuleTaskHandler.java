...
public class BusinessRuleTaskHandler extends AbstractNodeHandler {
    ...
    protected void readDataInputAssociation(org.w3c.dom.Node xmlNode, RuleSetNode ruleSetNode, Map<String, String> dataInputs) {
        ...
        if ("sourceRef".equals(subNode.getNodeName())) {
            ...
            ruleSetNode.addInAssociation(new org.jbpm.workflow.core.node.DataAssociation(source, dataInputs.get(target), assignments, transformation));
        } else {
            // targetRef
    ...
    protected void readDataOutputAssociation(org.w3c.dom.Node xmlNode, RuleSetNode ruleSetNode, Map<String, String> dataOutputs) {
        ...
        ruleSetNode.addOutAssociation(new org.jbpm.workflow.core.node.DataAssociation(dataOutputs.get(source), target, assignments, transformation));
    }
    ...
    protected void writeIO(RuleSetNode ruleSetNode, StringBuilder xmlDump) {
        ...
        for (Map.Entry<String, String> entry : ruleSetNode.getInMappings().entrySet()) {
            ...
            xmlDump.append(((((((((("        <sourceRef>" + (XmlDumper.replaceIllegalChars(entry.getValue()))) + "</sourceRef>") + (EOL)) + "        <targetRef>") + (XmlBPMNProcessDumper.getUniqueNodeId(ruleSetNode))) + "_") + (XmlDumper.replaceIllegalChars(entry.getKey()))) + "Input</targetRef>") + (EOL)));
            xmlDump.append(("      </dataInputAssociation>" + (EOL)));
        }
        for (Map.Entry<String, Object> entry : ruleSetNode.getParameters().entrySet()) {
            if ((!("ActorId".equals(entry.getKey()))) && ((entry.getValue()) != null)) {
        ...
        for (Map.Entry<String, String> entry : ruleSetNode.getOutMappings().entrySet()) {
            ...
            xmlDump.append(((((((((("        <sourceRef>" + (XmlBPMNProcessDumper.getUniqueNodeId(ruleSetNode))) + "_") + (XmlDumper.replaceIllegalChars(entry.getKey()))) + "Output</sourceRef>") + (EOL)) + "        <targetRef>") + (XmlDumper.replaceIllegalChars(entry.getValue()))) + "</targetRef>") + (EOL)));
            xmlDump.append(("      </dataOutputAssociation>" + (EOL)));
        }
    }
}
