...
public class BPMN2XMLTest extends XMLTestCase {
    ...
    public void testXML() throws IOException, SAXException {
        ...
        modules.addSemanticModule(new org.jbpm.bpmn2.xml.BPMNSemanticModule());
        modules.addSemanticModule(new org.jbpm.bpmn2.xml.BPMNDISemanticModule());
        XmlProcessReader processReader = new XmlProcessReader(modules, getClass().getClassLoader());
        for (String processName : BPMN2XMLTest.processes) {
            ...
            diff.overrideDifferenceListener(new DifferenceListener() {
                public int differenceFound(Difference diff) {
                    ...
                    BPMN2XMLTest.logger.info("! {}", diff.getTestNodeDetail().getNode().getNodeName());
                    return RETURN_ACCEPT_DIFFERENCE;
                }

                public void skippedComparison(Node one, Node two) {
                    BPMN2XMLTest.logger.info("{} : {}", one.getLocalName(), two.getLocalName());
                }
            });
            // nodes should only be compared if their attributes are the same
            diff.overrideElementQualifier(new org.custommonkey.xmlunit.ElementNameAndAttributeQualifier());
