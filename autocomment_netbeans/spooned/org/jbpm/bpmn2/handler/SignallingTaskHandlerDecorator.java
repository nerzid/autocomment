...
public class SignallingTaskHandlerDecorator extends AbstractExceptionHandlingTaskHandler {
    ...
    private final Map<Long, Integer> processInstanceExceptionMap = new HashMap<Long, Integer>();
    ...
    public void setWorkItemExceptionParameterName(String parameterName) {
        SignallingTaskHandlerDecorator.this.workItemExceptionParameterName = parameterName;
    }

    public String getWorkItemExceptionParameterName() {
        return SignallingTaskHandlerDecorator.this.workItemExceptionParameterName;
    }
    ...
    public void handleExecuteException(Throwable cause, WorkItem workItem, WorkItemManager manager) {
        if ((getAndIncreaseExceptionCount(workItem.getProcessInstanceId())) < (exceptionCountLimit)) {
            workItem.getParameters().put(SignallingTaskHandlerDecorator.this.workItemExceptionParameterName, cause);
            ((org.drools.core.process.instance.WorkItemManager) (manager)).signalEvent(SignallingTaskHandlerDecorator.this.eventType, ((org.drools.core.process.instance.WorkItem) (workItem)), workItem.getProcessInstanceId());
        } else {
            if (cause instanceof RuntimeException) {
                throw ((RuntimeException) (cause));
            } else {
                throw new WorkItemHandlerRuntimeException(cause, (((("Signalling process instance " + (workItem.getProcessInstanceId())) + " with '") + (SignallingTaskHandlerDecorator.this.eventType)) + "' resulted this exception."));
            }
        }
    }
    ...
    public void setExceptionCountLimit(int limit) {
        SignallingTaskHandlerDecorator.this.exceptionCountLimit = limit;
    }
