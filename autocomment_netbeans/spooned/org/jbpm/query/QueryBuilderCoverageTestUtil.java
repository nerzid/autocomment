...
public abstract class QueryBuilderCoverageTestUtil {
    ...
    public static EntityManagerFactory beforeClass(String persistenceUnit) {
        ...
        QueryBuilderCoverageTestUtil.context = PersistenceUtil.setupWithPoolingDataSource(persistenceUnit);
        QueryBuilderCoverageTestUtil.emf = ((EntityManagerFactory) (QueryBuilderCoverageTestUtil.context.get(EnvironmentName.ENTITY_MANAGER_FACTORY)));
        return QueryBuilderCoverageTestUtil.emf;
    }

    public static void afterClass() {
        PersistenceUtil.cleanUp(QueryBuilderCoverageTestUtil.context);
    }
    ...
    public static void queryBuilderCoverageTest(ExtendedParametrizedQueryBuilder queryBuilder, Class builderClass, QueryBuilderCoverageTestUtil.ModuleSpecificInputFiller inputFiller, String... skipMethodName) {
        ...
        for (Method methodA : queryMethods) {
            for (Method methodB : queryMethods) {
                ...
                try {
                    ...
                    QueryBuilderCoverageTestUtil.logger.debug(testName.toString());
                    // try queryT
    ...
    private static void callMethod(Method method, Object obj, Object input) {
        ...
        try {
            ...
            e.printStackTrace();
            Assert.fail((((("Method [" + (method.getName())) + ".(") + (QueryBuilderCoverageTestUtil.getInputAsString(input))) + ")]"));
        }
    }
    ...
    private static Object[] fillInput(Type[] types, QueryBuilderCoverageTestUtil.ModuleSpecificInputFiller inputFiller) {
        ...
        for (int i = 0; i < (types.length); ++i) {
            ...
            if (type.equals(int.class)) {
                ...
                if (arrayType.equals(int.class)) {
                    ...
                    long[] longArr = new long[]{ 1 , 9 , 25 };
                    result[i] = longArr;
                } else if (arrayType.equals(String.class)) {
                    String[] strArr = new String[]{ "blu" , "red" , "gro" };
                    result[i] = strArr;
                } else if (arrayType.equals(Date.class)) {
                    Date[] strArr = new Date[]{ new Date() , new Date() };
                    result[i] = strArr;
                } 
            } 
            if ((result[i]) == null) {
