...
public abstract class AbstractQuerySerializationTest {
    ...
    public void queryCriteriaTest() throws Exception {
        ...
        Assert.assertEquals("union", criteria.isUnion(), copyCrit.isUnion());
        Assert.assertEquals("parameter type", criteria.getType(), copyCrit.getType());
        Assert.assertEquals("value list size", criteria.getParameters().size(), copyCrit.getParameters().size());
        for (int i = 0; i < (criteria.getParameters().size()); ++i) {
            Assert.assertEquals(("value " + i), criteria.getParameters().get(i), copyCrit.getParameters().get(i));
        }
        ...
        Assert.assertEquals("union", queryWhere.isUnion(), copyWhere.isUnion());
        for (int i = 0; i < (queryWhere.getCriteria().size()); ++i) {
            ...
            for (int j = 0; j < (origCrit.getParameters().size()); ++j) {
                Assert.assertEquals(("value " + i), origCrit.getParameters().get(i), copyCrit.getParameters().get(i));
            }
        }
    }
