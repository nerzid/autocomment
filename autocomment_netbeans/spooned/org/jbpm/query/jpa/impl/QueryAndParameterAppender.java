...
public class QueryAndParameterAppender {
    ...
    public QueryAndParameterAppender(StringBuilder queryBuilder, Map<String, Object> params) {
        ...
        QueryAndParameterAppender.this.noWhereClauseYet = !(queryBuilder.toString().contains("WHERE"));
    }

    public boolean hasBeenUsed() {
        return !(QueryAndParameterAppender.this.noClauseAddedYet);
    }

    public void markAsUsed() {
        QueryAndParameterAppender.this.noClauseAddedYet = false;
    }
    ...
    public <T> void addRangeQueryParameters(List<? extends Object> paramList, String listId, Class<T> type, String fieldName, String joinClause, boolean union) {
        ...
        for (Map.Entry<String, T> nameMinMaxEntry : paramNameMinMaxMap.entrySet()) {
    ...
    public void addRegexQueryParameters(List<String> paramValList, String listId, String fieldName, String joinClause, boolean union) {
        ...
        // add query string to query builder and fill params map
        internalAddToQueryBuilder(queryClause.toString(), union);
        for (Map.Entry<String, String> nameRegexEntry : paramNameRegexMap.entrySet()) {
    ...
    private void internalAddToQueryBuilder(String query, boolean union) {
        if (QueryAndParameterAppender.this.noClauseAddedYet) {
            if (noWhereClauseYet) {
            ...
            QueryAndParameterAppender.this.noClauseAddedYet = false;
        } else if (QueryAndParameterAppender.this.alreadyUsed) {
    ...
    public void queryBuilderModificationCleanup() {
        QueryAndParameterAppender.this.alreadyUsed = true;
    }
    ...
    private <T> List<T> checkAndConvertListToType(List<?> inputList, Object inputObject, String listId, Class<T> type) {
        if (QueryAndParameterAppender.logger.isDebugEnabled()) {
    ...
    public static void debugQueryParametersIdentifiers() {
        try {
            ...
            for (Map.Entry<String, String> entry : fieldValueMap.entrySet()) {
                QueryAndParameterAppender.logger.debug(String.format("%-12s : %s", entry.getKey(), entry.getValue()));
            }
        } catch (Exception e) {
            // ignore
        }
