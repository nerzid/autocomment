...
public class EMigrateProcessTest extends AbstractRuntimeEJBServicesTest {
    ...
    public void deployKieJar() {
        ...
        if ((EMigrateProcessTest.kieJar2) == null) {
            EMigrateProcessTest.kieJar2 = archive.deployMigrationV2KieJar().getIdentifier();
        } 
    }
    ...
    public void cleanup() {
        super.cleanup();
        EMigrateProcessTest.kieJar2 = null;
    }
    ...
    public void testUpgradeProcessInstance() throws Exception {
        ...
        List<TaskSummary> tasks = runtimeDataService.getTasksAssignedAsPotentialOwner(EMigrateProcessTest.USER_YODA, new org.kie.internal.query.QueryFilter(0, 10));
        ...
        MigrationReport report = migrationService.migrate(kieJar, processInstanceId, EMigrateProcessTest.kieJar2, EVALUATION_PROCESS_ID_V2);
        Assertions.assertThat(report).isNotNull();
        Assertions.assertThat(report.isSuccessful()).isTrue();
        // it stays in the same task
        tasks = runtimeDataService.getTasksAssignedAsPotentialOwner(EMigrateProcessTest.USER_YODA, new org.kie.internal.query.QueryFilter(0, 10));
        Assertions.assertThat(tasks.size()).isEqualTo(1);
        task = tasks.get(0);
        Assertions.assertThat(task.getName()).isEqualTo("Evaluate items");
        Assertions.assertThat(task.getDeploymentId()).isEqualTo(EMigrateProcessTest.kieJar2);
        Assertions.assertThat(task.getProcessId()).isEqualTo(EVALUATION_PROCESS_ID_V2);
        Assertions.assertThat(task.getProcessInstanceId()).isEqualTo(processInstanceId);
        userTaskService.completeAutoProgress(task.getId(), EMigrateProcessTest.USER_YODA, null);
    ...
    public void testUpgradeProcessInstanceWithNodeMapping() throws Exception {
        ...
        // migrate process instance to evaluation 2 in container 2
        MigrationReport report = migrationService.migrate(kieJar, processInstanceId, EMigrateProcessTest.kieJar2, EVALUATION_PROCESS_ID_V2, nodeMapping);
        Assertions.assertThat(report).isNotNull();
        Assertions.assertThat(report.isSuccessful()).isTrue();
    ...
    public void testUpgradeProcessInstances() throws Exception {
        ...
        // migrate process instance to evaluation 2 in container 2
        List<MigrationReport> reports = migrationService.migrate(kieJar, ids, EMigrateProcessTest.kieJar2, EVALUATION_PROCESS_ID_V2);
    ...
    public void testUpgradeProcessInstancesWithNodeMapping() throws Exception {
        ...
        nodeMapping.put("_4E8E7545-FB70-494E-9136-2B9ABE655889", "_56FB3E50-DEDD-415B-94DD-0357C91836B9");
        // migrate process instance to evaluation 2 in container 2
        List<MigrationReport> reports = migrationService.migrate(kieJar, ids, EMigrateProcessTest.kieJar2, EVALUATION_PROCESS_ID_V2, nodeMapping);
        Assertions.assertThat(reports).isNotNull();
        for (MigrationReport report : reports) {
            Assertions.assertThat(report.isSuccessful()).isTrue();
        }
