...
public class ERestServiceTaskTest extends AbstractRuntimeEJBServicesTest {
    ...
    public void testRestWorkItem() throws Exception {
        ...
        Collection<VariableDesc> result = runtimeDataService.getVariableHistory(pid, "result", new org.kie.internal.query.QueryContext());
        Assertions.assertThat(result).hasSize(1);
        Assertions.assertThat(result.iterator().next().getNewValue()).isEqualTo("pong");
        Collection<VariableDesc> status = runtimeDataService.getVariableHistory(pid, "status", new org.kie.internal.query.QueryContext());
        Assertions.assertThat(status).hasSize(1);
        Assertions.assertThat(status.iterator().next().getNewValue()).isEqualTo("200");
        Collection<VariableDesc> statusMsg = runtimeDataService.getVariableHistory(pid, "statusMsg", new org.kie.internal.query.QueryContext());
