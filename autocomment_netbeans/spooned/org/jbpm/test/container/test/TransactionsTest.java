 ...
 */
@Category(value = { EAP.class , WLS.class , WAS.class , EWS.class })
public class TransactionsTest extends JbpmContainerTest {
    ...
    static {
        TransactionsTest.RESOURCES.add(TransactionsTest.lt.getResource(LocalTransactions.BPMN_TRANSACTIONS));
        TransactionsTest.RESOURCES.add(TransactionsTest.lt.getResource(LocalTransactions.BPMN_HELLO_WORLD));
        TransactionsTest.RESOURCES.add(TransactionsTest.lt.getResource(LocalTransactions.RULES_TRANSACTIONS));
    }
    ...
    public static Archive<?> deployLocalTransactions() {
        WebArchive war = TransactionsTest.lt.buildArchive();
    ...
    public void testStartProcessCommit() throws Exception {
        ...
        UserTransaction ut = TransactionsTest.getUserTransaction();
        ut.begin();
    ...
    public void testStartProcessRollback() throws Exception {
        ...
        try {
            ...
            TransactionsTest.LOGGER.error("Non-XA database thrown NPE on process started before rollback", npe);
        }
    }
    ...
    public void testScript() throws Exception {
        ...
        ksession = reloadSession(ksession);
        ksession.addEventListener(process);
        ut = TransactionsTest.getUserTransaction();
        ut.begin();
        ksession.signalEvent("start", "script", processId);
        Thread.sleep(1000);
    ...
    private KieBase getKnowledgeBase() {
        return getKieBase(KieUtils.newKieBuilder(TransactionsTest.RESOURCES));
    }
