...
public class KieUtils {
    ...
    public static final KieBuilder newKieBuilder(Collection<Resource> resources) {
        return KieUtils.buildResources(resources.toArray(new Resource[resources.size()]));
    }
    ...
    public static final KieBuilder newKieBuilder(Resource... resources) {
        return KieUtils.buildResources(resources);
    }
    ...
    public static final KieBuilder newKieBuilder(boolean assertNoErrors, Resource... resources) {
        return KieUtils.buildResources(assertNoErrors, resources);
    }
    ...
    public static final KieBuilder newCEPKieBuilder(Resource... resources) {
        return KieUtils.buildCEPResources(resources);
    }
    ...
    public static final KieBuilder newCEPKieBuilder(boolean assertNoErrors, Resource... resources) {
        return KieUtils.buildCEPResources(assertNoErrors, resources);
    }

    public static ReleaseId buildKieModule(Collection<Resource> resources) {
        return KieUtils.buildKieModule(resources.toArray(new Resource[resources.size()]));
    }
    ...
    public static ReleaseId buildKieModule(Resource... resources) {
        ReleaseId id = KieUtils.generateReleaseId();
        KieUtils.buildKieModule(id, resources);
        return id;
    }
    ...
    public static void buildKieModule(ReleaseId id, Resource... resources) {
        ...
        KieModuleModel module = ks.newKieModuleModel();
        {
            ...
            final String name = KieUtils.KBASE_DEFAULT;
            ...
            base.newKieSessionModel(KieUtils.getSessionName(name, KieUtils.KSESSION_STATELESS)).setDefault(true).setType(KieSessionType.STATELESS);
            base.newKieSessionModel(KieUtils.getSessionName(name, KieUtils.KSESSION_DEFAULT)).setDefault(true).setType(KieSessionType.STATEFUL);
            base.newKieSessionModel(KieUtils.getSessionName(name, KieUtils.KSESSION_PSEUDO)).setType(KieSessionType.STATEFUL).setClockType(ClockTypeOption.get("pseudo"));
        }
        {
            // kie base and sessions working in CLOUD mode
            final String name = KieUtils.KBASE_CLOUD;
            KieBaseModel base = module.newKieBaseModel(name);
            base.addInclude(KieUtils.KBASE_DEFAULT);// include resources from default
            
            // KieBase
            base.setEventProcessingMode(EventProcessingOption.CLOUD);
            base.newKieSessionModel(KieUtils.getSessionName(name, KieUtils.KSESSION_STATELESS)).setType(KieSessionType.STATELESS);
            base.newKieSessionModel(KieUtils.getSessionName(name, KieUtils.KSESSION_DEFAULT)).setType(KieSessionType.STATEFUL);
            base.newKieSessionModel(KieUtils.getSessionName(name, KieUtils.KSESSION_PSEUDO)).setType(KieSessionType.STATEFUL).setClockType(ClockTypeOption.get("pseudo"));
        }
        {
            // kie base and sessions with equality behavior and STREAM mode
            final String name = KieUtils.KBASE_EQUALITY;
            KieBaseModel base = module.newKieBaseModel(name);
            base.addInclude(KieUtils.KBASE_DEFAULT);// include resources from default
        ...
        KieUtils.LOGGER.debug("kmodule.xml: {}", module.toXML());
        kfs.writeKModuleXML(module.toXML());
        kfs.generateAndWritePomXML(id);
        KieBuilder kbuilder = KieUtils.buildResources(kfs, resources);
    ...
    private static KieBuilder buildCEPResources(Resource... resources) {
        return KieUtils.buildResources(KieUtils.createDefaultKieFileSystemForCEP(), resources);
    }
    ...
    private static KieBuilder buildCEPResources(boolean assertNoErrors, Resource... resources) {
        return KieUtils.buildResources(KieUtils.createDefaultKieFileSystemForCEP(), assertNoErrors, resources);
    }

    private static KieFileSystem createDefaultKieFileSystemForCEP() {
        KieServices ks = KieServices.Factory.get();
        KieModuleModel module = ks.newKieModuleModel();
        KieBaseModel base = module.newKieBaseModel(KieUtils.KBASE_DEFAULT);
        base.setDefault(true);
        base.addPackage("*");
        base.setEventProcessingMode(EventProcessingOption.STREAM);
        KieFileSystem kfs = ks.newKieFileSystem();
    ...
    private static KieBuilder buildResources(Resource... resources) {
        return KieUtils.buildResources(KieServices.Factory.get().newKieFileSystem(), true, resources);
    }

    private static KieBuilder buildResources(boolean assertNoErrors, Resource... resources) {
        return KieUtils.buildResources(KieServices.Factory.get().newKieFileSystem(), assertNoErrors, resources);
    }

    private static KieBuilder buildResources(KieFileSystem kfs, Resource... resources) {
        return KieUtils.buildResources(kfs, true, resources);
    }
    ...
    private static KieBuilder buildResources(KieFileSystem kfs, boolean assertNoErrors, Resource... resources) {
        ...
        for (Resource res : resources) {
            if (((res.getSourcePath()) == null) && ((res.getTargetPath()) == null)) {
                res.setTargetPath(String.format("/org/jboss/qa/brms/asset%s.%s", (counter++), KieUtils.getAssetExtension(res)));
            } 
            kfs.write(res);
        }
        ...
        if ((msgs.size()) > 0) {
            KieUtils.LOGGER.info("KieBuilder information: {}", msgs.toString());
        } 
        msgs = kbuilder.getResults().getMessages(Level.WARNING);
        if ((msgs.size()) > 0) {
            KieUtils.LOGGER.warn("KieBuilder warnings: {}", msgs.toString());
        } 
        msgs = kbuilder.getResults().getMessages(Level.ERROR);
        if ((msgs.size()) > 0) {
            KieUtils.LOGGER.error("KieBuilder errors: {}", msgs.toString());
        } 
        if (assertNoErrors) {
    ...
    private static String getAssetExtension(Resource res) {
        ...
        if (type == null) {
            KieUtils.LOGGER.debug("Resource without ResourceType encountered: {}", res);
            return "drl";
        } else {
