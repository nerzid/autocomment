...
public class ExecutorTestUtil {
    ...
    private static Properties getDatasourceProperties() {
        ...
        InputStream propsInputStream = ExecutorTestUtil.class.getResourceAsStream(ExecutorTestUtil.DATASOURCE_PROPERTIES);
        ...
        if (propsInputStream != null) {
            try {
                ...
                ExecutorTestUtil.logger.warn(("Unable to find properties, using default H2 properties: " + (ioe.getMessage())));
                ioe.printStackTrace();
            }
        } else {
            propertiesNotFound = true;
        ...
        if (("${maven.jdbc.password}".equals(password)) || propertiesNotFound) {
            ExecutorTestUtil.logger.warn((("Unable to load datasource properties [" + (ExecutorTestUtil.DATASOURCE_PROPERTIES)) + "]"));
        } 
    ...
    private static void setDefaultProperties(Properties props) {
        String[] keyArr = new String[]{ "serverName" , "portNumber" , "databaseName" , ExecutorTestUtil.JDBC_URL , ExecutorTestUtil.USER , ExecutorTestUtil.PASSWORD , ExecutorTestUtil.DRIVER_CLASS_NAME , ExecutorTestUtil.DATASOURCE_CLASS_NAME , ExecutorTestUtil.MAX_POOL_SIZE , ExecutorTestUtil.ALLOW_LOCAL_TXS };
    ...
    public static void cleanupSingletonSessionId() {
        ...
        if (tempDir.exists()) {
            ...
            for (String file : jbpmSerFiles) {
                ExecutorTestUtil.logger.debug("Temp dir to be removed {} file {}", tempDir, file);
