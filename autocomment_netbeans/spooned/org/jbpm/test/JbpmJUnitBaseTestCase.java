...
SINGLETON, REQUEST, PROCESS_INSTANCE;    }
    ...
    private JbpmJUnitBaseTestCase.TestWorkItemHandler workItemHandler = new JbpmJUnitBaseTestCase.TestWorkItemHandler();
    ...
    protected UserGroupCallback userGroupCallback = new org.jbpm.services.task.identity.JBossUserGroupCallbackImpl("classpath:/usergroups.properties");
    ...
    public JbpmJUnitBaseTestCase(boolean setupDataSource, boolean sessionPersistence, String persistenceUnitName) {
        JbpmJUnitBaseTestCase.this.setupDataSource = setupDataSource;
        JbpmJUnitBaseTestCase.this.sessionPersistence = sessionPersistence;
        JbpmJUnitBaseTestCase.this.persistenceUnitName = persistenceUnitName;
        if ((!(JbpmJUnitBaseTestCase.this.setupDataSource)) && (JbpmJUnitBaseTestCase.this.sessionPersistence)) {
        ...
        JbpmJUnitBaseTestCase.logger.debug("Configuring entire test case to have data source enabled {} and session persistence enabled {} with persistence unit name {}", JbpmJUnitBaseTestCase.this.setupDataSource, JbpmJUnitBaseTestCase.this.sessionPersistence, JbpmJUnitBaseTestCase.this.persistenceUnitName);
    }
    ...
    public void setUp() throws Exception {
        if (setupDataSource) {
            ...
            JbpmJUnitBaseTestCase.logger.debug("Data source configured with unique id {}", ds.getUniqueName());
    ...
    protected RuntimeManager createRuntimeManager(String... process) {
        return createRuntimeManager(JbpmJUnitBaseTestCase.Strategy.SINGLETON, null, process);
    }
    ...
    protected RuntimeManager createRuntimeManager(Map<String, ResourceType> resources) {
        return createRuntimeManager(JbpmJUnitBaseTestCase.Strategy.SINGLETON, resources, null);
    }
    ...
    protected RuntimeManager createRuntimeManager(Map<String, ResourceType> resources, String identifier) {
        return createRuntimeManager(JbpmJUnitBaseTestCase.Strategy.SINGLETON, resources, identifier);
    }
        ...
        if (!(activeEngines.isEmpty())) {
            for (RuntimeEngine engine : activeEngines) {
                try {
                    ...
                    JbpmJUnitBaseTestCase.logger.debug("Exception during dipose of runtime engine, might be already disposed - {}", e.getMessage());
                }
            }
            activeEngines.clear();
        } 
    ...
    ****************************************
    ...
    protected EntityManagerFactory getEmf() {
        return JbpmJUnitBaseTestCase.this.emf;
    }

    protected DataSource getDs() {
        return JbpmJUnitBaseTestCase.this.ds;
    }
    ...
    protected PoolingDataSource setupPoolingDataSource() {
        ...
        try {
            ...
            JbpmJUnitBaseTestCase.logger.warn((("DBPOOL_MGR:Looks like there is an issue with creating db pool because of " + (e.getMessage())) + " cleaing up..."));
            ...
            for (String resource : resources) {
                ...
                JbpmJUnitBaseTestCase.logger.debug(("DBPOOL_MGR:Removed resource " + resource));
            }
            JbpmJUnitBaseTestCase.logger.debug("DBPOOL_MGR: attempting to create db pool again...");
            ...
            pds.getDriverProperties().put("driverClassName", "org.h2.Driver");
            pds.init();
            JbpmJUnitBaseTestCase.logger.debug("DBPOOL_MGR:Pool created after cleanup of leftover resources");
        }
        return pds;
    }
    ...
    protected void clearCustomRegistry() {
        JbpmJUnitBaseTestCase.this.customAgendaListeners.clear();
        JbpmJUnitBaseTestCase.this.customHandlers.clear();
        JbpmJUnitBaseTestCase.this.customProcessListeners.clear();
        JbpmJUnitBaseTestCase.this.customTaskListeners.clear();
    }
    ...
    protected static class TestWorkItemHandler implements WorkItemHandler {
        ...
        public WorkItem getWorkItem() {
            ...
            if ((workItems.size()) == 1) {
                ...
                JbpmJUnitBaseTestCase.TestWorkItemHandler.this.workItems.clear();
                return result;
            } else {
