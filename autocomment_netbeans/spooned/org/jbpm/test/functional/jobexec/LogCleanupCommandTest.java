...
public class LogCleanupCommandTest extends JbpmAsyncJobTestCase {
    ...
    private String dateFormat = LogCleanupCommandTest.DATE_FORMAT;
    ...
    public void skipProcessLog() throws Exception {
        ...
        // Generate data
        KieSession kieSession = createKSession(LogCleanupCommandTest.HELLO_WORLD);
        startProcess(kieSession, LogCleanupCommandTest.HELLO_WORLD_ID, 1);
        // Verify presence of data
        Assertions.assertThat(getProcessLogSize(LogCleanupCommandTest.HELLO_WORLD_ID)).isEqualTo(1);
        Assertions.assertThat(getNodeInstanceLogSize(LogCleanupCommandTest.HELLO_WORLD_ID)).isPositive();
        // Schedule cleanup job
        scheduleLogCleanup(true, false, false, getTomorrow(), null, LogCleanupCommandTest.HELLO_WORLD_ID);
    ...
    public void skipTaskLog() throws Exception {
        ...
        try {
            ...
            ((ExecutorServiceImpl) (getExecutorService())).addAsyncJobListener(countDownListener);
            // Generate data
            kieSession = createKSession(LogCleanupCommandTest.LOG_CLEANUP);
            Map<String, Object> paramMap = new HashMap<String, Object>();
            paramMap.put("assigneeName", "krisv");
            processInstanceList = startProcess(kieSession, LogCleanupCommandTest.LOG_CLEANUP_ID, paramMap, 1);
            ...
            // Verify presence of data
            Assertions.assertThat(getProcessLogSize(LogCleanupCommandTest.LOG_CLEANUP_ID)).isEqualTo(1);
            Assertions.assertThat(getTaskLogSize(LogCleanupCommandTest.LOG_CLEANUP_ID)).isEqualTo(1);
            Assertions.assertThat(getNodeInstanceLogSize(LogCleanupCommandTest.LOG_CLEANUP_ID)).isPositive();
            Assertions.assertThat(getVariableLogSize(LogCleanupCommandTest.LOG_CLEANUP_ID)).isEqualTo(3);
            // Schedule cleanup job
            scheduleLogCleanup(false, true, false, getTomorrow(), null, LogCleanupCommandTest.LOG_CLEANUP_ID);
            countDownListener.waitTillCompleted();
            // Verify absence of data
            Assertions.assertThat(getProcessLogSize(LogCleanupCommandTest.LOG_CLEANUP_ID)).isZero();
            Assertions.assertThat(getTaskLogSize(LogCleanupCommandTest.LOG_CLEANUP_ID)).isEqualTo(1);
            Assertions.assertThat(getNodeInstanceLogSize(LogCleanupCommandTest.LOG_CLEANUP_ID)).isZero();
            Assertions.assertThat(getVariableLogSize(LogCleanupCommandTest.LOG_CLEANUP_ID)).isZero();
        } finally {
    ...
    private void deleteAllLogsOlderThan(Date date, CountDownAsyncJobListener countDownListener) throws Exception {
        deleteAllLogsOlderThan(date, LogCleanupCommandTest.HELLO_WORLD_ID, LogCleanupCommandTest.HELLO_WORLD_ID, countDownListener);
    }
    ...
    public void deleteAllLogsOlderThanYesterday() throws Exception {
        ...
        Assertions.assertThat(getProcessLogSize(LogCleanupCommandTest.HELLO_WORLD_ID)).isPositive();
        Assertions.assertThat(getNodeInstanceLogSize(LogCleanupCommandTest.HELLO_WORLD_ID)).isPositive();
    }
    ...
    public void deleteAllLogsOlderThanNow() throws Exception {
        ...
        Assertions.assertThat(getProcessLogSize(LogCleanupCommandTest.HELLO_WORLD_ID)).isZero();
        Assertions.assertThat(getNodeInstanceLogSize(LogCleanupCommandTest.HELLO_WORLD_ID)).isZero();
    }
    ...
    public void deleteAllLogsOlderThanTomorrowDifferentProcess() throws Exception {
        CountDownAsyncJobListener countDownListener = new CountDownAsyncJobListener(1);
        ((ExecutorServiceImpl) (getExecutorService())).addAsyncJobListener(countDownListener);
        deleteAllLogsOlderThan(getTomorrow(), LogCleanupCommandTest.HELLO_WORLD_ID, LogCleanupCommandTest.ANOTHER_PROCESS, countDownListener);
        Assertions.assertThat(getProcessLogSize(LogCleanupCommandTest.HELLO_WORLD_ID)).isPositive();
        Assertions.assertThat(getNodeInstanceLogSize(LogCleanupCommandTest.HELLO_WORLD_ID)).isPositive();
    }
    ...
    public void deleteAllLogsOlderThanPeriod() throws Exception {
        ...
        // Generate data
        KieSession kieSession = createKSession(LogCleanupCommandTest.HELLO_WORLD);
        startProcess(kieSession, LogCleanupCommandTest.HELLO_WORLD_ID, 3);
        countDownListener.waitTillCompleted();
        startProcess(kieSession, LogCleanupCommandTest.HELLO_WORLD_ID, 2);
        // Verify presence of data
        Assertions.assertThat(getProcessLogSize(LogCleanupCommandTest.HELLO_WORLD_ID)).isEqualTo(5);
        Assertions.assertThat(getNodeInstanceLogSize(LogCleanupCommandTest.HELLO_WORLD_ID)).isPositive();
        // Schedule cleanup job
        scheduleLogCleanup(false, false, false, null, "8s", LogCleanupCommandTest.HELLO_WORLD_ID);
        countDownListener.waitTillCompleted();
        // Verify absence of data
        Assertions.assertThat(getProcessLogSize(LogCleanupCommandTest.HELLO_WORLD_ID)).isEqualTo(2);
    }

    // ------------------------ Helper Methods ------------------------
    private int getProcessLogSize(String processId) {
        return auditLogService.processInstanceLogQuery().processId(processId).build().getResultList().size();
    }

    private int getTaskLogSize(String processId) {
        return taskAuditService.auditTaskQuery().processId(processId).build().getResultList().size();
    }

    private int getNodeInstanceLogSize(String processId) {
        return auditLogService.nodeInstanceLogQuery().processId(processId).build().getResultList().size();
    }

    private int getVariableLogSize(String processId) {
        return auditLogService.variableInstanceLogQuery().processId(processId).build().getResultList().size();
    }
