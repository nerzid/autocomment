...
public class MultipleRuntimeManagerTest extends JbpmTestCase {
    ...
    public void testCreationOfRuntimeManagersConcurrently() throws Exception {
        ...
        MultipleRuntimeManagerTest.HumanTaskResolver htr1 = new MultipleRuntimeManagerTest.HumanTaskResolver(1, managers);
        MultipleRuntimeManagerTest.HumanTaskResolver htr2 = new MultipleRuntimeManagerTest.HumanTaskResolver(2, managers);
    ...
    class HumanTaskResolver implements Runnable {
        ...
        public HumanTaskResolver(long pid, List<RuntimeManager> managers) {
            this.pid = pid;
            MultipleRuntimeManagerTest.HumanTaskResolver.this.managers = managers;
        }
        ...
        private RuntimeManager getRuntimeManager(String process) {
            ...
            RuntimeEnvironment environment = RuntimeEnvironmentBuilder.getDefault().persistence(false).entityManagerFactory(getEmf()).userGroupCallback(userGroupCallback).knowledgeBase(kbuilder.newKnowledgeBase()).registerableItemsFactory(new DefaultRegisterableItemsFactory() {
                @Override
                public List<TaskLifeCycleEventListener> getTaskListeners() {
                    List<TaskLifeCycleEventListener> defaultListeners = new ArrayList<TaskLifeCycleEventListener>();
                    defaultListeners.add(new org.jbpm.services.task.audit.JPATaskLifeCycleEventListener(false));
