...
public class ProcessPersistenceHumanTaskTest extends JbpmTestCase {
    ...
    public void testProcess() throws Exception {
        ...
        ProcessPersistenceHumanTaskTest.logger.debug("Reloading the environemnt to simulate system restart");
        ...
        ProcessPersistenceHumanTaskTest.logger.debug(("John is executing task " + (task.getName())));
        taskService.start(task.getId(), "john");
        taskService.complete(task.getId(), "john", null);
        assertNodeTriggered(processInstance.getId(), "Task 2");
        // simulating a system restart
        ProcessPersistenceHumanTaskTest.logger.debug("Reloading the environemnt to simulate system restart once again");
        disposeRuntimeManager();
        createRuntimeManager("org/jbpm/test/functional/task/humantask.bpmn");
        runtimeEngine = getRuntimeEngine();
        ksession = runtimeEngine.getKieSession();
        ...
        task = list.get(0);
        ProcessPersistenceHumanTaskTest.logger.debug(("Mary is executing task " + (task.getName())));
