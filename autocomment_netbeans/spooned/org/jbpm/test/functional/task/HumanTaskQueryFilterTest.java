...
public class HumanTaskQueryFilterTest extends JbpmTestCase {
    ...
    public void setUp() throws Exception {
        super.setUp();
        createRuntimeManager(HumanTaskQueryFilterTest.CONF_HUMAN_TASK);
    ...
    public void testFirstResult() {
        ...
        List<TaskSummary> taskList = taskService.getTasksAssignedAsPotentialOwner("john", null, null, new QueryFilter(2, 2, "t.name", true));
    ...
    public void testMaxResults() {
        ...
        List<TaskSummary> taskList = taskService.getTasksAssignedAsPotentialOwner("john", null, null, new QueryFilter(0, 2, "t.id", true));
        Assertions.assertThat(taskList).hasSize(2);
        logger.debug(("### Potential owner task list: " + taskList));
    ...
    public void testAscendingOrder() {
        startHumanTaskProcess(3, "john's task", "john");
        List<TaskSummary> taskList = taskService.getTasksAssignedAsPotentialOwner("john", null, null, new QueryFilter(0, 0, "t.name", true));
        logger.debug(("### Potential owner task list: " + taskList));
        Assertions.assertThat(taskList).hasSize(3);
        for (int i = 0; i < (taskList.size()); i++) {
            logger.debug(("### Task Name: " + (taskList.get(i).getName())));
    ...
    public void testSingleResult() {
        startHumanTaskProcess(4, "john's task", "john");
        List<TaskSummary> taskList = taskService.getTasksAssignedAsPotentialOwner("john", null, null, new QueryFilter(0, 0, true));
        logger.debug(("### Potential owner task list: " + taskList));
        Assertions.assertThat(taskList).hasSize(1);
        abortHumanTaskProcess(4);
    }
    ...
    private void startHumanTaskProcess(int instanceCount, String taskName, String assigneeName, String localeName) {
        for (int i = 0; i < instanceCount; i++) {
            ...
            instanceList.add(kieSession.startProcess(HumanTaskQueryFilterTest.CONF_HUMAN_TASK_ID, parameters));
        }
    }
