...
public class LocalTaskServiceTest extends JbpmTestCase {
    ...
    // member of Administrators group from properties
    ...
    public void init() throws Exception {
        createRuntimeManager(LocalTaskServiceTest.HUMAN_TASK, LocalTaskServiceTest.EVALUATION, LocalTaskServiceTest.TASK_MULTIPLE_ACTORS, LocalTaskServiceTest.TASK_SINGLE_TYPE);
        RuntimeEngine re = getRuntimeEngine();
        kieSession = re.getKieSession();
        taskService = LocalHTWorkItemHandlerUtil.registerLocalHTWorkItemHandler(kieSession, getEmf(), new org.jbpm.services.task.identity.JBossUserGroupCallbackImpl(LocalTaskServiceTest.USER_GROUP_RES));
    }

    @Test
    public void executeTaskCompleteTest() {
        ProcessInstance processInstance = kieSession.startProcess(LocalTaskServiceTest.HUMAN_TASK_ID);
    ...
    public void executeTaskDelegationTest() {
        ...
        assertProcessInstanceActive(processInstance.getId());
        assertNodeTriggered(processInstance.getId(), "start", "user task");
        List<TaskSummary> list = taskService.getTasksAssignedAsBusinessAdministrator(LocalTaskServiceTest.BUSINESS_ADMINISTRATOR, "en-UK");
        TaskSummary task = list.get(0);
    ...
    public void executeTaskDelegationByBusinessAdministratorTest() {
        ...
        List<TaskSummary> list = taskService.getTasksAssignedAsBusinessAdministrator(LocalTaskServiceTest.BUSINESS_ADMINISTRATOR, "en-UK");
        TaskSummary task = list.get(0);
        delegateTask(task, LocalTaskServiceTest.BUSINESS_ADMINISTRATOR, "doctor");
        assertNodeTriggered(processInstance.getId(), "end");
        assertProcessInstanceCompleted(processInstance.getId());
    }
    ...
    public void groupTaskQueryTest() throws Exception {
        ...
        ProcessInstance process = kieSession.startProcess(LocalTaskServiceTest.EVALUTION_ID, parameters);
    ...
    public void testMultipleActorsClaimedQuery() {
        ...
        ProcessInstance pi = kieSession.startProcess(LocalTaskServiceTest.TASK_MULTIPLE_ACTORS_ID, params);
        // krisv claim task
    ...
    public void testHumanTaskWithSingleTypeContent() {
        ...
        kieSession.startProcess(LocalTaskServiceTest.TASK_SINGLE_TYPE_ID, params);
