...
public class ProcessInstanceLogCleanTest extends JbpmTestCase {
    ...
    public void deleteLogsByProcessName() {
        KieSession kieSession = createKSession(ProcessInstanceLogCleanTest.HELLO_WORLD_PROCESS);
        startProcess(kieSession, ProcessInstanceLogCleanTest.HELLO_WORLD_PROCESS_ID, 2);
        // Check that all records are created.
        List<ProcessInstanceLog> resultList = auditService.processInstanceLogQuery().processId(ProcessInstanceLogCleanTest.HELLO_WORLD_PROCESS_ID).processVersion("1.0").build().getResultList();
        Assertions.assertThat(resultList).hasSize(2);
        Assertions.assertThat(resultList).extracting("processName").containsExactly(ProcessInstanceLogCleanTest.HELLO_WORLD_P1NAME, ProcessInstanceLogCleanTest.HELLO_WORLD_P1NAME);
        // Perform delete
        int resultCount = auditService.processInstanceLogDelete().processName(ProcessInstanceLogCleanTest.HELLO_WORLD_P1NAME).build().execute();
        Assertions.assertThat(resultCount).isEqualTo(2);
        // Check that all records are gone
        resultList = auditService.processInstanceLogQuery().processId(ProcessInstanceLogCleanTest.HELLO_WORLD_PROCESS_ID).processVersion("1.0").build().getResultList();
    ...
    public void deleteLogsByProcessId() {
        KieSession kieSession = createKSession(ProcessInstanceLogCleanTest.HELLO_WORLD_PROCESS);
        startProcess(kieSession, ProcessInstanceLogCleanTest.HELLO_WORLD_PROCESS_ID, 3);
        // Check that all records are created.
        List<ProcessInstanceLog> resultList = auditService.processInstanceLogQuery().processName(ProcessInstanceLogCleanTest.HELLO_WORLD_P1NAME).processVersion("1.0").build().getResultList();
        Assertions.assertThat(resultList).hasSize(3);
        Assertions.assertThat(resultList).extracting("processId").containsExactly(ProcessInstanceLogCleanTest.HELLO_WORLD_PROCESS_ID, ProcessInstanceLogCleanTest.HELLO_WORLD_PROCESS_ID, ProcessInstanceLogCleanTest.HELLO_WORLD_PROCESS_ID);
        // Perform delete
        int resultCount = auditService.processInstanceLogDelete().processId(ProcessInstanceLogCleanTest.HELLO_WORLD_PROCESS_ID).build().execute();
        Assertions.assertThat(resultCount).isEqualTo(3);
        // Check that all records are gone
        resultList = auditService.processInstanceLogQuery().processName(ProcessInstanceLogCleanTest.HELLO_WORLD_P1NAME).processVersion("1.0").build().getResultList();
        Assertions.assertThat(resultList).hasSize(0);
    }
    ...
    public void deleteLogsByVersion() {
        KieSession kieSession = createKSession(ProcessInstanceLogCleanTest.HELLO_WORLD_PROCESS);
        startProcess(kieSession, ProcessInstanceLogCleanTest.HELLO_WORLD_PROCESS_ID, 7);
        disposeRuntimeManager();
        kieSession = createKSession(ProcessInstanceLogCleanTest.HELLO_WORLD_PROCESS2);
        startProcess(kieSession, ProcessInstanceLogCleanTest.HELLO_WORLD_PROCESS2_ID, 2);
    ...
    public void deleteLogsWithStatusActive() {
        ...
        try {
            kieSession = createKSession(ProcessInstanceLogCleanTest.HELLO_WORLD_PROCESS, ProcessInstanceLogCleanTest.HUMAN_TASK);
            instanceList1 = startProcess(kieSession, ProcessInstanceLogCleanTest.HELLO_WORLD_PROCESS_ID, 3);
            instanceList2 = startProcess(kieSession, ProcessInstanceLogCleanTest.HUMAN_TASK_ID, 5);
            ...
            Assertions.assertThat(queryList).extracting("processId").containsExactly(ProcessInstanceLogCleanTest.HELLO_WORLD_PROCESS_ID, ProcessInstanceLogCleanTest.HELLO_WORLD_PROCESS_ID, ProcessInstanceLogCleanTest.HELLO_WORLD_PROCESS_ID);
    ...
    public void deleteLogsByDate() {
        ...
        KieSession kieSession = createKSession(ProcessInstanceLogCleanTest.HELLO_WORLD_PROCESS);
        startProcess(kieSession, ProcessInstanceLogCleanTest.HELLO_WORLD_PROCESS_ID, 4);
        ...
        Assertions.assertThat(resultList).hasSize(4).extracting("processId").containsExactly(ProcessInstanceLogCleanTest.HELLO_WORLD_PROCESS_ID, ProcessInstanceLogCleanTest.HELLO_WORLD_PROCESS_ID, ProcessInstanceLogCleanTest.HELLO_WORLD_PROCESS_ID, ProcessInstanceLogCleanTest.HELLO_WORLD_PROCESS_ID);
    ...
    public void deleteLogsByDateRange() throws InterruptedException {
        KieSession kieSession = createKSession(ProcessInstanceLogCleanTest.PARENT_PROCESS_CALLER, ProcessInstanceLogCleanTest.PARENT_PROCESS_INFO, ProcessInstanceLogCleanTest.HELLO_WORLD_PROCESS);
        Date date1 = new Date();
        startProcess(kieSession, ProcessInstanceLogCleanTest.PARENT_PROCESS_CALLER_ID, 4);
        Date date2 = new Date();
        Thread.sleep(1000);
        startProcess(kieSession, ProcessInstanceLogCleanTest.HELLO_WORLD_PROCESS_ID, 2);
        Date date3 = new Date();
        ...
        Assertions.assertThat(resultList).hasSize(2).extracting("processId").containsExactly(ProcessInstanceLogCleanTest.HELLO_WORLD_PROCESS_ID, ProcessInstanceLogCleanTest.HELLO_WORLD_PROCESS_ID);
    ...
    private void deleteLogsByDateRange(Date startDate, Date endDate, boolean expectRemoval) {
        ...
        int resultCount = auditService.processInstanceLogDelete().startDateRangeStart(startDate).startDateRangeEnd(endDate).build().execute();
        // 1 for ReussableSubprocess and 1 for ParentProcessInfo called by it
    ...
    private int getProcessInstanceLogSize(Date startDateRangeStart, Date startDateRangeEnd) {
        return auditService.processInstanceLogQuery().startDateRangeStart(startDateRangeStart).startDateRangeEnd(startDateRangeEnd).build().getResultList().size();
    }
