...
public class TaskLogCleanTest extends JbpmTestCase {
    ...
    public void testDeleteLogsByProcessId() {
        kieSession = createKSession(TaskLogCleanTest.HUMAN_TASK);
        processInstanceList = startProcess(kieSession, TaskLogCleanTest.HUMAN_TASK_ID, 2);
        int deletedLogs = taskAuditService.auditTaskDelete().processId(TaskLogCleanTest.HUMAN_TASK_ID).build().execute();
    ...
    public void testDeleteLogsByProcessName() {
        kieSession = createKSession(TaskLogCleanTest.HUMAN_TASK);
        processInstanceList = startProcess(kieSession, TaskLogCleanTest.HUMAN_TASK_ID, 3);
    ...
    public void testDeleteLogsByDate() {
        kieSession = createKSession(TaskLogCleanTest.HUMAN_TASK);
        processInstanceList = startProcess(kieSession, TaskLogCleanTest.HUMAN_TASK_ID, 4);
        TaskService taskService = getRuntimeEngine().getTaskService();
    ...
    public void testDeleteLogsByDateRange() throws InterruptedException {
        ...
        kieSession = createKSession(TaskLogCleanTest.HUMAN_TASK, TaskLogCleanTest.INPUT_ASSOCIATION);
        Date date1 = new Date();
        processInstanceList.addAll(startProcess(kieSession, TaskLogCleanTest.INPUT_ASSOCIATION_ID, 2));
        processInstanceList.addAll(startProcess(kieSession, TaskLogCleanTest.HUMAN_TASK_ID, 1));
        Date date2 = new Date();
        Thread.sleep(1000);
        processInstanceList.addAll(startProcess(kieSession, TaskLogCleanTest.INPUT_ASSOCIATION_ID, 1));
        processInstanceList.addAll(startProcess(kieSession, TaskLogCleanTest.HUMAN_TASK_ID, 1));
    ...
    private void testDeleteLogsByDateRange(Date startDate, Date endDate, boolean remove) {
        processInstanceList = new ArrayList<ProcessInstance>();
        kieSession = createKSession(TaskLogCleanTest.HUMAN_TASK);
        processInstanceList.addAll(startProcess(kieSession, TaskLogCleanTest.HUMAN_TASK_ID, 5));
        // Delete tasks created from date1 to date2.
    ...
    public void testDeleteTaskEventByDate() {
        kieSession = createKSession(TaskLogCleanTest.HUMAN_TASK_MULTIACTORS);
        Date startDate = new Date();
        processInstanceList = startProcess(kieSession, TaskLogCleanTest.HUMAN_TASK_MULTIACTORS_ID, 1);
        // Get the task
        TaskService taskService = getRuntimeEngine().getTaskService();
    ...
    private int deleteAuditTaskInstanceLogs(Date startDate, Date endDate) {
        return taskAuditService.auditTaskDelete().dateRangeStart(startDate).dateRangeEnd(endDate).build().execute();
    }

    private List<AuditTask> getAllAuditTaskLogs() {
        return taskAuditService.auditTaskQuery().build().getResultList();
    }
