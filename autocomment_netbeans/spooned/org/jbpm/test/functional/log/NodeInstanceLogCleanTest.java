...
public class NodeInstanceLogCleanTest extends JbpmTestCase {
    ...
    public void deleteLogsByNodeNameAndInstanceId() {
        KieSession kieSession = createKSession(NodeInstanceLogCleanTest.HELLO_WORLD_PROCESS);
        List<ProcessInstance> instanceList = startProcess(kieSession, NodeInstanceLogCleanTest.HELLO_WORLD_PROCESS_ID, 2);
    ...
    public void deleteLogsByNodeId() {
        KieSession kieSession = createKSession(NodeInstanceLogCleanTest.HELLO_WORLD_PROCESS);
        startProcess(kieSession, NodeInstanceLogCleanTest.HELLO_WORLD_PROCESS_ID, 2);
        // Delete one node named "Hello world"
        int resultCount = // the 'End' node
        auditService.nodeInstanceLogDelete().nodeId("_3").build().execute();
