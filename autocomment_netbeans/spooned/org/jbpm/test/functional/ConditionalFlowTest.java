...
public class ConditionalFlowTest extends JbpmTestCase {
    ...
    public void testConditionalFlow() {
        KieSession ksession = createKSession(ConditionalFlowTest.PROCESS);
        IterableProcessEventListener listener = new IterableProcessEventListener();
        ksession.addEventListener(listener);
        ksession.execute(((Command<?>) (getCommands().newStartProcess(ConditionalFlowTest.PROCESS_ID))));
        assertProcessStarted(listener, ConditionalFlowTest.PROCESS_ID);
        ...
        assertProcessCompleted(listener, ConditionalFlowTest.PROCESS_ID);
    }
}
