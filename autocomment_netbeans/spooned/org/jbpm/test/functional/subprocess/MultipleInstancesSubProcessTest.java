...
public class MultipleInstancesSubProcessTest extends JbpmTestCase {
    ...
    public void testMultipleInstances() {
        KieSession kieSession = createKSession(MultipleInstancesSubProcessTest.MULTIPLE_INSTANCES);
        IterableProcessEventListener eventListener = new IterableProcessEventListener();
        kieSession.addEventListener(eventListener);
        kieSession.addEventListener(new org.jbpm.test.listener.DebugProcessEventListener());
        ...
        kieSession.execute(((Command<?>) (getCommands().newStartProcess(MultipleInstancesSubProcessTest.MULTIPLE_INSTANCES_ID, params))));
        assertChangedVariable(eventListener, "list", null, items);
        assertProcessStarted(eventListener, MultipleInstancesSubProcessTest.MULTIPLE_INSTANCES_ID);
        ...
        assertProcessCompleted(eventListener, MultipleInstancesSubProcessTest.MULTIPLE_INSTANCES_ID);
    }
}
