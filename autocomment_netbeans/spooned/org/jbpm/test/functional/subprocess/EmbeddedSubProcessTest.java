...
public class EmbeddedSubProcessTest extends JbpmTestCase {
    ...
    public void testProcessWithEmbeddedSubprocess() {
        KieSession kieSession = createKSession(EmbeddedSubProcessTest.EMBEDDED);
        IterableProcessEventListener eventListener = new IterableProcessEventListener();
        kieSession.addEventListener(eventListener);
        kieSession.execute(((Command<?>) (getCommands().newStartProcess(EmbeddedSubProcessTest.EMBEDDED_ID))));
        assertProcessStarted(eventListener, EmbeddedSubProcessTest.EMBEDDED_ID);
        ...
        assertProcessCompleted(eventListener, EmbeddedSubProcessTest.EMBEDDED_ID);
    }
}
