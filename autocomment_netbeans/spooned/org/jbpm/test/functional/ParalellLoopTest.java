...
public class ParalellLoopTest extends JbpmTestCase {
    ...
    public void testProcessMaryReject() {
        ...
        for (int i = 0; i < 20; i++) {
            ParalellLoopTest.logger.debug(">>> Loop: {}", i);
    ...
    private void complete(String user, String outcome) {
        ...
        ParalellLoopTest.logger.debug("complete task");
        ParalellLoopTest.logger.debug("- {} is executing task {}", user, task.getName());
        taskService.start(task.getId(), user);
        Map<String, Object> map = new HashMap<String, Object>();
        map.put("OUTCOME", outcome);
        taskService.complete(task.getId(), user, map);
        ParalellLoopTest.logger.debug("- {} executed task {}", user, task.getName());
    }
