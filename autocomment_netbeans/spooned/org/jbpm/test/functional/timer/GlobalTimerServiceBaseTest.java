...
public abstract class GlobalTimerServiceBaseTest extends TimerBaseTest {
    ...
    public void testInterediateTimerWithHTAfterWithGlobalTestService() throws Exception {
        ...
        assertTrue(((processInstance.getState()) == (ProcessInstance.STATE_ACTIVE)));
        GlobalTimerServiceBaseTest.logger.debug("Disposed after start");
        // dispose session to force session to be reloaded on timer expiration
        manager.disposeRuntimeEngine(runtime);
        countDownListener.waitTillCompleted();
    ...
    public void testInterediateTimerWithGlobalTestServiceSimulateCMT() throws Exception {
        ...
        TransactionManager tm = new org.jbpm.persistence.jta.ContainerManagedTransactionManager();
        Environment env = EnvironmentFactory.newEnvironment();
        env.set(EnvironmentName.ENTITY_MANAGER_FACTORY, emf);
        env.set(EnvironmentName.TRANSACTION_MANAGER, tm);
        environment = RuntimeEnvironmentBuilder.Factory.get().newDefaultBuilder().entityManagerFactory(emf).addAsset(ResourceFactory.newClassPathResource("org/jbpm/test/functional/timer/IntermediateCatchEventTimerCycleWithHT2.bpmn2"), ResourceType.BPMN2).addEnvironmentEntry(EnvironmentName.TRANSACTION_MANAGER, tm).addEnvironmentEntry(EnvironmentName.PERSISTENCE_CONTEXT_MANAGER, new org.jbpm.persistence.JpaProcessPersistenceContextManager(env)).addEnvironmentEntry(EnvironmentName.TASK_PERSISTENCE_CONTEXT_MANAGER, new org.jbpm.services.task.persistence.JPATaskPersistenceContextManager(env)).schedulerService(globalScheduler).registerableItemsFactory(new TestRegisterableItemsFactory(listener, countDownListener)).userGroupCallback(userGroupCallback).get();
