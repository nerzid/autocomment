...
public class TimerPersistenceTest extends JbpmTestCase {
    ...
    @Before
    public void setup() {
        System.clearProperty(TimerPersistenceTest.TIMER_FIRED_PROP);
        System.clearProperty(TimerPersistenceTest.TIMER_FIRED_TIME_PROP);
    }
    ...
    public void boundaryEventTimerAndCompleteHumanTask() throws InterruptedException {
         ...
         */
        createRuntimeManager(TimerPersistenceTest.PROCESS_FILE_NAME);
        RuntimeEngine runtimeEngine = getRuntimeEngine();
        KieSession ksession = runtimeEngine.getKieSession();
        long ksessionId = ksession.getIdentifier();
        assertTrue("session id not saved.", (ksessionId > 0));
        TimerPersistenceTest.HumanTaskMockHandler humanTaskMockHandler = new TimerPersistenceTest.HumanTaskMockHandler();
    ...
    public void timerEventOutsideOfKnowledgeSessionTest() throws Exception {
        ...
        long ksessionId = ksession.getIdentifier();
        // Start the process
        ProcessInstance process = ksession.startProcess(TimerPersistenceTest.DELAY_TIMER_PROCESS);
        ...
        // The timer fires..
        int sleep = 2000;
        TimerPersistenceTest.logger.debug("Sleeping {} seconds", (sleep / 1000));
        Thread.sleep(sleep);
        TimerPersistenceTest.logger.debug("Awake!");
    ...
    private ProcessInstance registerHTHandlerAndStartProcess(KieSession ksession, TimerPersistenceTest.HumanTaskMockHandler humanTaskMockHandler) {
        // Register Human Task Handler
        ksession.getWorkItemManager().registerWorkItemHandler(TimerPersistenceTest.WORK_ITEM_HANLDER_TASK, humanTaskMockHandler);
        // Start the process
        ProcessInstance process = ksession.startProcess(TimerPersistenceTest.PROCESS_NAME);
        long processId = process.getId();
    ...
    private void completeWork(KieSession ksession, TimerPersistenceTest.HumanTaskMockHandler humanTaskMockHandler) {
        ...
        try {
            ...
            TimerPersistenceTest.logger.warn("Work item could not be completed!");
            e.printStackTrace();
    ...
    private boolean timerHasFired() {
        String hasFired = System.getProperty(TimerPersistenceTest.TIMER_FIRED_PROP);
        if (hasFired != null) {
            return true;
        } 
        return false;
    ...
    private int timerFiredCount() {
        String timerFiredCount = System.getProperty(TimerPersistenceTest.TIMER_FIRED_PROP);
    ...
    private long timerFiredTime() {
        String timerFiredCount = System.getProperty(TimerPersistenceTest.TIMER_FIRED_TIME_PROP);
        if (timerFiredCount == null) {
            return 0;
        } 
    ...
    private static class HumanTaskMockHandler implements WorkItemHandler {
        ...
        public void executeWorkItem(WorkItem workItem, WorkItemManager manager) {
            TimerPersistenceTest.HumanTaskMockHandler.this.workItem = workItem;
            TimerPersistenceTest.HumanTaskMockHandler.this.workItemManager = manager;
            TimerPersistenceTest.logger.debug("Work completed!");
        }

        public void abortWorkItem(WorkItem workItem, WorkItemManager manager) {
            TimerPersistenceTest.HumanTaskMockHandler.this.workItemManager.abortWorkItem(workItem.getId());
            TimerPersistenceTest.logger.debug("Work aborted.");
        }
    }
}
