...
public class EventBasedGatewayTest extends JbpmTestCase {
    ...
    public void setUp() throws Exception {
        super.setUp();
        ksession = createKSession(EventBasedGatewayTest.EVENT_BASED_GATEWAY);
    }
    ...
    public void testConditional() {
        ...
        commands.add(CommandFactory.newStartProcess(EventBasedGatewayTest.EVENT_BASED_GATEWAY_ID));
        ksession.execute(CommandFactory.newBatchExecution(commands));
        TrackingListenerAssert.assertProcessStarted(tpel, EventBasedGatewayTest.EVENT_BASED_GATEWAY_ID);
        ...
        TrackingListenerAssert.assertProcessCompleted(tpel, EventBasedGatewayTest.EVENT_BASED_GATEWAY_ID);
    }
    ...
    public void testMessage() {
        TrackingProcessEventListener tpel = new TrackingProcessEventListener();
        ksession.addEventListener(tpel);
        ProcessInstance pi = ((ProcessInstance) (ksession.execute(((Command<?>) (CommandFactory.newStartProcess(EventBasedGatewayTest.EVENT_BASED_GATEWAY_ID))))));
        TrackingListenerAssert.assertProcessStarted(tpel, EventBasedGatewayTest.EVENT_BASED_GATEWAY_ID);
        TrackingListenerAssert.assertTriggeredAndLeft(tpel, "start");
        TrackingListenerAssert.assertTriggeredAndLeft(tpel, "fork");
    ...
    public void testTimer() throws Exception {
        ...
        TrackingProcessEventListener tpel = new TrackingProcessEventListener();
        ksession.addEventListener(tpel);
        ksession.execute(((Command<?>) (CommandFactory.newStartProcess(EventBasedGatewayTest.EVENT_BASED_GATEWAY_ID))));
        TrackingListenerAssert.assertProcessStarted(tpel, EventBasedGatewayTest.EVENT_BASED_GATEWAY_ID);
        TrackingListenerAssert.assertTriggeredAndLeft(tpel, "start");
