...
public class LaneTest extends JbpmTestCase {
    ...
    public void testLane() {
        KieSession ksession = createKSession(LaneTest.PROCESS);
        TrackingProcessEventListener tpel = new TrackingProcessEventListener();
        ksession.addEventListener(tpel);
        ksession.execute(((Command<?>) (getCommands().newStartProcess(LaneTest.PROCESS_ID))));
        assertProcessStarted(tpel, LaneTest.PROCESS_ID);
        ...
        assertProcessCompleted(tpel, LaneTest.PROCESS_ID);
    }
}
