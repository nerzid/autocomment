...
public abstract class LdapJbpmTestCase extends JbpmTestCase {
    ...
    public void startDirectoryServer() throws LDAPException {
        ...
        InMemoryDirectoryServerConfig serverConfig = new InMemoryDirectoryServerConfig(new com.unboundid.ldap.sdk.DN("dc=jboss,dc=org"));
        serverConfig.setListenerConfigs(listenerConfig);
        serverConfig.addAdditionalBindCredentials("uid=admin,ou=system", "secret");
        serverConfig.setSchema(null);
        LdapJbpmTestCase.server = new InMemoryDirectoryServer(serverConfig);
        LdapJbpmTestCase.server.importFromLDIF(false, ldif);
        LdapJbpmTestCase.server.startListening();
    }

    @After
    public void stopDirectoryServer() {
        if ((LdapJbpmTestCase.server) != null) {
            LdapJbpmTestCase.server.shutDown(true);
        } 
    }
