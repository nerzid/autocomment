...
public abstract class JbpmJUnitTestCase extends AbstractBaseTest {
    ...
    private JbpmJUnitTestCase.H2Server server = new JbpmJUnitTestCase.H2Server();

    private TaskService taskService;

    private JbpmJUnitTestCase.TestWorkItemHandler workItemHandler = new JbpmJUnitTestCase.TestWorkItemHandler();
    ...
    public JbpmJUnitTestCase(boolean setupDataSource) {
        ...
        JbpmJUnitTestCase.this.setupDataSource = setupDataSource;
    }
    ...
    public void setPersistence(boolean sessionPersistence) {
        JbpmJUnitTestCase.this.sessionPersistence = sessionPersistence;
    }
    ...
    public void setUp() throws Exception {
        if (setupDataSource) {
            server.start();
            ds = JbpmJUnitTestCase.setupPoolingDataSource();
        ...
        if (setupDataSource) {
            ...
            if (tx != null) {
                ...
                if (((testTxState != (Status.STATUS_NO_TRANSACTION)) && (testTxState != (Status.STATUS_ROLLEDBACK))) && (testTxState != (Status.STATUS_COMMITTED))) {
                    ...
                    Assert.fail((("Transaction had status " + (JBPMHelper.txStateName[testTxState])) + " at the end of the test."));
                } 
            } 
        } 
        if ((manager) != null) {
    ...
    protected KieBase createKnowledgeBase(Map<String, ResourceType> resources) {
        ...
        builder.userGroupCallback(new org.jbpm.services.task.identity.JBossUserGroupCallbackImpl("classpath:/usergroups.properties"));
    ...
    protected KnowledgeBase createKnowledgeBaseGuvnor(boolean dynamic, String url, String username, String password, String pkg, String... assets) throws Exception {
        ...
        String changeSet = (((((("<change-set xmlns='http://drools.org/drools-5.0/change-set'" + (JbpmJUnitTestCase.EOL)) + "            xmlns:xs='http://www.w3.org/2001/XMLSchema-instance'") + (JbpmJUnitTestCase.EOL)) + "            xs:schemaLocation='http://drools.org/drools-5.0/change-set http://anonsvn.jboss.org/repos/labs/labs/jbossrules/trunk/drools-api/src/main/resources/change-set-1.0.0.xsd' >") + (JbpmJUnitTestCase.EOL)) + "    <add>") + (JbpmJUnitTestCase.EOL);
        for (String a : assets) {
            if ((a.indexOf(".bpmn")) >= 0) {
                a = a.substring(0, a.indexOf(".bpmn"));
            } 
            changeSet += (((((((((("        <resource source='" + url) + "/rest/packages/") + pkg) + "/assets/") + a) + "/binary\' type=\'BPMN2\' basicAuthentication=\"enabled\" username=\"") + username) + "\" password=\"") + password) + "\" />") + (JbpmJUnitTestCase.EOL);
        }
        changeSet += ("    </add>" + (JbpmJUnitTestCase.EOL)) + "</change-set>";
    ...
    protected KnowledgeBase createKnowledgeBaseGuvnor(boolean dynamic, String url, String username, String password, String... packages) throws Exception {
        ...
        for (String p : packages) {
            changeSet += (((((((("        <resource source='" + url) + "/rest/packages/") + p) + "/binary\' type=\'PKG\' basicAuthentication=\"enabled\" username=\"") + username) + "\" password=\"") + password) + "\" />") + (JbpmJUnitTestCase.EOL);
        }
        changeSet += ("    </add>" + (JbpmJUnitTestCase.EOL)) + "</change-set>";
        kbuilder.add(ResourceFactory.newByteArrayResource(changeSet.getBytes()), ResourceType.CHANGE_SET);
        return kbuilder.newKnowledgeBase();
    ...
    protected KieSession createKnowledgeSession() {
        ...
        if (sessionPersistence) {
            logService = new org.jbpm.process.audit.JPAAuditLogService(environment.getEnvironment());
        } else {
    ...
    public static class TestWorkItemHandler implements WorkItemHandler {
        ...
        public WorkItem getWorkItem() {
            ...
            if ((workItems.size()) == 1) {
                ...
                JbpmJUnitTestCase.TestWorkItemHandler.this.workItems.clear();
                return result;
            } else {
    ...
    public TaskService getService() {
        return ((InternalTaskService) (HumanTaskServiceFactory.newTaskServiceConfigurator().userGroupCallback(new org.jbpm.services.task.identity.MvelUserGroupCallbackImpl(true)).entityManagerFactory(emf).getTaskService()));
    }
