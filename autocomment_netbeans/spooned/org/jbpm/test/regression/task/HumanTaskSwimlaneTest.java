...
public class HumanTaskSwimlaneTest extends JbpmTestCase {
    ...
    public void testSameGroups() {
        createRuntimeManager(HumanTaskSwimlaneTest.SWIMLANE_SAME_GROUPS);
        KieSession ksession = getRuntimeEngine().getKieSession();
        taskService = getRuntimeEngine().getTaskService();
        ProcessInstance pi = ksession.startProcess(HumanTaskSwimlaneTest.SWIMLANE_SAME_GROUPS_ID);
    ...
    public void testDifferentGroups() {
        createRuntimeManager(HumanTaskSwimlaneTest.SWIMLANE_DIFFERENT_GROUPS);
        KieSession ksession = getRuntimeEngine().getKieSession();
        taskService = getRuntimeEngine().getTaskService();
        ProcessInstance pi = ksession.startProcess(HumanTaskSwimlaneTest.SWIMLANE_DIFFERENT_GROUPS_ID);
        long task1 = getActiveTask(pi).getId();
        taskService.claim(task1, "john");
        taskService.start(task1, "john");
        taskService.complete(task1, "john", null);
    ...
    public void testSwimlaneWithMultipleActorsAssigned() {
        createRuntimeManager(HumanTaskSwimlaneTest.SWIMLANE_MULTIPLE_ACTORS);
        String user = "john";
        ...
        ProcessInstance instance = kSession.startProcess(HumanTaskSwimlaneTest.SWIMLANE_MULTIPLE_ACTORS_ID, map);
