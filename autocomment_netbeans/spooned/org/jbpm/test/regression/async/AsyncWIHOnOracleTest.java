...
public class AsyncWIHOnOracleTest extends JbpmAsyncJobTestCase {
    ...
    public AsyncWIHOnOracleTest() {
        super(AsyncWIHOnOracleTest.EXECUTOR_THREADS, AsyncWIHOnOracleTest.EXECUTOR_RETRIES, AsyncWIHOnOracleTest.EXECUTOR_INTERVAL);
    }
    ...
    public void testAsyncWIHExecutedMoreThanOnceOnOracle() throws Exception {
        KieSession ksession = createKSession(AsyncWIHOnOracleTest.PROCESS);
        TrackingProcessEventListener tpel = new TrackingProcessEventListener();
        ksession.addEventListener(tpel);
        WorkItemHandler wih = new org.jbpm.executor.impl.wih.AsyncWorkItemHandler(getExecutorService(), AsyncWIHOnOracleTest.CounterCommand.class.getName());
        ksession.getWorkItemManager().registerWorkItemHandler("async", wih);
        ksession.startProcess(AsyncWIHOnOracleTest.PROCESS_ID);
        ...
        Assertions.assertThat(AsyncWIHOnOracleTest.CounterCommand.getCounter()).as("The job has not been executed").isNotEqualTo(0);
        Assertions.assertThat(AsyncWIHOnOracleTest.CounterCommand.getCounter()).as("The job has been executed multiple times").isEqualTo(1);
    }
    ...
    public static class CounterCommand implements Command {
        ...
        public ExecutionResults execute(CommandContext commandContext) throws Exception {
            ++(AsyncWIHOnOracleTest.CounterCommand.counter);
            return new ExecutionResults();
        }

        public static int getCounter() {
            return AsyncWIHOnOracleTest.CounterCommand.counter;
        }
    }
}
