...
public class MultipleInstancesSubprocessTest extends JbpmTestCase {
    ...
    public void testTimerEvent() throws Exception {
        KieSession ksession = createKSession(MultipleInstancesSubprocessTest.TIMER_EVENT_PARENT, MultipleInstancesSubprocessTest.TIMER_EVENT_SUBPROCESS1, MultipleInstancesSubprocessTest.TIMER_EVENT_SUBPROCESS2);
        ...
        commands.add(getCommands().newStartProcess(MultipleInstancesSubprocessTest.TIMER_EVENT_PARENT_ID, params));
    ...
    public void testEntryAndExitScript() {
        createRuntimeManager(MultipleInstancesSubprocessTest.ENTRY_AND_EXIT_SCRIPT_PARENT, MultipleInstancesSubprocessTest.ENTRY_AND_EXIT_SCRIPT_SUBPROCESS);
        KieSession ksession = getRuntimeEngine().getKieSession();
        ProcessInstance pi = ksession.startProcess(MultipleInstancesSubprocessTest.ENTRY_AND_EXIT_SCRIPT_PARENT_ID);
        ...
        Assertions.assertThat(varList).hasSize(1);
        Assertions.assertThat(Boolean.valueOf(varList.get(0).getValue())).isTrue();
        List<? extends ProcessInstanceLog> processList = getLogService().findProcessInstances(MultipleInstancesSubprocessTest.ENTRY_AND_EXIT_SCRIPT_PARENT_ID);
        ...
        processList = getLogService().findProcessInstances(MultipleInstancesSubprocessTest.ENTRY_AND_EXIT_SCRIPT_SUBPROCESS_ID);
