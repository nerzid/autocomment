...
public class ReusableSubprocessTest extends JbpmTestCase {
    ...
    public void testWaitForCompletionFalse() throws Exception {
        createRuntimeManager(Strategy.PROCESS_INSTANCE, "BZ1194180-ppi-manager", ReusableSubprocessTest.WAIT_FOR_COMPLETION_FALSE_PARENT, ReusableSubprocessTest.WAIT_FOR_COMPLETION_FALSE_SUBPROCESS);
        KieSession ksession = getRuntimeEngine().getKieSession();
        Assertions.assertThat(ksession).isNotNull();
        ksession.startProcess(ReusableSubprocessTest.WAIT_FOR_COMPLETION_FALSE_PARENT_ID);
        Thread.sleep(3000);
    }
    ...
    public void testDependentSubprocessAbort() {
        RuntimeManager manager = createRuntimeManager(Strategy.PROCESS_INSTANCE, "myPpiManager", ReusableSubprocessTest.DEPENDENT_SUBPROCESS_ABORT_PARENT, ReusableSubprocessTest.DEPENDENT_SUBPROCESS_ABORT_SUBPROCESS);
        ...
        ProcessInstance pi1 = ksession.startProcess(ReusableSubprocessTest.DEPENDENT_SUBPROCESS_ABORT_PARENT_ID);
        ...
        List<? extends ProcessInstanceLog> logs = logService.findActiveProcessInstances(ReusableSubprocessTest.DEPENDENT_SUBPROCESS_ABORT_PARENT_ID);
        Assertions.assertThat(logs).isNotNull();
        Assertions.assertThat(logs).hasSize(0);
        logs = logService.findActiveProcessInstances(ReusableSubprocessTest.DEPENDENT_SUBPROCESS_ABORT_SUBPROCESS_ID);
        Assertions.assertThat(logs).isNotNull();
        Assertions.assertThat(logs).hasSize(0);
        logs = logService.findProcessInstances(ReusableSubprocessTest.DEPENDENT_SUBPROCESS_ABORT_PARENT_ID);
        Assertions.assertThat(logs).isNotNull();
        Assertions.assertThat(logs).hasSize(1);
        assertProcessInstanceAborted(logs.get(0).getProcessInstanceId());
        logs = logService.findProcessInstances(ReusableSubprocessTest.DEPENDENT_SUBPROCESS_ABORT_SUBPROCESS_ID);
        Assertions.assertThat(logs).isNotNull();
        Assertions.assertThat(logs).hasSize(1);
        assertProcessInstanceAborted(logs.get(0).getProcessInstanceId());
        manager.close();
