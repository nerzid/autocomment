...
public class StartEventTest extends JbpmTestCase {
    ...
    public void testErrorStartEventDefaultExceptionHandler() {
        KieSession ksession = createKSession(StartEventTest.ERROR_EXCEPTION_HANDLER);
        ProcessInstance pi = ksession.startProcess(StartEventTest.ERROR_EXCEPTION_HANDLER_ID);
        List<? extends VariableInstanceLog> variables = getLogService().findVariableInstances(pi.getId(), "capturedException");
    ...
    public void testErrorStartEventDataOutputMapping() {
        KieSession ksession = createKSession(StartEventTest.ERROR_EXCEPTION_MAPPING);
        ProcessInstance pi = ksession.startProcess(StartEventTest.ERROR_EXCEPTION_MAPPING_ID);
        List<? extends VariableInstanceLog> variables = getLogService().findVariableInstances(pi.getId(), "capturedException");
        // TODO: size should probably be just 1 (2 records caused by JBPM-4669 fix)
        Assertions.assertThat(variables).hasSize(2);
        Assertions.assertThat(variables.get(0).getValue()).isEqualTo("java.lang.RuntimeException: XXX");
    ...
    public void testSignalStartEventDataMapping() throws Exception {
        KieSession ksession = createKSession(StartEventTest.SIGNAL_DATA_MAPPING);
    ...
    public void testSignalOutputType() throws Exception {
        KieSession ksession = createKSession(StartEventTest.SIGNAL_OUTPUT_TYPE);
