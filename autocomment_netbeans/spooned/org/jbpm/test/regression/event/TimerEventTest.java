...
public class TimerEventTest extends JbpmTestCase {
    ...
    public void testRuntimeExceptionAfterTimer() throws InterruptedException {
        KieSession ksession = createKSession(TimerEventTest.EXCEPTION_AFTER_TIMER);
        ProcessInstance pi = ksession.startProcess(TimerEventTest.EXCEPTION_AFTER_TIMER_ID);
        Thread.sleep(3000);
    ...
    public void testStartTimerCycle() throws InterruptedException {
        KieSession ksession = createKSession(TimerEventTest.START_TIMER_CYCLE);
        AuditService auditService = getLogService();
        ksession.startProcess(TimerEventTest.START_TIMER_CYCLE_ID);
    ...
    public void testCancelledTimerNotScheduled() {
        for (int i = 0; i < 5; i++) {
            createRuntimeManager(Strategy.PROCESS_INSTANCE, ((String) (null)), TimerEventTest.CANCELLED_TIMER);
            KieSession ksession = getRuntimeEngine().getKieSession();
            TaskService taskService = getRuntimeEngine().getTaskService();
            Map<String, Object> params = new HashMap<String, Object>();
            ProcessInstance pi = ksession.startProcess(TimerEventTest.CANCELLED_TIMER_ID, params);
    ...
    public void testTimerAndGateway() throws Exception {
        KieSession ksession = createKSession(TimerEventTest.TIMER_AND_GATEWAY);
        int sessionId = ksession.getId();
        TimerEventTest.TestAsyncWorkItemHandler handler1 = new TimerEventTest.TestAsyncWorkItemHandler();
        TimerEventTest.TestAsyncWorkItemHandler handler2 = new TimerEventTest.TestAsyncWorkItemHandler();
        ...
        ProcessInstance instance = ksession.createProcessInstance(TimerEventTest.TIMER_AND_GATEWAY_ID, new HashMap<String, Object>());
        ...
        ksession = restoreKSession(TimerEventTest.TIMER_AND_GATEWAY);
        ksession.getWorkItemManager().registerWorkItemHandler("task1", handler1);
        ksession.getWorkItemManager().registerWorkItemHandler("task2", handler2);
    ...
    public void testBoundaryTimerInMultipleInstancesSubprocess() throws InterruptedException {
        KieSession ksession = createKSession(TimerEventTest.BOUNDARY_MULTIPLE_INSTANCES);
        ...
        ksession.startProcess(TimerEventTest.BOUNDARY_MULTIPLE_INSTANCES_ID, parameters);
    ...
    private static class TestAsyncWorkItemHandler implements WorkItemHandler {
        ...
        public void executeWorkItem(WorkItem workItem, WorkItemManager manager) {
            ...
            TimerEventTest.TestAsyncWorkItemHandler.this.workItem = workItem;
            (TimerEventTest.TestAsyncWorkItemHandler.this.activations)++;
        }

        public void abortWorkItem(WorkItem workItem, WorkItemManager manager) {
            TimerEventTest.TestAsyncWorkItemHandler.this.workItem = null;
        }
