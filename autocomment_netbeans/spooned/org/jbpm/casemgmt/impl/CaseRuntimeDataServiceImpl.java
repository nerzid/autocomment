...
public class CaseRuntimeDataServiceImpl implements CaseRuntimeDataService , DeploymentEventListener {
    ...
    public void setCaseIdGenerator(CaseIdGenerator caseIdGenerator) {
        CaseRuntimeDataServiceImpl.this.caseIdGenerator = caseIdGenerator;
    }

    public void setRuntimeDataService(RuntimeDataService runtimeDataService) {
        CaseRuntimeDataServiceImpl.this.runtimeDataService = runtimeDataService;
    }

    public void setCommandService(TransactionalCommandService commandService) {
        CaseRuntimeDataServiceImpl.this.commandService = commandService;
    }

    public void setIdentityProvider(IdentityProvider identityProvider) {
        CaseRuntimeDataServiceImpl.this.identityProvider = identityProvider;
    }

    public void setDeploymentRolesManager(DeploymentRolesManager deploymentRolesManager) {
        CaseRuntimeDataServiceImpl.this.deploymentRolesManager = deploymentRolesManager;
    }
    ...
    public void onUnDeploy(DeploymentEvent event) {
        Collection<CaseDefinitionImpl> undeployed = availableCases.stream().filter(( caseDef) -> caseDef.getDeploymentId().equals(event.getDeploymentId())).collect(Collectors.toList());
    ...
    public CaseDefinition getCase(String deploymentId, String caseDefinitionId) {
        return availableCases.stream().filter(( caseDef) -> (caseDef.getDeploymentId().equals(deploymentId)) && (caseDef.getId().equals(caseDefinitionId))).findFirst().orElse(null);
    }

    @Override
    public Collection<CaseDefinition> getCases(QueryContext queryContext) {
        Collection<CaseDefinition> cases = availableCases.stream().skip(queryContext.getOffset()).filter(( caseDef) -> caseDef.isActive()).limit(queryContext.getCount()).sorted(new org.jbpm.casemgmt.impl.model.CaseDefinitionComparator(queryContext.getOrderBy())).collect(Collectors.toList());
        return cases;
    }
    ...
    public Collection<CaseDefinition> getCases(String filter, QueryContext queryContext) {
        ...
        Collection<CaseDefinition> cases = availableCases.stream().skip(queryContext.getOffset()).filter(( caseDef) -> (caseDef.isActive()) && ((caseDef.getId().matches(pattern)) || (caseDef.getName().matches(pattern)))).limit(queryContext.getCount()).sorted(new org.jbpm.casemgmt.impl.model.CaseDefinitionComparator(queryContext.getOrderBy())).collect(Collectors.toList());
        return cases;
    }

    @Override
    public Collection<CaseDefinition> getCasesByDeployment(String deploymentId, QueryContext queryContext) {
        Collection<CaseDefinition> cases = availableCases.stream().skip(queryContext.getOffset()).filter(( caseDef) -> (caseDef.isActive()) && (caseDef.getDeploymentId().equals(deploymentId))).limit(queryContext.getCount()).sorted(new org.jbpm.casemgmt.impl.model.CaseDefinitionComparator(queryContext.getOrderBy())).collect(Collectors.toList());
        return cases;
    }
    ...
    public Collection<CaseMilestoneInstance> getCaseInstanceMilestones(String caseId, boolean achievedOnly, QueryContext queryContext) {
        CorrelationKey correlationKey = correlationKeyFactory.newCorrelationKey(caseId);
        Collection<NodeInstanceDesc> nodes = runtimeDataService.getNodeInstancesByCorrelationKeyNodeType(correlationKey, Arrays.asList(ProcessInstance.STATE_ACTIVE), Arrays.asList("MilestoneNode"), queryContext);
        Collection<Long> completedNodes = nodes.stream().filter(( n) -> (((NodeInstanceDesc) (n)).getType()) == null).map(( n) -> n.getId()).collect(Collectors.toList());
        Predicate<NodeInstanceDesc> filterNodes = null;
        if (achievedOnly) {
            filterNodes = ( n) -> (((NodeInstanceDesc) (n)).getType()) == null;
        } else {
            filterNodes = ( n) -> (((NodeInstanceDesc) (n)).getType()) == null;
        }
        Collection<CaseMilestoneInstance> milestones = nodes.stream().filter(filterNodes).map(( n) -> new CaseMilestoneInstanceImpl(String.valueOf(n.getId()), n.getName(), completedNodes.contains(n.getId()), n.getDataTimeStamp())).collect(Collectors.toList());
        return milestones;
    }
    ...
    public Collection<CaseStageInstance> getCaseInstanceStages(String caseId, boolean activeOnly, QueryContext queryContext) {
        ...
        if ((pi == null) || (!(pi.getState().equals(ProcessInstance.STATE_ACTIVE)))) {
            throw new org.jbpm.casemgmt.api.CaseNotFoundException((("No case instance found with id " + caseId) + " or it's not active anymore"));
        } 
    ...
    public Collection<NodeInstanceDesc> getActiveNodesForCase(String caseId, QueryContext queryContext) {
        ...
        List<NodeInstanceDesc> nodeInstances = commandService.execute(new org.jbpm.shared.services.impl.commands.QueryNameCommand<List<NodeInstanceDesc>>("getActiveNodesForCase", params));
    ...
    public Collection<CaseInstance> getCaseInstances(List<Integer> statuses, QueryContext queryContext) {
        ...
        applyQueryContext(params, queryContext);
        applyDeploymentFilter(params);
        List<CaseInstance> processInstances = commandService.execute(new org.jbpm.shared.services.impl.commands.QueryNameCommand<List<CaseInstance>>("getCaseInstances", params));
    ...
    public Collection<CaseInstance> getCaseInstancesByDeployment(String deploymentId, List<Integer> statuses, QueryContext queryContext) {
        ...
        applyQueryContext(params, queryContext);
        applyDeploymentFilter(params);
        List<CaseInstance> processInstances = commandService.execute(new org.jbpm.shared.services.impl.commands.QueryNameCommand<List<CaseInstance>>("getCaseInstancesByDeployment", params));
        return processInstances;
    }
    ...
    public Collection<CaseInstance> getCaseInstancesByDefinition(String definitionId, List<Integer> statuses, QueryContext queryContext) {
        ...
        applyQueryContext(params, queryContext);
        applyDeploymentFilter(params);
        List<CaseInstance> processInstances = commandService.execute(new org.jbpm.shared.services.impl.commands.QueryNameCommand<List<CaseInstance>>("getCaseInstancesByDefinition", params));
        return processInstances;
    }
    ...
    public Collection<CaseInstance> getCaseInstancesOwnedBy(String owner, List<Integer> statuses, QueryContext queryContext) {
        ...
        applyQueryContext(params, queryContext);
        applyDeploymentFilter(params);
        List<CaseInstance> processInstances = commandService.execute(new org.jbpm.shared.services.impl.commands.QueryNameCommand<List<CaseInstance>>("getCaseInstancesOwnedBy", params));
        return processInstances;
    }
    ...
    public Collection<AdHocFragment> getAdHocFragmentsForCase(String caseId) {
        ...
        CaseDefinition caseDef = getCase(pi.getDeploymentId(), pi.getProcessId());
        List<AdHocFragment> adHocFragments = new ArrayList<AdHocFragment>();
    ...
    public Collection<CaseStageInstance> internalGetCaseStages(CaseDefinition caseDef, String caseId, boolean activeOnly, QueryContext queryContext) {
        CorrelationKey correlationKey = correlationKeyFactory.newCorrelationKey(caseId);
        Collection<NodeInstanceDesc> nodes = runtimeDataService.getNodeInstancesByCorrelationKeyNodeType(correlationKey, Arrays.asList(ProcessInstance.STATE_ACTIVE), Arrays.asList("DynamicNode"), queryContext);
        Map<String, CaseStage> stagesByName = caseDef.getCaseStages().stream().collect(Collectors.toMap(CaseStage::getId, ( c) -> c));
        Predicate<NodeInstanceDesc> filterNodes = null;
        if (activeOnly) {
            Collection<Long> completedNodes = nodes.stream().filter(( n) -> (((NodeInstanceDesc) (n)).getType()) == null).map(( n) -> n.getId()).collect(Collectors.toList());
            filterNodes = ( n) -> ((((NodeInstanceDesc) (n)).getType()) == null) && (!(completedNodes.contains(((NodeInstanceDesc) (n)).getId())));
        } else {
            filterNodes = ( n) -> (((NodeInstanceDesc) (n)).getType()) == null;
        }
        Collection<CaseStageInstance> stages = nodes.stream().filter(filterNodes).map(( n) -> new CaseStageInstanceImpl(n.getNodeId(), n.getName(), stagesByName.get(n.getNodeId()).getAdHocFragments(), Collections.emptyList())).collect(Collectors.toList());
        return stages;
    }
    ...
    private void getMilestones(NodeContainer container, Collection<CaseMilestone> result) {
        for (Node node : container.getNodes()) {
            if (node instanceof MilestoneNode) {
                result.add(new org.jbpm.casemgmt.impl.model.CaseMilestoneImpl(((String) (node.getMetaData().get("UniqueId"))), node.getName(), ((MilestoneNode) (node)).getConstraint(), false));
            } 
    ...
    private Collection<CaseStage> collectCaseStages(String deploymentId, String processId, NodeContainer process) {
        ...
        for (Node node : process.getNodes()) {
            if (node instanceof DynamicNode) {
                ...
                result.add(new org.jbpm.casemgmt.impl.model.CaseStageImpl(((String) (((DynamicNode) (node)).getMetaData("UniqueId"))), node.getName(), adHocFragments));
            } 
        }
        return result;
    }
    ...
    private void checkAdHoc(NodeContainer nodeContainer, List<AdHocFragment> result) {
        for (Node node : nodeContainer.getNodes()) {
            ...
            if (node.getIncomingConnections().isEmpty()) {
                result.add(new org.jbpm.casemgmt.impl.model.AdHocFragmentImpl(node.getName(), node.getClass().getSimpleName()));
            } 
        }
    }
