...
public class CountDownListenerFactory {
    private static Map<String, CountDownProcessEventListener> listeners = new ConcurrentHashMap<CountDownProcessEventListener>();

    public static CountDownProcessEventListener get(String id, String nodeName, int threads) {
        if (CountDownListenerFactory.listeners.containsKey(id)) {
            return CountDownListenerFactory.listeners.get(id);
        } 
        CountDownProcessEventListener listener = new CountDownProcessEventListener(nodeName, threads);
        CountDownListenerFactory.listeners.put(id, listener);
        return listener;
    }
    ...
    public static CountDownProcessEventListener removeExisting(String id) {
        return CountDownListenerFactory.listeners.remove(id);
    }

    public static void clear() {
        CountDownListenerFactory.listeners.clear();
    }
}
