...
public class CaseCommentCommand extends CaseCommand<Void> {
    ...
    public CaseCommentCommand(String author, String comment) {
        CaseCommentCommand.this.author = author;
        CaseCommentCommand.this.comment = comment;
        CaseCommentCommand.this.add = true;
    }

    public CaseCommentCommand(String commentId) {
        CaseCommentCommand.this.commentId = commentId;
        CaseCommentCommand.this.remove = true;
    }

    public CaseCommentCommand(String commentId, String author, String updatedText) {
        CaseCommentCommand.this.commentId = commentId;
        CaseCommentCommand.this.author = author;
        CaseCommentCommand.this.updatedText = updatedText;
        CaseCommentCommand.this.update = true;
    }
    ...
    public Void execute(Context context) {
        ...
        if (add) {
            ...
            CommentInstance toUpdate = ((CaseFileInstanceImpl) (caseFile)).getComments().stream().filter(( c) -> c.getId().equals(commentId)).findFirst().get();
            if (!(CaseCommentCommand.this.author.equals(toUpdate.getAuthor()))) {
            ...
            CommentInstance toRemove = ((CaseFileInstanceImpl) (caseFile)).getComments().stream().filter(( c) -> c.getId().equals(commentId)).findFirst().get();
