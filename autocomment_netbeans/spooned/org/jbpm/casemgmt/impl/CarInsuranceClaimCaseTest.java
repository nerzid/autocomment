...
public class CarInsuranceClaimCaseTest extends AbstractCaseServicesBaseTest {
    ...
    @Before
    public void prepare() {
        System.setProperty("org.jbpm.document.storage", CarInsuranceClaimCaseTest.TEST_DOC_STORAGE);
        deleteFolder(CarInsuranceClaimCaseTest.TEST_DOC_STORAGE);
        configureServices();
    ...
    public void testCarInsuranceClaimCase() {
        ...
        try {
            ...
            assertCaseInstance(deploymentId, CarInsuranceClaimCaseTest.CAR_INS_CASE_ID);
            ...
            caseService.addDataToCaseFile(CarInsuranceClaimCaseTest.CAR_INS_CASE_ID, "claimReportDone", true);
            // we should be in another stage - Claim assessment
            assertClaimAssesmentStage();
            // let's trigger claim offer calculation
            caseService.triggerAdHocFragment(CarInsuranceClaimCaseTest.CAR_INS_CASE_ID, "Calculate claim", null);
            ...
            Collection<ProcessInstanceDesc> caseProcesInstances = caseRuntimeDataService.getProcessInstancesForCase(CarInsuranceClaimCaseTest.CAR_INS_CASE_ID, Arrays.asList(ProcessInstance.STATE_ACTIVE), new org.kie.internal.query.QueryContext());
            assertEquals(0, caseProcesInstances.size());
            caseId = null;
        } catch (Exception e) {
            CarInsuranceClaimCaseTest.logger.error("Unexpected error {}", e.getMessage(), e);
    ...
    public void testCarInsuranceClaimCaseWithContactByInsured() {
        ...
        try {
            ...
            caseService.triggerAdHocFragment(CarInsuranceClaimCaseTest.CAR_INS_CASE_ID, "Contacted by insured", null);
    ...
    public void testCarInsuranceClaimCaseWithAssessorInvolved() {
        ...
        try {
            ...
            caseService.assignToCaseRole(CarInsuranceClaimCaseTest.CAR_INS_CASE_ID, "assessor", new org.jbpm.services.task.impl.model.UserImpl("krisv"));
            // since the first task assigned to insured is with auto start it should be already active
            // the same task can be claimed by insuranceRepresentative in case claim is reported over phone
            long taskId = assertBuildClaimReportAvailableForBothRoles();
            // let's provide claim report with initial data
    ...
    protected String startAndAssertCaseInstance(String deploymentId, String insured, String insuranceRepresentative) {
        ...
        roleAssignments.put("insured", new org.jbpm.services.task.impl.model.UserImpl(insured));
        roleAssignments.put("insuranceRepresentative", new org.jbpm.services.task.impl.model.UserImpl(insuranceRepresentative));
        // start new instance of a case with data and role assignment
        Map<String, Object> data = new HashMap<>();
        CaseFileInstance caseFile = caseService.newCaseFileInstance(deploymentId, CarInsuranceClaimCaseTest.CAR_INSURANCE_CLAIM_PROC_ID, data, roleAssignments);
        String caseId = caseService.startCase(deploymentId, CarInsuranceClaimCaseTest.CAR_INSURANCE_CLAIM_PROC_ID, caseFile);
        assertNotNull(caseId);
        assertEquals(CarInsuranceClaimCaseTest.CAR_INS_CASE_ID, caseId);
        return caseId;
    }
    ...
    protected void assertBuildClaimReportStage() {
        Collection<CaseStageInstance> activeStages = caseRuntimeDataService.getCaseInstanceStages(CarInsuranceClaimCaseTest.CAR_INS_CASE_ID, true, new org.kie.internal.query.QueryContext());
    ...
    protected long assertTasksForBothRoles(String taskName, String actor1, String actor2, Status status) {
        List<TaskSummary> tasks = runtimeDataService.getTasksAssignedAsPotentialOwner(actor1, new org.kie.internal.query.QueryFilter());
        assertNotNull(tasks);
        assertEquals(1, tasks.size());
        assertTask(tasks.get(0), null, taskName, status);
        // the same task can be claimed by insuranceRepresentative in case claim is reported over phone
        tasks = runtimeDataService.getTasksAssignedAsPotentialOwner(actor2, new org.kie.internal.query.QueryFilter());
        assertNotNull(tasks);
        assertEquals(1, tasks.size());
        assertTask(tasks.get(0), null, taskName, status);
    ...
    protected void provideAndAssertClaimReport(Long taskId) {
        ...
        // claim report should be stored in case file data
        CaseFileInstance caseFile = caseService.getCaseFileInstance(CarInsuranceClaimCaseTest.CAR_INS_CASE_ID);
    ...
    protected void attachAndAssertPoliceReport() {
        caseService.triggerAdHocFragment(CarInsuranceClaimCaseTest.CAR_INS_CASE_ID, "Submit police report", null);
        List<TaskSummary> tasks = runtimeDataService.getTasksAssignedAsPotentialOwner("john", new org.kie.internal.query.QueryFilter());
        assertNotNull(tasks);
    ...
    protected void attachAndAssertPoliceReport(boolean callback, String callbackAfter) {
        List<TaskSummary> tasks = runtimeDataService.getTasksOwned("mary", new org.kie.internal.query.QueryFilter());
        assertNotNull(tasks);
        assertEquals(1, tasks.size());
        ...
        if (callback) {
            ...
            tasks = runtimeDataService.getTasksOwned("mary", new org.kie.internal.query.QueryFilter());
            assertNotNull(tasks);
            assertEquals(1, tasks.size());
    ...
    protected void assertAndNegotiateClaimOffer(int numberOfNegotiations) {
        ...
        while (numberOfNegotiations > 0) {
            ...
            caseService.triggerAdHocFragment(CarInsuranceClaimCaseTest.CAR_INS_CASE_ID, "Negotiation meeting", params);
    ...
    private void assertAndRunClaimAssessment() {
        List<TaskSummary> tasks = runtimeDataService.getTasksAssignedAsPotentialOwner("krisv", new org.kie.internal.query.QueryFilter());
        assertNotNull(tasks);
        assertEquals(1, tasks.size());
