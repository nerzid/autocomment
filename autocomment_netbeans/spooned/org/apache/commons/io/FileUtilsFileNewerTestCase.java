...
public class FileUtilsFileNewerTestCase extends FileBasedTestCase {
    ...
    public void setUp() throws Exception {
        ...
        try {
            TestUtils.generateTestData(output1, ((long) (FileUtilsFileNewerTestCase.FILE1_SIZE)));
        } finally {
        ...
        try {
            TestUtils.generateTestData(output, ((long) (FileUtilsFileNewerTestCase.FILE2_SIZE)));
        } finally {
    ...
    protected void testIsFileNewer(final String description, final File file, final long time, final boolean wantedResult) {
        Assert.assertEquals((description + " - time"), wantedResult, FileUtils.isFileNewer(file, time));
        Assert.assertEquals((description + " - date"), wantedResult, FileUtils.isFileNewer(file, new Date(time)));
        final File temporaryFile = m_testFile2;
        temporaryFile.setLastModified(time);
        Assert.assertEquals("The temporary file hasn't the right last modification date", time, temporaryFile.lastModified());
        Assert.assertEquals((description + " - file"), wantedResult, FileUtils.isFileNewer(file, temporaryFile));
    }
    ...
    public void testIsFileNewerNoFile() {
        ...
        Assert.fail("File not specified");
    }
    ...
    public void testIsFileNewerNoDate() {
        ...
        Assert.fail("Date not specified");
    }
    ...
    public void testIsFileNewerNoFileReference() {
        ...
        Assert.fail("Reference file not specified");
    }
}
