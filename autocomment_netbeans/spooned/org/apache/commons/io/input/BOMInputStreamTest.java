...
public class BOMInputStreamTest {
    ...
    // Test cases
    // ----------------------------------------------------------------------------
    private void parseXml(final InputStream in) throws IOException, ParserConfigurationException, SAXException {
    ...
    private void parseXml(final Reader in) throws IOException, ParserConfigurationException, SAXException {
        final Document doc = DocumentBuilderFactory.newInstance().newDocumentBuilder().parse(new InputSource(in));
        assertNotNull(doc);
        assertEquals("X", doc.getFirstChild().getNodeName());
    }
    ...
    private void readBOMInputStreamTwice(final String resource) throws Exception {
        final InputStream inputStream = BOMInputStreamTest.this.getClass().getResourceAsStream(resource);
        Assert.Assert.assertNotNull(inputStream);
        final BOMInputStream bomInputStream = new BOMInputStream(inputStream);
        bomInputStream.mark(1000000);
        BOMInputStreamTest.this.readFile(bomInputStream);
    ...
    public void testClose() throws Exception {
        final BOMInputStreamTest.ExpectCloseInputStream del = new BOMInputStreamTest.ExpectCloseInputStream();
    ...
    public void testGetBOMFirstThenReadInclude() throws Exception {
        ...
        assertTrue("hasBOM(UTF-8)", in.hasBOM(ByteOrderMark.UTF_8));
        assertEquals("getBOM", ByteOrderMark.UTF_8, in.getBOM());
        assertEquals(239, in.read());
        assertEquals(187, in.read());
        assertEquals(191, in.read());
        assertEquals('A', in.read());
        assertEquals('B', in.read());
        assertEquals('C', in.read());
        assertEquals((-1), in.read());
    ...
    public void testLeadingNonBOMSingleRead() throws Exception {
        ...
        final InputStream in = new BOMInputStream(createUtf8DataStream(data, false));
        assertEquals(239, in.read());
        assertEquals(171, in.read());
        assertEquals(205, in.read());
        assertEquals((-1), in.read());
        in.close();
    }
    ...
    public void testReadTwiceWithBOM() throws Exception {
        BOMInputStreamTest.this.readBOMInputStreamTwice("/org/apache/commons/io/testfileBOM.xml");
    }

    @Test
    public void testReadTwiceWithoutBOM() throws Exception {
        BOMInputStreamTest.this.readBOMInputStreamTwice("/org/apache/commons/io/testfileNoBOM.xml");
    }
    ...
    public void testSupportCode() throws Exception {
        ...
        assertEquals(5, len);
        assertEquals(239, ((buf[0]) & 255));
        assertEquals(187, ((buf[1]) & 255));
        assertEquals(191, ((buf[2]) & 255));
        assertEquals('A', ((buf[3]) & 255));
        assertEquals('B', ((buf[4]) & 255));
        assertData(new byte[]{ ((byte) (239)) , ((byte) (187)) , ((byte) (191)) , 'A' , 'B' }, buf, len);
    }

    private boolean jvmAndSaxBothSupportCharset(String charSetName) throws IOException, ParserConfigurationException, SAXException {
        return (Charset.isSupported(charSetName)) && (doesSaxSupportCharacterSet(charSetName));
    }

    private boolean doesSaxSupportCharacterSet(String charSetName) throws IOException, ParserConfigurationException, SAXException {
