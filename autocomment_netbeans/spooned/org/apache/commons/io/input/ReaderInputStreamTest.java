...
public class ReaderInputStreamTest {
    ...
    static {
        ...
        for (int i = 0; i < 100; i++) {
            buffer.append(ReaderInputStreamTest.TEST_STRING);
        }
    ...
    private void testWithBufferedRead(final String testString, final String charsetName) throws IOException {
        ...
        while (true) {
            ...
            if (read == (-1)) {
                ...
                Assert.assertTrue((read <= bufferLength));
                while (read > 0) {
                    Assert.assertTrue((offset < (expected.length)));
                    Assert.assertEquals(expected[offset], buffer[bufferOffset]);
                    offset++;
                    bufferOffset++;
                    read--;
                }
    ...
    public void testUTF8WithSingleByteRead() throws IOException {
        testWithSingleByteRead(ReaderInputStreamTest.TEST_STRING, "UTF-8");
    }

    @Test
    public void testLargeUTF8WithSingleByteRead() throws IOException {
        testWithSingleByteRead(ReaderInputStreamTest.LARGE_TEST_STRING, "UTF-8");
    }

    @Test
    public void testUTF8WithBufferedRead() throws IOException {
        testWithBufferedRead(ReaderInputStreamTest.TEST_STRING, "UTF-8");
    }

    @Test
    public void testLargeUTF8WithBufferedRead() throws IOException {
        testWithBufferedRead(ReaderInputStreamTest.LARGE_TEST_STRING, "UTF-8");
    }

    @Test
    public void testUTF16WithSingleByteRead() throws IOException {
        testWithSingleByteRead(ReaderInputStreamTest.TEST_STRING, "UTF-16");
    }
    ...
    public void testReadZero() throws Exception {
        ...
        Assert.assertEquals(inStr.length(), r.read(bytes, 0, ((inStr.length()) + 1)));
    ...
    public void testReadZeroEmptyString() throws Exception {
        ...
        // Should always return 0 for length == 0
        Assert.assertEquals(0, r.read(bytes, 0, 0));
        Assert.assertEquals((-1), r.read(bytes, 0, 1));
        Assert.assertEquals(0, r.read(bytes, 0, 0));
        Assert.assertEquals((-1), r.read(bytes, 0, 1));
        r.close();
    }
