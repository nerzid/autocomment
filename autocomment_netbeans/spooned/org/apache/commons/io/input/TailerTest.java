...
public class TailerTest extends FileBasedTestCase {
    ...
    public void testLongFile() throws Exception {
        ...
        final TailerTest.TestTailerListener listener = new TailerTest.TestTailerListener();
    ...
    public void testMultiByteBreak() throws Exception {
        ...
        final long delay = 50;
        final File origin = new File(TailerTest.this.getClass().getResource("/test-file-utf8.bin").toURI());
        ...
        try {
            ...
            Assert.assertEquals("line count", lines.size(), tailerlines.size());
    ...
    public void testTailerEof() throws Exception {
        ...
        try {
            ...
            List<String> lines = listener.getLines();
            Assert.assertEquals("1 line count", 0, lines.size());
            writeString(file, " one\n");
            TestUtils.sleep((delay * 2));
            lines = listener.getLines();
            Assert.assertEquals("1 line count", 1, lines.size());
            Assert.assertEquals("1 line 1", "Line one", lines.get(0));
            listener.clear();
        } finally {
            tailer.stop();
            TestUtils.sleep((delay * 2));
    ...
    public void testTailer() throws Exception {
        ...
        TestUtils.sleep(testDelayMillis);
        List<String> lines = listener.getLines();
        Assert.assertEquals("1 line count", 2, lines.size());
        Assert.assertEquals("1 line 1", "Line one", lines.get(0));
        Assert.assertEquals("1 line 2", "Line two", lines.get(1));
        listener.clear();
        // Write another line to the file
        write(file, "Line three");
        TestUtils.sleep(testDelayMillis);
        lines = listener.getLines();
        Assert.assertEquals("2 line count", 1, lines.size());
        Assert.assertEquals("2 line 3", "Line three", lines.get(0));
        listener.clear();
        // Check file does actually have all the lines
        lines = FileUtils.readLines(file, "UTF-8");
        Assert.assertEquals("3 line count", 3, lines.size());
        Assert.assertEquals("3 line 1", "Line one", lines.get(0));
        Assert.assertEquals("3 line 2", "Line two", lines.get(1));
        Assert.assertEquals("3 line 3", "Line three", lines.get(2));
        // Delete & re-create
        file.delete();
        ...
        TestUtils.sleep(testDelayMillis);
        lines = listener.getLines();
        Assert.assertEquals("4 line count", 1, lines.size());
        Assert.assertEquals("4 line 3", "Line four", lines.get(0));
        listener.clear();
        // Stop
        thread.interrupt();
        TestUtils.sleep((testDelayMillis * 4));
        write(file, "Line five");
        Assert.assertEquals("4 line count", 0, listener.getLines().size());
        Assert.assertNotNull("Missing InterruptedException", listener.exception);
        Assert.assertTrue(("Unexpected Exception: " + (listener.exception)), ((listener.exception) instanceof InterruptedException));
        Assert.assertEquals("Expected init to be called", 1, listener.initialised);
        Assert.assertEquals("fileNotFound should not be called", 0, listener.notFound);
        Assert.assertEquals("fileRotated should be be called", 1, listener.rotated);
        tailer.stop();
        tailer = null;
    }
    ...
    public void testTailerEndOfFileReached() throws Exception {
        ...
        Assert.assertTrue("end of file reached at least 3 times", ((listener.reachedEndOfFile) >= 3));
    }
    ...
    public void testStopWithNoFile() throws Exception {
        ...
        Assert.assertFalse("nosuchfile should not exist", file.exists());
        final TailerTest.TestTailerListener listener = new TailerTest.TestTailerListener();
        final int delay = 100;
        ...
        Assert.assertNull("Should not generate Exception", listener.exception);
        Assert.assertEquals("Expected init to be called", 1, listener.initialised);
        Assert.assertTrue("fileNotFound should be called", ((listener.notFound) > 0));
        Assert.assertEquals("fileRotated should be not be called", 0, listener.rotated);
        Assert.assertEquals("end of file never reached", 0, listener.reachedEndOfFile);
    }
    ...
    public void testIO335() throws Exception {
        ...
        Assert.assertEquals("line count", 4, lines.size());
        Assert.assertEquals("line 1", "CRLF", lines.get(0));
        Assert.assertEquals("line 2", "LF", lines.get(1));
        Assert.assertEquals("line 3", "CR", lines.get(2));
        Assert.assertEquals("line 4", "CRCR\r", lines.get(3));
        // Stop
        tailer.stop();
        tailer = null;
        thread.interrupt();
