...
public class XmlStreamReaderUtilitiesTest {
    ...
    private void checkContentTypeEncoding(final String expected, final String httpContentType) {
        Assert.assertEquals((("ContentTypeEncoding=[" + httpContentType) + "]"), expected, XmlStreamReader.getContentTypeEncoding(httpContentType));
    }
    ...
    private void checkContentTypeMime(final String expected, final String httpContentType) {
        Assert.assertEquals((("ContentTypeMime=[" + httpContentType) + "]"), expected, XmlStreamReader.getContentTypeMime(httpContentType));
    }
    ...
    private void checkAppXml(final boolean expected, final String mime) {
        Assert.assertEquals((("Mime=[" + mime) + "]"), expected, XmlStreamReader.isAppXml(mime));
    }
    ...
    private void checkTextXml(final boolean expected, final String mime) {
        Assert.assertEquals((("Mime=[" + mime) + "]"), expected, XmlStreamReader.isTextXml(mime));
    }
    ...
    private void checkRawEncoding(final String expected, final String bomEnc, final String xmlGuessEnc, final String xmlEnc, final String defaultEncoding) throws IOException {
        ...
        Assert.assertEquals(builder.toString(), expected, encoding);
    }
    ...
    protected String calculateRawEncoding(final String bomEnc, final String xmlGuessEnc, final String xmlEnc, final String defaultEncoding) throws IOException {
        final XmlStreamReaderUtilitiesTest.MockXmlStreamReader mock = new XmlStreamReaderUtilitiesTest.MockXmlStreamReader(defaultEncoding);
    ...
    private void checkRawError(final String msgSuffix, final String bomEnc, final String xmlGuessEnc, final String xmlEnc, final String defaultEncoding) {
        try {
            ...
            Assert.assertTrue(("Msg End: " + (e.getMessage())), e.getMessage().endsWith(msgSuffix));
            Assert.assertEquals("bomEnc", bomEnc, e.getBomEncoding());
            Assert.assertEquals("xmlGuessEnc", xmlGuessEnc, e.getXmlGuessEncoding());
            Assert.assertEquals("xmlEnc", xmlEnc, e.getXmlEncoding());
            Assert.assertNull("ContentTypeEncoding", e.getContentTypeEncoding());
            Assert.assertNull("ContentTypeMime", e.getContentTypeMime());
        } catch (final Exception e) {
            Assert.fail(("Expected XmlStreamReaderException, but threw " + e));
        }
    }
    ...
    private void checkHttpError(final String msgSuffix, final boolean lenient, final String httpContentType, final String bomEnc, final String xmlGuessEnc, final String xmlEnc, final String defaultEncoding) {
        try {
            ...
            Assert.assertEquals("xmlGuessEnc", xmlGuessEnc, e.getXmlGuessEncoding());
            Assert.assertEquals("xmlEnc", xmlEnc, e.getXmlEncoding());
            Assert.assertEquals("ContentTypeEncoding", XmlStreamReader.getContentTypeEncoding(httpContentType), e.getContentTypeEncoding());
            Assert.assertEquals("ContentTypeMime", XmlStreamReader.getContentTypeMime(httpContentType), e.getContentTypeMime());
        } catch (final Exception e) {
            Assert.fail(("Expected XmlStreamReaderException, but threw " + e));
        }
    }
