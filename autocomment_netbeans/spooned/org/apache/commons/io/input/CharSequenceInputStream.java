...
public class CharSequenceInputStream extends InputStream {
    ...
    public CharSequenceInputStream(final CharSequence cs, final Charset charset, final int bufferSize) {
        ...
        CharSequenceInputStream.this.bbuf.flip();
        this.cbuf = CharBuffer.wrap(cs);
        CharSequenceInputStream.this.mark_cbuf = CharSequenceInputStream.NO_MARK;
        CharSequenceInputStream.this.mark_bbuf = CharSequenceInputStream.NO_MARK;
    }
    ...
    public CharSequenceInputStream(final CharSequence cs, final Charset charset) {
        this(cs, charset, CharSequenceInputStream.BUFFER_SIZE);
    }
    ...
    private void fillBuffer() throws CharacterCodingException {
        CharSequenceInputStream.this.bbuf.compact();
        final CoderResult result = CharSequenceInputStream.this.encoder.encode(CharSequenceInputStream.this.cbuf, CharSequenceInputStream.this.bbuf, true);
        if (result.isError()) {
    ...
    public int read(final byte[] b, int off, int len) throws IOException {
        ...
        int bytesRead = 0;
        while (len > 0) {
            if (CharSequenceInputStream.this.bbuf.hasRemaining()) {
                final int chunk = Math.min(CharSequenceInputStream.this.bbuf.remaining(), len);
                CharSequenceInputStream.this.bbuf.get(b, off, chunk);
                off += chunk;
                len -= chunk;
                bytesRead += chunk;
            } else {
    ...
    public int read() throws IOException {
        for (; ;) {
            if (CharSequenceInputStream.this.bbuf.hasRemaining()) {
                return (CharSequenceInputStream.this.bbuf.get()) & 255;
            } 
            fillBuffer();
            if ((!(CharSequenceInputStream.this.bbuf.hasRemaining())) && (!(CharSequenceInputStream.this.cbuf.hasRemaining()))) {
                return IOUtils.EOF;
    ...
    public long skip(long n) throws IOException {
        ...
        while ((n > 0) && ((available()) > 0)) {
            CharSequenceInputStream.this.read();
            n--;
            skipped++;
        }
        return skipped;
    ...
    public int available() throws IOException {
        ...
        return (CharSequenceInputStream.this.bbuf.remaining()) + (CharSequenceInputStream.this.cbuf.remaining());
    }
    ...
    public synchronized void mark(final int readlimit) {
        CharSequenceInputStream.this.mark_cbuf = CharSequenceInputStream.this.cbuf.position();
        CharSequenceInputStream.this.mark_bbuf = CharSequenceInputStream.this.bbuf.position();
        CharSequenceInputStream.this.cbuf.mark();
        CharSequenceInputStream.this.bbuf.mark();
    ...
    public synchronized void reset() throws IOException {
        ...
        restart from there.
         */
        if ((CharSequenceInputStream.this.mark_cbuf) != (CharSequenceInputStream.NO_MARK)) {
            // if cbuf is at 0, we have not started reading anything, so skip re-encoding
            if ((CharSequenceInputStream.this.cbuf.position()) != 0) {
                CharSequenceInputStream.this.encoder.reset();
                CharSequenceInputStream.this.cbuf.rewind();
                CharSequenceInputStream.this.bbuf.rewind();
                CharSequenceInputStream.this.bbuf.limit(0);// rewind does not clear the buffer
                
                while ((CharSequenceInputStream.this.cbuf.position()) < (CharSequenceInputStream.this.mark_cbuf)) {
                    CharSequenceInputStream.this.bbuf.rewind();// empty the buffer (we only refill when empty during normal processing)
                    
                    CharSequenceInputStream.this.bbuf.limit(0);
                    fillBuffer();
                }
            } 
            if ((CharSequenceInputStream.this.cbuf.position()) != (CharSequenceInputStream.this.mark_cbuf)) {
                throw new IllegalStateException((((("Unexpected CharBuffer postion: actual=" + (cbuf.position())) + " ") + "expected=") + (CharSequenceInputStream.this.mark_cbuf)));
            } 
            CharSequenceInputStream.this.bbuf.position(CharSequenceInputStream.this.mark_bbuf);
            CharSequenceInputStream.this.mark_cbuf = CharSequenceInputStream.NO_MARK;
            CharSequenceInputStream.this.mark_bbuf = CharSequenceInputStream.NO_MARK;
        } 
    }
