...
public class CharSequenceInputStreamTest {
    ...
    static {
        ...
        for (int i = 0; i < 100; i++) {
            buffer.append(CharSequenceInputStreamTest.TEST_STRING);
        }
    ...
    private void testBufferedRead(final String testString, final String charsetName) throws IOException {
        ...
        try {
            ...
            while (true) {
                ...
                if (read == (-1)) {
                    Assert.assertEquals(("EOF: offset should equal length for charset " + charsetName), expected.length, offset);
                    break;
                } else {
                    Assert.assertTrue(((("Read " + read) + " <= ") + bufferLength), (read <= bufferLength));
                    while (read > 0) {
                        Assert.assertTrue(((((("offset for " + charsetName) + " ") + offset) + " < ") + (expected.length)), (offset < (expected.length)));
                        Assert.assertEquals(("bytes should agree for " + charsetName), expected[offset], buffer[bufferOffset]);
                        offset++;
                        bufferOffset++;
                        read--;
                    }
    ...
    public void testBufferedRead_AvailableCharset() throws IOException {
        for (final String csName : Charset.availableCharsets().keySet()) {
            ...
            if (isAvailabilityTestableForCharset(csName)) {
                testBufferedRead(CharSequenceInputStreamTest.TEST_STRING, csName);
            } 
        }
    }
    ...
    public void testBufferedRead_UTF8() throws IOException {
        testBufferedRead(CharSequenceInputStreamTest.TEST_STRING, "UTF-8");
    }
    ...
    private void testIO_356(final int bufferSize, final int dataSize, final int readFirst, final String csName) throws Exception {
        final CharSequenceInputStream is = new CharSequenceInputStream(CharSequenceInputStreamTest.ALPHABET, csName, bufferSize);
        ...
        Assert.assertEquals(dataSize, readCount1);
        is.reset();// should allow data to be re-read
        
        final byte[] data2 = new byte[dataSize];
        final int readCount2 = is.read(data2);
        Assert.assertEquals(dataSize, readCount2);
        is.close();
        // data buffers should be identical
        Assert.assertArrayEquals(((("bufferSize=" + bufferSize) + " dataSize=") + dataSize), data1, data2);
    }
    ...
    public void testLargeBufferedRead_RequiredCharsets() throws IOException {
        for (final String csName : getRequiredCharsetNames()) {
            testBufferedRead(CharSequenceInputStreamTest.LARGE_TEST_STRING, csName);
        }
    }

    @Test
    public void testLargeBufferedRead_UTF8() throws IOException {
        testBufferedRead(CharSequenceInputStreamTest.LARGE_TEST_STRING, "UTF-8");
    }

    @Test
    public void testLargeSingleByteRead_RequiredCharsets() throws IOException {
        for (final String csName : getRequiredCharsetNames()) {
            testSingleByteRead(CharSequenceInputStreamTest.LARGE_TEST_STRING, csName);
        }
    }

    @Test
    public void testLargeSingleByteRead_UTF8() throws IOException {
        testSingleByteRead(CharSequenceInputStreamTest.LARGE_TEST_STRING, "UTF-8");
    }
    ...
    private void testMarkReset(final String csName) throws Exception {
        ...
        try {
            Assert.assertEquals(2, r.skip(2));
            r.mark(0);
            Assert.assertEquals(csName, 's', r.read());
            Assert.assertEquals(csName, 't', r.read());
            Assert.assertEquals(csName, (-1), r.read());
            r.reset();
            Assert.assertEquals(csName, 's', r.read());
            Assert.assertEquals(csName, 't', r.read());
            Assert.assertEquals(csName, (-1), r.read());
    ...
    public void testSingleByteRead_RequiredCharsets() throws IOException {
        for (final String csName : getRequiredCharsetNames()) {
            testSingleByteRead(CharSequenceInputStreamTest.TEST_STRING, csName);
        }
    }

    @Test
    public void testSingleByteRead_UTF16() throws IOException {
        testSingleByteRead(CharSequenceInputStreamTest.TEST_STRING, "UTF-16");
    }

    @Test
    public void testSingleByteRead_UTF8() throws IOException {
        testSingleByteRead(CharSequenceInputStreamTest.TEST_STRING, "UTF-8");
    }
    ...
    private void testSkip(final String csName) throws Exception {
        final InputStream r = new CharSequenceInputStream("test", csName);
        try {
            Assert.assertEquals(1, r.skip(1));
            Assert.assertEquals(2, r.skip(2));
            Assert.assertEquals(csName, 't', r.read());
            r.skip(100);
            Assert.assertEquals(csName, (-1), r.read());
    ...
    private int checkAvail(InputStream is, int min) throws Exception {
        ...
        Assert.assertTrue(((("avail should be >= " + min) + ", but was ") + available), (available >= min));
        return available;
    }
    ...
    private void testAvailableSkip(final String csName) throws Exception {
        ...
        try {
            ...
            Assert.assertEquals((available - 1), r.skip((available - 1)));// skip all but one
    ...
    private void testAvailableRead(final String csName) throws Exception {
        ...
        try {
            ...
            Assert.assertEquals(available, r.read(buff, 0, available));
        } finally {
            r.close();
        }
    }
    ...
    public void testAvailable() throws Exception {
        for (final String csName : Charset.availableCharsets().keySet()) {
            ...
            try {
                ...
                Assert.fail(("Operation not supported for " + csName));
            }
        }
    }
