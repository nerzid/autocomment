...
public class ByteArrayOutputStreamTestCase {
    ...
    private int writeData(final ByteArrayOutputStream baout, final java.io.ByteArrayOutputStream ref, final int count) {
        if (count > (ByteArrayOutputStreamTestCase.DATA.length)) {
        ...
        if (count == 0) {
            ...
            baout.write(ByteArrayOutputStreamTestCase.DATA, 0, count);
            ref.write(ByteArrayOutputStreamTestCase.DATA, 0, count);
            return count;
        }
    }
    ...
    private void checkByteArrays(final byte[] expected, final byte[] actual) {
        if ((expected.length) != (actual.length)) {
            Assert.fail("Resulting byte arrays are not equally long");
        } 
        if (!(ByteArrayOutputStreamTestCase.byteCmp(expected, actual))) {
            Assert.fail("Resulting byte arrays are not equal");
        } 
    }

    private void checkStreams(final ByteArrayOutputStream actual, final java.io.ByteArrayOutputStream expected) {
        Assert.assertEquals("Sizes are not equal", expected.size(), actual.size());
    ...
    public void testToInputStream() throws IOException {
        ...
        Assert.assertEquals(8224, baoutData.length);
        ...
        Assert.assertEquals(8254, baoutData.length);
        checkByteArrays(refData, baoutData);
        baout.close();
        in.close();
    }
    ...
    public void testToInputStreamWithReset() throws IOException {
        ...
        baoutData = IOUtils.toByteArray(baout.toInputStream());
        refData = ref.toByteArray();
        Assert.assertEquals(30, baoutData.length);
        checkByteArrays(refData, baoutData);
        baout.close();
        in.close();
    }
    ...
    public void testStream() throws Exception {
        ...
        Assert.assertEquals(36, written);
        ...
        Assert.assertEquals(32, written);
        checkStreams(baout, ref);
        ...
        Assert.assertEquals(155, written);
        checkStreams(baout, ref);
        ...
        Assert.assertEquals("ASCII decoded String must be equal", refString, baoutString);
        // Make sure that empty ByteArrayOutputStreams really don't create garbage
        // on toByteArray()
        final ByteArrayOutputStream baos1 = new ByteArrayOutputStream();
        final ByteArrayOutputStream baos2 = new ByteArrayOutputStream();
        Assert.assertSame(baos1.toByteArray(), baos2.toByteArray());
        baos1.close();
        baos2.close();
        baout.close();
        baout1.close();
