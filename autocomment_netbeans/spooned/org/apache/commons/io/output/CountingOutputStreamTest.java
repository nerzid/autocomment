...
public class CountingOutputStreamTest {
    ...
    public void testCounting() throws IOException {
        ...
        Assert.assertEquals("CountingOutputStream.getCount()", cos.getCount(), 20);
        ...
        Assert.assertEquals("CountingOutputStream.getCount()", cos.getCount(), 30);
        for (int i = 25; i < 35; i++) {
            array[(i - 25)] = ((byte) (i));
        }
        cos.write(array, 5, 5);
        assertByteArrayEquals("CountingOutputStream.write(byte[], int, int)", baos.toByteArray(), 0, 35);
        Assert.assertEquals("CountingOutputStream.getCount()", cos.getCount(), 35);
        final int count = cos.resetCount();
        Assert.assertEquals("CountingOutputStream.resetCount()", count, 35);
        for (int i = 0; i < 10; i++) {
            cos.write(i);
        }
        assertByteArrayEquals("CountingOutputStream.write(int)", baos.toByteArray(), 35, 45);
        Assert.assertEquals("CountingOutputStream.getCount()", cos.getCount(), 10);
        cos.close();
    }
    ...
    public void testLargeFiles_IO84() throws Exception {
        ...
        // Test long methods
        IOUtils.copyLarge(mock, cos);
        Assert.assertEquals("getByteCount()", size, cos.getByteCount());
        Assert.assertEquals("resetByteCount()", size, cos.resetByteCount());
    }

    private void assertByteArrayEquals(final String msg, final byte[] array, final int start, final int end) {
        for (int i = start; i < end; i++) {
            Assert.assertEquals((((msg + ": array[") + i) + "] mismatch"), array[i], (i - start));
        }
    }
}
