...
public class TeeOutputStreamTest {
    ...
    public void testCloseBranchIOException() {
        final ByteArrayOutputStream badOs = new TeeOutputStreamTest.ExceptionOnCloseByteArrayOutputStream();
        final TeeOutputStreamTest.RecordCloseByteArrayOutputStream goodOs = new TeeOutputStreamTest.RecordCloseByteArrayOutputStream();
    ...
    private void assertByteArrayEquals(final String msg, final byte[] array1, final byte[] array2) {
        Assert.assertEquals((msg + ": array size mismatch"), array1.length, array2.length);
        for (int i = 0; i < (array1.length); i++) {
            Assert.assertEquals((((msg + ": array[ ") + i) + "] mismatch"), array1[i], array2[i]);
        }
    }
}
