...
public class IOUtils {
    ...
    public static void writeChunked(final byte[] data, final OutputStream output) throws IOException {
        if (data != null) {
            ...
            int offset = 0;
            while (bytes > 0) {
                int chunk = Math.min(bytes, IOUtils.DEFAULT_BUFFER_SIZE);
    ...
    public static long copyLarge(final InputStream input, final OutputStream output) throws IOException {
        return IOUtils.copy(input, output, IOUtils.DEFAULT_BUFFER_SIZE);
    }
    ...
    public static boolean contentEqualsIgnoreEOL(final Reader input1, final Reader input2) throws IOException {
        ...
        final BufferedReader br1 = IOUtils.toBufferedReader(input1);
        final BufferedReader br2 = IOUtils.toBufferedReader(input2);
    ...
    public static long skip(final InputStream input, final long toSkip) throws IOException {
        ...
        while (remain > 0) {
            ...
            final long n = input.read(IOUtils.SKIP_BYTE_BUFFER, 0, ((int) (Math.min(remain, IOUtils.SKIP_BUFFER_SIZE))));
            if (n < 0) {
                // EOF
                break;
            } 
    ...
    public static long skip(final ReadableByteChannel input, final long toSkip) throws IOException {
        ...
        final ByteBuffer skipByteBuffer = ByteBuffer.allocate(((int) (Math.min(toSkip, IOUtils.SKIP_BUFFER_SIZE))));
        long remain = toSkip;
        while (remain > 0) {
            skipByteBuffer.position(0);
            skipByteBuffer.limit(((int) (Math.min(remain, IOUtils.SKIP_BUFFER_SIZE))));
    ...
    public static long skip(final Reader input, final long toSkip) throws IOException {
        ...
        while (remain > 0) {
            // See https://issues.apache.org/jira/browse/IO-203 for why we use read() rather than delegating to skip()
            final long n = input.read(IOUtils.SKIP_CHAR_BUFFER, 0, ((int) (Math.min(remain, IOUtils.SKIP_BUFFER_SIZE))));
            if (n < 0) {
                // EOF
                break;
            } 
    ...
    public static void skipFully(final InputStream input, final long toSkip) throws IOException {
        ...
        final long skipped = IOUtils.skip(input, toSkip);
        if (skipped != toSkip) {
    ...
    public static void readFully(final Reader input, final char[] buffer, final int offset, final int length) throws IOException {
        final int actual = IOUtils.read(input, buffer, offset, length);
        if (actual != length) {
    ...
    public static void readFully(final ReadableByteChannel input, final ByteBuffer buffer) throws IOException {
        ...
        final int actual = IOUtils.read(input, buffer);
        if (actual != expected) {
