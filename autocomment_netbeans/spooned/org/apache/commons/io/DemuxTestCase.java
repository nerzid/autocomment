...
public class DemuxTestCase {
    ...
    private String getOutput(final String threadName) {
        ...
        Assert.assertNotNull("getOutput()", output);
        return output.toString(Charsets.UTF_8);
    }

    private String getInput(final String threadName) {
        final DemuxTestCase.ReaderThread thread = ((DemuxTestCase.ReaderThread) (m_threadMap.get(threadName)));
        Assert.assertNotNull("getInput()", thread);
    ...
    private void startWriter(final String name, final String data, final DemuxOutputStream demux) throws Exception {
        ...
        final DemuxTestCase.WriterThread thread = new DemuxTestCase.WriterThread(name, data, output, demux);
    ...
    private void startReader(final String name, final String data, final DemuxInputStream demux) throws Exception {
        ...
        final DemuxTestCase.ReaderThread thread = new DemuxTestCase.ReaderThread(name, input, demux);
        m_threadMap.put(name, thread);
    }
    ...
    public void testOutputStream() throws Exception {
        ...
        startWriter(DemuxTestCase.T1, DemuxTestCase.DATA1, output);
        startWriter(DemuxTestCase.T2, DemuxTestCase.DATA2, output);
        startWriter(DemuxTestCase.T3, DemuxTestCase.DATA3, output);
        startWriter(DemuxTestCase.T4, DemuxTestCase.DATA4, output);
        doStart();
        doJoin();
        Assert.assertEquals("Data1", DemuxTestCase.DATA1, getOutput(DemuxTestCase.T1));
        Assert.assertEquals("Data2", DemuxTestCase.DATA2, getOutput(DemuxTestCase.T2));
        Assert.assertEquals("Data3", DemuxTestCase.DATA3, getOutput(DemuxTestCase.T3));
        Assert.assertEquals("Data4", DemuxTestCase.DATA4, getOutput(DemuxTestCase.T4));
    }

    @Test
    public void testInputStream() throws Exception {
        final DemuxInputStream input = new DemuxInputStream();
        startReader(DemuxTestCase.T1, DemuxTestCase.DATA1, input);
        startReader(DemuxTestCase.T2, DemuxTestCase.DATA2, input);
        startReader(DemuxTestCase.T3, DemuxTestCase.DATA3, input);
        startReader(DemuxTestCase.T4, DemuxTestCase.DATA4, input);
        doStart();
        doJoin();
        Assert.assertEquals("Data1", DemuxTestCase.DATA1, getInput(DemuxTestCase.T1));
        Assert.assertEquals("Data2", DemuxTestCase.DATA2, getInput(DemuxTestCase.T2));
        Assert.assertEquals("Data3", DemuxTestCase.DATA3, getInput(DemuxTestCase.T3));
        Assert.assertEquals("Data4", DemuxTestCase.DATA4, getInput(DemuxTestCase.T4));
    }
        ...
        public void run() {
            ...
            try {
                ...
                while ((-1) != ch) {
                    ...
                    final int sleepTime = Math.abs(((DemuxTestCase.c_random.nextInt()) % 10));
