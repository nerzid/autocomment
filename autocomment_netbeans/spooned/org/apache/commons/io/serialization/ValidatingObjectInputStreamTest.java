...
public class ValidatingObjectInputStreamTest extends ClosingBase {
    ...
    private static final ClassNameMatcher ALWAYS_TRUE = new ClassNameMatcher() {
        @Override
    ...
    private void assertSerialization(ObjectInputStream ois) throws IOException, ClassNotFoundException {
    ...
    public void exceptionIncludesClassName() throws Exception {
        try {
            ...
            Assert.assertTrue(("Expecting message to contain " + name), ice.getMessage().contains(name));
        }
    }

    @Test
    public void acceptCustomMatcher() throws Exception {
        assertSerialization(willClose(new ValidatingObjectInputStream(testStream)).accept(ValidatingObjectInputStreamTest.ALWAYS_TRUE));
    }

    @Test(expected = InvalidClassException.class)
    public void rejectCustomMatcher() throws Exception {
        assertSerialization(willClose(new ValidatingObjectInputStream(testStream)).accept(MockSerializedClass.class).reject(ValidatingObjectInputStreamTest.ALWAYS_TRUE));
    }
